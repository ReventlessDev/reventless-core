...
- `@retired` — names the boolean `@schema type state` field whose truth withdraws the row from ordinary use (a deactivated customer, an archived category). Placed **before the field name**, on ReadModel / StateViewSlice specs. Payload forms: bare, `@retired("Archived")`, or `@retired({label: "Archived", showWhenFalse: true})`. Both halves come from the one annotation — a presentation half on the schema and an enforcement half in the resolvers — which is what separates it from `@groupBy`.
  ...
  - **Annotation or nothing** — no `@status`-style fallback by field name, since guessing wrong makes rows disappear for every non-elevated caller.
