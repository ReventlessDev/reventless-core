...
- `@@reventless.gwt` — on GWT test files. Auto-injects `include ReventlessGwt.<Kind>_GWT.Make(<Spec>)` directly after the first top-level module in the structure. `<Kind>` derived from filename or folder path (substring match, longest wins): `AutomationSlice`, `InboundTranslationSlice`, `OutboundTranslationSlice`, `StateChangeSlice`, `StateViewSlice`, `Projection`, `Behavior`. Spec auto-inferred as the first top-level module. Convention: place the attribute at the top of the file; actual injection position is always after the relevant module, independent of where the attribute appears.
- `@@reventless.gwt(SpecModule)` — explicit Spec; Kind still from filename. Use when the file has multiple top-level modules and the first isn't the right Spec.
