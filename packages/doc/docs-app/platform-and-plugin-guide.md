...
The plugin's **composition root** is `src/Plugin.res` — auto-generated by `generate-plugin` (from `reventless-spec`) before each build. A developer creates files in the right folders; the next `pnpm run build` regenerates and compiles `Plugin.res` automatically.
