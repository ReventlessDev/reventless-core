...
Each `Aggregate` can specify `EventMapping`s from one or more source Aggregates.
...
```rescript title="Customer_Mappings.res" showLineNumbers
@@reventless.mappings
...
See [Counter](/framework/runtime-components/counter#counter-in-eventmappings) component for further details.
...
See [CommandTopic](/framework/runtime-components/commandtopic#sync-vs-async) for the channel-level details of how each flavor is wired.
...
- **[EventLog](/framework/runtime-components/eventlog)** - Stores events generated by the Aggregate
- **[CommandTopic](/framework/runtime-components/commandtopic)** - Delivers commands to the Aggregate
- **[EventTopic](/framework/runtime-components/eventtopic)** - Distributes events from the Aggregate's EventLog
- **[CommandGenerator](/framework/runtime-components/commandgenerator)** - Generates commands for the Aggregate from API mutations
- **[EventMapper](/framework/runtime-components/eventmapper)** - Maps events from other Aggregates to commands for this Aggregate
...
- **[Counter](/framework/runtime-components/counter)** - Used by EventMappers for deduplication when mapping to this Aggregate
