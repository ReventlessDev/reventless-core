...
This component follows the Reventless [Component Structure Pattern](/framework/internals/component-structure-pattern), using separate files for interface definitions (`CommandGenerator.res`), builder logic (`CommandGenerator_Builder.res`), and runtime callbacks (`CommandGenerator_Callback.res`).
:::
...
Define the GraphQL schema for your aggregate's mutations (see [API](/app/graphql-api-guide) for complete details):
...
- **[Aggregate](/app/components/aggregate)** - Receives commands from CommandGenerator
- **[CommandTopic](./commandtopic.md)** - Receives generated commands for delivery
- **[API](/app/graphql-api-guide)** - Defines GraphQL schema for mutations
- **[Behavior](/app/components/aggregate#behavior)** - Defines aggregate business logic (decide, evolve)
- **[EventMapper](./eventmapper.md)** - Alternative command source (from events)

## AWS Implementation

For detailed AWS implementation, see [CommandGenerator AWS Adapter Documentation](/infrastructure/aws/adapters/commandgenerator).