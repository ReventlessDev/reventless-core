...
assert_js_contains "$JS" 'DcbTag$Reventless.markDeclaredForKey(CustomerId$TestDcb.schema, "sellerId")' "@dcbTag(key) on an identity keeps the explicit key"
...
assert_js_contains    "$JS" 'DcbTag$Reventless.declared' "@dcbTag: tag marked as declared"
...
assert_js_contains "$JS" 'declaredForKey' "@dcbTag(key): declaredForKey constructor emitted"
