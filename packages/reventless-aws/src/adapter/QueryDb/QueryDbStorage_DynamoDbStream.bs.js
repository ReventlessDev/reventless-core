...
function make(name, indexes, subIdField, ttl, api, apiRole, opts) {
  var table = Util_DynamoDbStream$ReventlessAws.makeTable(QueryDbStorage_DynamoDb$ReventlessAws.attributes(subIdField, indexes), Caml_option.some(QueryDbStorage_DynamoDb$ReventlessAws.globalSecondaryIndexes(indexes)), ttl, subIdField, "NEW_AND_OLD_IMAGES", Caml_option.some(Js_dict.fromArray([
                [
                  "Name",
                  name
                ],
