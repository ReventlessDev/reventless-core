...
function make(name, indexes, subIdField, ttl, api, apiRole, opts) {
  var table = Util_DynamoDbStream$ReventlessAws.makeTable(QueryDbStorage_DynamoDb$ReventlessAws.attributes(subIdField, indexes), QueryDbStorage_DynamoDb$ReventlessAws.globalSecondaryIndexes(indexes), ttl, subIdField, "NEW_AND_OLD_IMAGES", AWS_Tags$ReventlessAws.make(name, QueryDb$Reventless.componentType), opts, name);
  return {
