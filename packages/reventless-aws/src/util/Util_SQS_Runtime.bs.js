...
async function sendBatch(queue, queueService, commandJsons) {
  var sendResult = await Promise.allSettled(Message$Reventless.log(SQS$AwsSdk.sendMessageBatch(queue.id.get(), Belt_Array.map(commandJsons, (function (commandJson) {
                      return makeEntry(queueService, commandJson);
                    }))), "sendMessageBatch result:"));
