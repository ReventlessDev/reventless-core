...
function batchWrite(params) {
  return DynamoDb_DocumentClient$AwsSdk.make().batchWrite(params).promise();
}
...
function hasUnprocessedItems(writeOutput) {
  return Object.keys(writeOutput._UnprocessedItems).length !== 0;
}
...
async function retryBatchWriteIfNecessary(p, allItems, numberOfRetries, maxRetries) {
  ...
  var unprocessedItems = writeOutput._UnprocessedItems;
