...
function Make(Spec) {
  return (function (CommandTopicAdapter) {
      return (function (EventTopicAdapter) {
          ...
          var mapIncomingCommands = function (commands$prime, mappings, scheduler, queryEngine, queue) {
            return Belt_Array.concatMany(Belt_Array.map(mappings, (function (Mapping) {
                              return Curry._4(Mapping.mapIncomingCommands, commands$prime, Schedule$Reventless.create(scheduler, queue), Schedule$Reventless.$$delete(scheduler, queue), queryEngine);
                            })));
          };
          ...
          var construct = function (mappings, scheduler, queryEngine, self, name, resources) {
            ...
            var applyCommandAction = function (param) {
              if (param.tag) {
                ...
                var __x$1 = JSON.stringify(param[2]);
                var __x$2 = SQS$AwsSdk.sendMessage(Util_Aggregate$Reventless.commandTopicConnectorResource(resources, param[0]).id.get(), __x$1, param[1], undefined, undefined, /* () */0);
                return __x$2.catch((function (err) {
                              return Promise.resolve((console.log("ExtensionPoint: Error on publish command:", err), /* () */0));
                            }));
              }
            };
            ...
            var incomingCommandsHandler = function (_id, cmds$primeJson) {
              var commandTopic$1 = Belt_Option.getExn(commandTopic[0]);
              var queue = Component$Reventless.extractOutputs(commandTopic$1).connector;
              var commandActions = mapIncomingCommands(cmds$primeJson, mappings, scheduler, queryEngine, queue);
              return Promise.all(Belt_Array.map(commandActions, applyCommandAction)).then((function (param) {
                            return Promise.resolve(/* () */0);
                          }));
            };
