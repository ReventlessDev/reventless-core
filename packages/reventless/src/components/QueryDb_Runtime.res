...
module Make = (Spec: ReventlessSpec.ReadModel_Spec.T) => {
  let decode = (id, item) =>
  ...
  let deleteState = delete => async (id, subId) => await delete(id->Spec.Id.toString, subId)

  let deleteStates = deleteBatch =>
    async ids => {
