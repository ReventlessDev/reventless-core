...
function Make(Config, QueryDbStorage, Handler) {
  var construct = function (counterEventsHandler, ttl, self, name) {
    ...
    var handler = Handler.make(name, name$1, Component$Reventless.extractOutputs(referencesDb), name$2, Component$Reventless.extractOutputs(countsDb), Counter_Runtime$Reventless.counterHandler(name, CountsDb.count(countsDb), counterEventsHandler), opts2);
    self.count = (function (countItems) {
        return Counter_Runtime$Reventless.count(ttl)(ReferencesDb.saveBatch(referencesDb), countItems);
      });
    self.addToCounterTarget = handler.addToCounterTarget;
    var outputs = {
      referencesDb: ReferencesDb.outputs(referencesDb),
      countsDb: CountsDb.outputs(countsDb)
    };
