...
function eventsToJson(events$p, specIdEncode, specEventEncode, id) {
  return Belt_Array.map(events$p, (function ($$event) {
                return eventToJson(specIdEncode, specEventEncode, id, $$event);
              }));
}

function storageAppendErrorHandler(specName, specIdToString, id, err) {
  var errMsg = "EventLog: Error: Couldn't append for " + specName + "(" + specIdToString(id) + "):" + Util_Error$Reventless.message(undefined, err);
  console.log(errMsg);
  return {
          TAG: "Error",
          _0: errMsg
...
async function publishToEventTopic(eventTopicPublish, specIdToString, id, events$p) {
  try {
    ...
    if (err.RE_EXN_ID === Js_exn.$$Error) {
      var err$1 = err._1;
      var msg = "EventLog.appendFn(" + specIdToString(id) + "): EventTopic.publish Error: ";
...
function appendFn(storageAppend, specIdToString, specIdEncode, specEventEncode, eventTopicPublish, specName) {
  return async function (sequenceNr, id, events$p) {
    try {
      ...
      var appendResult;
      try {
        appendResult = await storageAppend(sequenceNr, specIdToString(id), eventsJson);
        exit = 1;
      }
      catch (raw_e){
...
function replayFn(storageReplay, specIdToString, specEventDecode) {
  return async function (id) {
    var jsonEvents = await storageReplay(specIdToString(id));
    var id$1 = specIdToString(id);
