        ...
        var outputs = {
          outputs: (
              Interstack$Reventless.coreStackOutput !== undefined ? Caml_option.valFromOption(Interstack$Reventless.coreStackOutput) : Js_exn.raiseError("No Core Stack configured! (Please set 'core:stack: user/project/stack' in you Pulumi.*.config!")
                  ...
                  var subscribeQueueToTopic = function (queueArn, topicArn) {
                    var __x = SNS$AwsSdk.snsClient(/* () */0).subscribe({
                            TopicArn: topicArn,
                            Protocol: "sqs",
                            Endpoint: queueArn,
                            Attributes: {
                    ...
                    return __x.then((function (subscriptionResponse) {
                                  return Promise.resolve((console.log("subscribed:", subscriptionResponse.SubscriptionArn), /* () */0));
                                }));
                  };
                  var unsubscribeQueueFromTopic = function (queueArn, topicArn) {
                    var __x = SNS$AwsSdk.snsClient(/* () */0).listSubscriptionsByTopic({
                            TopicArn: topicArn
                          }).promise();
                    return __x.then((function (response) {
                                  return Belt_Option.getWithDefault(Belt_Option.map(Belt_Array.getBy(response.Subscriptions, (function (subscription) {
                                                        return subscription.Endpoint === queueArn;
                                                      })), (function (subscription) {
                                                    var __x = SNS$AwsSdk.snsClient(/* () */0).unsubscribe({
                                                            SubscriptionArn: subscription.SubscriptionArn
                                                          }).promise();
                                                    return __x.then((function (param) {
                                                                  return Promise.resolve((console.log("unsubscribed:", subscription.SubscriptionArn), /* () */0));
                                                                }));
                                                  })), Promise.resolve(/* () */0));
                                }));
                  };
                  var subscribe = function (action, extensionPointName, eventTopic, pluginId, eventCollector) {
                    ...
                    var __x = subscribeQueueToTopic(eventCollector, eventTopic);
                  ...
                  var unsubscribe = function (action, extensionPointName, eventTopic, pluginId, eventCollector) {
                    ...
                    var eventCollectorName = AWS$Reventless.arn2Name(eventCollector);
                    AWS$Reventless.validateName(extensionPointName + ("-" + pluginId));
                    var __x = unsubscribeQueueFromTopic(eventCollector, eventTopic);
                    var __x$1 = __x.then((function (param) {
                            return Promise.resolve((console.log("" + (String(action) + (": " + (String(extensionPointName) + ("->" + (String(pluginId) + (" (" + (String(eventTopicName) + ("->" + (String(eventCollectorName) + ")")))))))))), /* () */0));
                          }));
                    return __x$1.catch((function (err) {
