...
module Make = (
  ...
  let errorHandler = (error, command, context: Message.context) => {
    ...
    let id = context.id
    Js.log(j`Behaviour error $errorJson in $serviceName($id): Command: $commandJson`)
    list{}
  }
 ...
 */
  ...
  let logCommand' = (
    ...
    let commandStr =
      ...
      |> Js.Json.stringify
    let idx = idx + 1
    Js.log(j`CommandTopic: handling command $idx/$count: $commandName($id) ref: $reference, complete command: $commandStr`)
  }
  ...
  let handleCommands = ((eventLogAppend, eventLogReplay), . allTopicItems) => {
    ...
    ->Belt.Array.map(((id, topicItems)) =>
      eventLogReplay(. id) |> Js.Promise.then_(history => {
          ...
          let runBehaviour = ((stateO, events)) =>
            ...
            | Some(state) =>
              ...
              Ok((
                ...
                \"@"(events, list{(generatedEvents, command'->updateMeta)}),
              ))->Js.Promise.resolve
            | None =>
        ...
        Js.log(j`finished eventLogReplay for id $id`)
        let _ =
          topicItems->Belt.Array.mapWithIndex((idx, {reference, command}) =>
        ...
        |> Js.Promise.then_(x =>
          ...
          | Ok((_, generatedEventsWithMeta)) =>
            ...
            ->Belt.List.map(((events, meta)) =>
              events->Belt.List.map(event => {Message.id: id, meta: meta, event: event})
            )
            ->Belt.List.flatten
            ->Belt.List.toArray
            ->Js.Promise.resolve
        ...
        |> Js.Promise.then_(x =>
          ...
          | [] =>
            {
              Js.log(j`Aggregate.handleCommands($id): no Event generated`)
              references->Belt.Array.map(reference => Ok(reference))
            }->Js.Promise.resolve
          | generatedEvents' =>
            let eventCount = generatedEvents'->Belt.Array.length
            Js.log2(
              j`Aggregate.handleCommands($id): $eventCount Event(s) generated:`,
            ...
            |> Js.Promise.then_(results => {
              Js.log(j`finished eventLogAppend for id $id`)
