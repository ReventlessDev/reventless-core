// Generated by BUCKLESCRIPT, PLEASE EDIT WITH CARE
'use strict';

var Block = require("bs-platform/lib/js/block.js");
var Curry = require("bs-platform/lib/js/curry.js");
var Decco = require("@ryb73/decco/src/Decco.js");
var Js_dict = require("bs-platform/lib/js/js_dict.js");
var Js_json = require("bs-platform/lib/js/js_json.js");
var Belt_Array = require("bs-platform/lib/js/belt_Array.js");
var Caml_array = require("bs-platform/lib/js/caml_array.js");
var Component = require("./Component");
var Belt_Option = require("bs-platform/lib/js/belt_Option.js");
var Caml_option = require("bs-platform/lib/js/caml_option.js");
var Id$Reventless = require("../Id.bs.js");
var Caml_exceptions = require("bs-platform/lib/js/caml_exceptions.js");
var Message$Reventless = require("../Message.bs.js");
var QueryDb$Reventless = require("./QueryDb.bs.js");
var ComponentType$Reventless = require("../ComponentType.bs.js");

var NotCounted = Caml_exceptions.create("Counter-Reventless.NotCounted");

var name = ComponentType$Reventless.toName(/* Counter */1);

function event_encode(v) {
  return /* array */["CountFinished"];
}

function event_decode(v) {
  var match = Js_json.classify(v);
  if (typeof match === "number" || match.tag !== /* JSONArray */3) {
    return Decco.error(undefined, "Not a variant", v);
  } else {
    var jsonArr = match[0];
    var tagged = jsonArr.map(Js_json.classify);
    var match$1 = Caml_array.caml_array_get(tagged, 0);
    if (typeof match$1 !== "number" && !match$1.tag && match$1[0] === "CountFinished") {
      var match$2 = tagged.length !== 1;
      if (match$2) {
        return Decco.error(undefined, "Invalid number of arguments to variant constructor", v);
      } else {
        return /* Ok */Block.__(0, [/* CountFinished */0]);
      }
    }
    return Decco.error(undefined, "Invalid variant constructor", Caml_array.caml_array_get(jsonArr, 0));
  }
}

var Source = {
  Id: 0,
  name: name,
  event_encode: event_encode,
  event_decode: event_decode
};

var Adapter = { };

function Make(Config) {
  return (function (QueryDbStorage) {
      return (function (Handler) {
          var construct = function (counterEventsHandler, ttl, self, name$1, resources) {
            var opts = {
              parent: self
            };
            var opts2 = {
              parent: self
            };
            var name$2 = name$1 + "References";
            var state_encode = function (v) {
              return Js_dict.fromArray(/* array */[/* tuple */[
                            "id",
                            Id$Reventless.$$String.t_encode(v[/* id */0])
                          ]]);
            };
            var state_decode = function (v) {
              var match = Js_json.classify(v);
              if (typeof match === "number" || match.tag !== /* JSONObject */2) {
                return Decco.error(undefined, "Not an object", v);
              } else {
                var match$1 = Js_dict.get(match[0], "id");
                var match$2 = Id$Reventless.$$String.t_decode(match$1 !== undefined ? Caml_option.valFromOption(match$1) : null);
                if (match$2.tag) {
                  var e = match$2[0];
                  return /* Error */Block.__(1, [/* record */[
                              /* path */".id" + e[/* path */0],
                              /* message */e[/* message */1],
                              /* value */e[/* value */2]
                            ]]);
                } else {
                  return /* Ok */Block.__(0, [/* record */[/* id */match$2[0]]]);
                }
              }
            };
            var ReferencesDb = QueryDb$Reventless.Make(Config)({
                        Id: Id$Reventless.$$String,
                        name: name$1
                      })({
                      name: name$2,
                      state_encode: state_encode,
                      state_decode: state_decode,
                      resolveIdConfigs: /* [] */0,
                      resolveIdsConfigs: /* [] */0,
                      sortConfig: undefined,
                      indexes: /* [] */0
                    })(QueryDbStorage)(QueryDb$Reventless.Adapter.NoResolvers(Config));
            var name$3 = name$1 + "Counts";
            var state_encode$1 = function (v) {
              return Js_dict.fromArray(/* array */[
                          /* tuple */[
                            "id",
                            Decco.stringToJson(v[/* id */0])
                          ],
                          /* tuple */[
                            "count",
                            Decco.intToJson(v[/* count */1])
                          ]
                        ]);
            };
            var state_decode$1 = function (v) {
              var match = Js_json.classify(v);
              if (typeof match === "number" || match.tag !== /* JSONObject */2) {
                return Decco.error(undefined, "Not an object", v);
              } else {
                var dict = match[0];
                var match$1 = Js_dict.get(dict, "id");
                var match$2 = Decco.stringFromJson(match$1 !== undefined ? Caml_option.valFromOption(match$1) : null);
                var match$3 = Js_dict.get(dict, "count");
                var match$4 = Decco.intFromJson(match$3 !== undefined ? Caml_option.valFromOption(match$3) : null);
                if (match$2.tag) {
                  var e = match$2[0];
                  return /* Error */Block.__(1, [/* record */[
                              /* path */".id" + e[/* path */0],
                              /* message */e[/* message */1],
                              /* value */e[/* value */2]
                            ]]);
                } else if (match$4.tag) {
                  var e$1 = match$4[0];
                  return /* Error */Block.__(1, [/* record */[
                              /* path */".count" + e$1[/* path */0],
                              /* message */e$1[/* message */1],
                              /* value */e$1[/* value */2]
                            ]]);
                } else {
                  return /* Ok */Block.__(0, [/* record */[
                              /* id */match$2[0],
                              /* count */match$4[0]
                            ]]);
                }
              }
            };
            var CountsDb = QueryDb$Reventless.Make(Config)({
                        Id: Id$Reventless.$$String,
                        name: name$1
                      })({
                      name: name$3,
                      state_encode: state_encode$1,
                      state_decode: state_decode$1,
                      resolveIdConfigs: /* [] */0,
                      resolveIdsConfigs: /* [] */0,
                      sortConfig: undefined,
                      indexes: /* [] */0
                    })(QueryDbStorage)(QueryDb$Reventless.Adapter.NoResolvers(Config));
            var separator = "#";
            var makeId = function (param) {
              return Curry._1(Id$Reventless.$$String.makeFromString, param[0] + (separator + param[1]));
            };
            var unmakeId = function (id) {
              var parts = id.split(separator);
              var len = parts.length;
              if (len !== 1) {
                if (len !== 0) {
                  return /* tuple */[
                          Caml_array.caml_array_get(parts, 0),
                          Caml_array.caml_array_get(parts, 1)
                        ];
                } else {
                  return /* tuple */[
                          "",
                          ""
                        ];
                }
              } else {
                var counterId = parts[0];
                return /* tuple */[
                        counterId,
                        ""
                      ];
              }
            };
            var groupCountItemsByCounterId = function (countItems) {
              var dict = { };
              Belt_Array.forEach(countItems, (function (param) {
                      var counterId = param[/* counterId */0];
                      var currentReferences = Belt_Option.getWithDefault(Js_dict.get(dict, counterId), /* array */[]);
                      dict[counterId] = Belt_Array.concat(currentReferences, /* array */[param[/* reference */1]]);
                      return /* () */0;
                    }));
              return Js_dict.entries(dict);
            };
            var logCountItems = function (countItems) {
              return Belt_Array.forEach(groupCountItemsByCounterId(countItems), (function (param) {
                            var references = param[1];
                            var size = references.length;
                            var referencesStr = references.join(",");
                            console.log("  " + (String(size) + (" references for counterId " + (String(param[0]) + (": " + (String(referencesStr) + ""))))));
                            return /* () */0;
                          }));
            };
            var referencesDb = Curry._4(ReferencesDb.make, ttl, Caml_option.some(opts), resources, /* () */0);
            var countsDb = Curry._4(CountsDb.make, ttl, Caml_option.some(opts), resources, /* () */0);
            var referencesName = Belt_Option.getWithDefault(name$2, name$1);
            var countsName = Belt_Option.getWithDefault(name$3, name$1);
            var groupByCounterId = function (references) {
              var dict = { };
              Belt_Array.forEach(references, (function (reference) {
                      var counterId = unmakeId(reference)[0];
                      var current = Belt_Option.getWithDefault(Js_dict.get(dict, counterId), 0);
                      dict[counterId] = current + 1 | 0;
                      return /* () */0;
                    }));
              return Js_dict.entries(dict);
            };
            var counterHandler = function (references, counts) {
              console.log("counterHandler: references:", references);
              console.log("counterHandler: counts:", counts);
              var __x = Promise.all(Belt_Array.map(groupByCounterId(references), (function (param) {
                          return Curry._1(CountsDb.count, countsDb)(Curry._1(Id$Reventless.$$String.makeFromString, param[0]), "count", -param[1] | 0);
                        })));
              var countP = __x.then((function (param) {
                      return Promise.resolve(/* () */0);
                    }));
              var counterEventsHandlerP = counterEventsHandler(Belt_Array.keepMap(counts, (function (state) {
                          var match = state_decode$1(state);
                          if (match.tag) {
                            var stateStr = JSON.stringify(state);
                            console.log("Counter-Reventless" + (".counterHandler: couldn\'t decode state " + (String(stateStr) + "")));
                            return ;
                          } else {
                            var match$1 = match[0];
                            var count = match$1[/* count */1];
                            var id = match$1[/* id */0];
                            if (count === 0) {
                              var match$2 = unmakeId(id);
                              console.log("Counter-Reventless" + (".counterHandler: finished " + (String(name$1) + ("(" + (String(id) + ")")))));
                              var meta = Message$Reventless.generateMeta(name, undefined, undefined, /* () */0);
                              return Caml_option.some(Js_dict.fromArray(/* array */[
                                              /* tuple */[
                                                "id",
                                                match$2[0]
                                              ],
                                              /* tuple */[
                                                "meta",
                                                Message$Reventless.meta_encode(meta)
                                              ],
                                              /* tuple */[
                                                "event",
                                                /* array */["CountFinished"]
                                              ]
                                            ]));
                            } else {
                              console.log("Counter-Reventless" + (".counterHandler: counted down " + (String(name$1) + ("(" + (String(id) + (") to " + (String(count) + "")))))));
                              return ;
                            }
                          }
                        })));
              var __x$1 = Promise.all(/* tuple */[
                    countP,
                    counterEventsHandlerP
                  ]);
              return __x$1.then((function (param) {
                            return Promise.resolve(/* () */0);
                          }));
            };
            var handler = Curry._6(Handler.make, name$1, referencesName, countsName, counterHandler, opts2, resources);
            var partial_arg = Curry._1(ReferencesDb.saveBatch, referencesDb);
            self.count = (function (param) {
                var saveBatch = partial_arg;
                var countItems = param;
                var __x = saveBatch(Belt_Array.map(countItems, (function (param) {
                            var id = makeId(/* tuple */[
                                  param[/* counterId */0],
                                  param[/* reference */1]
                                ]);
                            var state = /* record */[/* id */id];
                            return /* tuple */[
                                    id,
                                    state,
                                    ttl
                                  ];
                          })));
                return __x.then((function (param) {
                              if (param.tag) {
                                var match = param[0];
                                if (typeof match !== "number" && !match.tag) {
                                  var batchSize = countItems.length;
                                  console.log("Counter error: couldn\'t save batch of " + (String(batchSize) + " references:"));
                                  logCountItems(countItems);
                                  return Promise.reject([
                                              NotCounted,
                                              match[0]
                                            ]);
                                }
                                var batchSize$1 = countItems.length;
                                console.log("Unknown Counter error: couldn\'t save batch of " + (String(batchSize$1) + " references:"));
                                logCountItems(countItems);
                                return Promise.reject([
                                            NotCounted,
                                            "Unknown error"
                                          ]);
                              } else {
                                var batchSize$2 = countItems.length;
                                console.log("Counter-Reventless" + (": saved batch of " + (String(batchSize$2) + " references:")));
                                logCountItems(countItems);
                                return Promise.resolve(/* () */0);
                              }
                            }));
              });
            self.addToCounterTarget = handler[/* addToCounterTarget */0];
            var self$1 = self;
            var outputs = {
              referencesDb: Curry._1(ReferencesDb.outputs, referencesDb).storage,
              countsDb: Curry._1(CountsDb.outputs, countsDb).storage
            };
            self$1.setOutputs(outputs);
            return self$1.registerOutputs(outputs);
          };
          var make = function (name, counterEventsHandler, $staropt$star, opts, resources, param) {
            var ttl = $staropt$star !== undefined ? $staropt$star : 604800;
            var partial_arg = ttl;
            var prim = ComponentType$Reventless.toString(/* Counter */1);
            var prim$1 = ComponentType$Reventless.name(name, /* Counter */1);
            var prim$2 = function (param, param$1, param$2) {
              return construct(counterEventsHandler, partial_arg, param, param$1, param$2);
            };
            var prim$3 = opts;
            var prim$4 = resources;
            return new Component.default(prim, prim$1, prim$2, prim$3, prim$4);
          };
          return {
                  make: make,
                  count: (function (prim) {
                      return prim.count;
                    }),
                  addToCounterTarget: (function (prim) {
                      return prim.addToCounterTarget;
                    })
                };
        });
    });
}

var componentType = /* Counter */1;

exports.componentType = componentType;
exports.NotCounted = NotCounted;
exports.Source = Source;
exports.Adapter = Adapter;
exports.Make = Make;
/* name Not a pure module */
