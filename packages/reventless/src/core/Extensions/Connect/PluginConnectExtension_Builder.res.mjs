  ...
  let callHandler = async command => {
    ...
    switch (command.TAG) {
      case "DoConnectPlugin" :
        ...
        let connectToExtensionPoints = Stdlib_Array.filterMap(Message$Reventless.log(match.extensionPoints, "otherPluginExtensionPoints:"), param => {
          let extensionPointName = param.name;
          if (Message$Reventless.log(Spec.extensionsOutputs.filter(extension => extension.extensionPointName === extensionPointName), "matching Extensions:").length !== 0) {
            return subscribe("connectToExtensionPoints", extensionPointName, param.eventTopic, id, pluginDefinition.eventCollector);
          }
        });
        let connectToExtensions = Stdlib_Array.filterMap(Message$Reventless.log(Spec.extensionPointsOutputs, "extensionPoints:"), extensionPoint => {
          if (Message$Reventless.log(otherPluginExtensions.filter(param => extensionPoint.name === param.extensionPointName), "matching otherPluginExtensions:").length !== 0) {
