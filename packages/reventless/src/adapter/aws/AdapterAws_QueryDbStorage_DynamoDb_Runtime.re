...
let load = table =>
  (. id) =>
    ...
    |> catch(err => {
         ...
         Js.log(
           {j|AdapterAws_QueryDbStorage_DynamoDb_Runtime: Error: Couldn't load state for $id from $tableName: $err|j},
         );
...
let save = table =>
  (. _id, json, saveMode: QueryDb.saveMode) => {
    ...
    | Init =>
      ...
      |> then_(_ => {
           Js.log(
             {j|AdapterAws_QueryDbStorage_DynamoDb_Runtime: saved Init state to $tableName: $stateStr|j},
           );
           Ok()->resolve;
         })
      |> catch(err => {
           ...
           | "ConditionalCheckFailedException" =>
             Js.log(
               {j|AdapterAws_QueryDbStorage_DynamoDb_Runtime: Error: Stale State in $tableName|j},
             );
             Error(QueryDb.StaleState)->resolve;
           | _ =>
             Js.log(
               {j|AdapterAws_QueryDbStorage_DynamoDb_Runtime: Error: Couldn't save Init state to $tableName: $err|j},
             );
    ...
    | Overwrite =>
      ...
      |> then_(_ => {
           Js.log(
             {j|AdapterAws_QueryDbStorage_DynamoDb_Runtime: saved Overwrite state to $tableName: $stateStr|j},
           );
           Ok()->resolve;
         })
      |> catch(err => {
           Js.log(
             {j|AdapterAws_QueryDbStorage_DynamoDb_Runtime: Error: Couldn't save Overwrite state to $tableName: $err|j},
           );
           Error(QueryDb.NotSavedToStorage(err))->resolve;
         })
    };
...
let delete = table =>
  (. id, sort) => {
    ...
    |> then_(_ => {
         Js.log(
           {j|AdapterAws_QueryDbStorage_DynamoDb_Runtime: deleted state for $id from $tableName|j},
         );
         Ok()->resolve;
       })
    |> catch(err => {
         Js.log(
           {j|AdapterAws_QueryDbStorage_DynamoDb_Runtime: Error: Couldn't delete state for $id from $tableName: $err|j},
         );
