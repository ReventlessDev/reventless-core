...
function handleAction(action, param, subIdConfig) {
  ...
  switch (action.TAG | 0) {
    ...
    case /* Delete */8 :
        ...
        return param.delete(id$4, undefined);
    case /* DeleteMany */9 :
        var ids = action._0;
        console.log("Projection.handleAction:", "DeleteMany(" + ids + ")");
        ...
        return __x$2.then(function (states) {
                    if (states.TAG === /* Ok */0) {
                      if (subIdConfig !== undefined) {
                        ...
                        console.log("Projection.handleAction:", "UpdateMultiState(" + id$6 + "): beforeStates:" + beforeCount + " afterStates:" + afterCount + " added:" + addedCount + " changed:" + changedCount + " deleted:" + deletedCount);
                        var __x = Promise.all([
                              deletedCount > 0 ? deleteBatch(batchToDelete) : Promise.resolve({
                                      TAG: /* Ok */0,
                                      _0: undefined
                                    }),
                              batchCount > 0 ? saveBatch(batchToSave) : Promise.resolve({
                                      TAG: /* Ok */0,
                                      _0: undefined
                                    })
                            ]);
...
function handleActions(actions, primitives, subIdConfig) {
  return Belt_Array.map(groupActionsById(actions), (function (param) {
                ...
                if (actionCount > 1) {
                  console.log("Projection.handleActions: handling " + actionCount + " actions for id=" + id);
                }
