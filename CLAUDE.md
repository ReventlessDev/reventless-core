...
- `trait-file-attachment-set` — the same shape for an ordered attachment set with a primary and captions on a StateChangeSlice; specimen hosts are the catalog's `ProductImages` and `CategoryImages`. Named for the *set*: `FileAttachment` is reserved for the unbuilt scanning-and-retention trait every analysis uses that name for.
...
- **Tracked**: outputs under `src/`. Either a publish-surface deliverable, shared workspace state, or — for monorepo `examples/` apps — the entry point that Pulumi (`Main.res.mjs`) and `pnpm run run` (`Main.res.mjs` + transitive `Plugin.res.mjs` etc.) resolve at runtime.
- **Not tracked**:
  - **Every package's `tests/**/*.res.mjs`** — regenerated in-source by every root `pnpm run build`, so never tracked. A single root `.gitignore` rule (`**/tests/**/*.res.mjs`) ignores them all. This closes the CI dark-coverage gap: `scripts/check-jest-projects.mjs` (`pnpm run test:projects`) fails if any declared jest project discovers 0 suites. The root `build` script is a **chain of `rescript build` invocations whose order matters** — under ReScript v12's rewatch, each build-root's clean step orphans the in-source `type: dev` test outputs of packages *in its dependency graph but outside the current root*, so the chain re-emits them last: (1) `rescript build` (framework + `local`/`aws`/`gwt` tests); (2) the three `examples/*/platform-local` builds (example plugins); (3) the three `examples/*/platform-aws` builds (regenerate the tracked deploy `Main.res.mjs` — a warm root build otherwise drops them; these must precede step 4 because they orphan `spec`); (4) `reventless/core` → `reventless/interop` → `reventless/spec`, **in that dependency order** (`core` depends on `interop`+`spec`, so building `core` first then the leaves last leaves all three emitted); (5) `reventless/seed-aws` → `reventless/seed`, again in dependency order, and last because the `platform-aws` builds in step 3 carry both packages in their graph and orphan their test outputs. Reordering or dropping a step silently drops a package's coverage or deletes a tracked entry point.
  - Outputs under `rescript/*/src/example/` and `rescript/*/src/examples/` — inline usage demos inside ReScript binding packages (e.g. `rescript-aws-sdk/src/example`, `rescript-uuid/src/examples`). Not part of any publish surface or deploy path.

Do **not** add per-package `.gitignore` rules for `tests/` outputs — the root rule covers every package.
