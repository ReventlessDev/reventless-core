...
- **Not tracked**:
  - **Every package's `tests/**/*.res.mjs`** — regenerated in-source by every root `pnpm run build`, so never tracked. The root `build` script emits all of them: the leading rewatch pass covers `core`/`spec`/`interop`/`local`/`aws`/`gwt` (incl. their `type: dev` tests); the `examples/*/platform-local` sub-builds cover the example plugins; a **trailing** bare `rescript build` re-emits the root-graph test outputs that the sub-builds orphan into `lib/bs`. A single root `.gitignore` rule (`**/tests/**/*.res.mjs`) ignores them all. This closes the CI dark-coverage gap: `scripts/check-jest-projects.mjs` (`pnpm run test:projects`) fails if any declared jest project discovers 0 suites. (The sub-builds also delete two tracked `examples/{dcb,hybrid}/platform-aws/src/Main.res.mjs` on every build — restore with `git checkout HEAD -- <path>`, do not rebuild.)
