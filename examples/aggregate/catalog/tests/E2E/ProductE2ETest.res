...
describe("Product E2E:", () => {
  ...
  testPromise("AddProduct command publishes 1 event to the event topic", async () => {
    let ops = await agg->ProductAgg.operations->ReventlessInMemory.TestRunner.resolve
    let commandJson = Product.AddProduct({
      productId: "prod-1",
      name: "Laptop",
  ...
  testPromise(
    "duplicate AddProduct for same id produces no events (ProductAlreadyExists error path)",
    async () => {
      let ops = await agg->ProductAgg.operations->ReventlessInMemory.TestRunner.resolve
      let commandJson = Product.AddProduct({
        productId: "prod-1",
