...
let _updateEmailSlice =
  ChangeEmailMaker.make(~dcbEventLog=eventLog, ~publishJsons=publishJsonsOutput)
let _updateAddressSlice =
  ChangeAddressMaker.make(~dcbEventLog=eventLog, ~publishJsons=publishJsonsOutput)
...
describe("Ordering DCB E2E:", () => {
  ...
  testPromise("ChangeEmail on existing customer publishes 1 event", async () => {
    let cmd =
      ChangeEmail.ChangeEmail({customerId: "cust-1", email: "alice2@example.com"})
      ->Message.encode(ChangeEmail.commandSchema)
    await dispatch(cmd, "cust-1")
    expect(capturedEventCount.contents)->toBe(1)
  })
