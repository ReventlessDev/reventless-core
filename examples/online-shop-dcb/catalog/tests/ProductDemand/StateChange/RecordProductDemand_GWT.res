...
describe("RecordProductDemand StateChangeSlice", () => {
  test("first RecordDemand produces ProductDemandRecorded", () =>
    givenEvents([])
    ->whenCmd(RecordDemand({productId: pid("p1"), orderId: "order-1"}))
    ->thenEvent(ProductDemandRecorded({productId: pid("p1"), orderId: "order-1"}))
  )
  ...
  test("RecordDemand for new orderId on existing product produces ProductDemandRecorded", () =>
    ...
    ->whenCmd(RecordDemand({productId: pid("p1"), orderId: "order-2"}))
    ->thenEvent(ProductDemandRecorded({productId: pid("p1"), orderId: "order-2"}))
  )
  ...
  test("RevokeDemand on never-recorded product produces no events (idempotent)", () =>
    givenEvents([])
    ->whenCmd(RevokeDemand({productId: pid("p1"), orderId: "order-1"}))
    ->thenNoEvent
  )
  ...
  test(
    ...
    () =>
      givenEvents([ProductDemandRecorded({orderId: "order-1"})])
      ->whenCmd(RevokeDemand({productId: pid("p1"), orderId: "order-2"}))
      ->thenNoEvent,
  )
  ...
  test("RevokeDemand for recorded orderId produces ProductDemandRevoked", () =>
    givenEvents([ProductDemandRecorded({orderId: "order-1"})])
    ->whenCmd(RevokeDemand({productId: pid("p1"), orderId: "order-1"}))
    ->thenEvent(ProductDemandRevoked({productId: pid("p1"), orderId: "order-1"}))
  )
})
