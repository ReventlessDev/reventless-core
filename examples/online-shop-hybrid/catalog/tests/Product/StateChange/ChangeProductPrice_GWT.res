...
describe("ChangeProductPrice StateChangeSlice", () => {
  test("non-existent product returns ProductNotFound", () =>
    givenEvents([])
    ->whenCmd(ChangeProductPrice({productId: pid("p1"), price: eur(1.0)}))
    ->thenError(ProductNotFound)
  )

  test("existing product produces ProductPriceChanged", () =>
    givenEvents([ProductAdded({price: eur(999.99)})])
    ->whenCmd(ChangeProductPrice({productId: pid("p1"), price: eur(899.99)}))
    ->thenEvent(ProductPriceChanged({productId: pid("p1"), price: eur(899.99)}))
  )

  test("same price produces no events (idempotent)", () =>
    givenEvents([ProductAdded({price: eur(999.99)})])
    ->whenCmd(ChangeProductPrice({productId: pid("p1"), price: eur(999.99)}))
    ->thenNoEvent
  )
