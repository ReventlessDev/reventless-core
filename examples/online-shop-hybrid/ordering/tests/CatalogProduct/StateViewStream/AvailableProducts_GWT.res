...
describe("AvailableProducts StateViewSliceStream", () => {
  test("CatalogProductSynced creates a row", () =>
    givenEvents([])
    ->whenEvent(CatalogProductSynced({productId: pid("p1"), name: "Laptop", price: eur(999.99)}))
    ->thenStateWithId("p1", {productId: pid("p1"), name: "Laptop", price: eur(999.99)})
  )

  test("CatalogProductPriceChanged updates the price", () =>
    givenEvents([CatalogProductSynced({productId: pid("p1"), name: "Laptop", price: eur(999.99)})])
    ->whenEvent(CatalogProductPriceChanged({productId: pid("p1"), price: eur(899.99)}))
    ->thenStateWithId("p1", {productId: pid("p1"), name: "Laptop", price: eur(899.99)})
  )
  ...
  test("CatalogProductWithdrawn removes the row", () =>
    givenEvents([CatalogProductSynced({productId: pid("p1"), name: "Laptop", price: eur(999.99)})])
    ->whenEvent(CatalogProductWithdrawn({productId: pid("p1")}))
    ->thenNoState
  )

  test("CatalogProductRelisted puts it back with its name and price", () =>
    givenEvents([
      CatalogProductSynced({productId: pid("p1"), name: "Laptop", price: eur(999.99)}),
      CatalogProductWithdrawn({productId: pid("p1")}),
    ])
    ->whenEvent(CatalogProductRelisted({productId: pid("p1"), name: "Laptop", price: eur(899.99)}))
    ->thenStateWithId("p1", {productId: pid("p1"), name: "Laptop", price: eur(899.99)})
  )
})
