...
describe("Category E2E:", () => {
  ...
  testPromise("Add command publishes 1 event to the event topic", async () => {
    ...
    let commandJson =
      Category.Add({name: "Electronics"})->Message.encode(
  ...
  testPromise("Rename on existing category publishes 1 event", async () => {
    let ops = await agg->CategoryAgg.operations->ReventlessInMemory.TestRunner.resolve
    let commandJson =
      Category.Rename({
  ...
  testPromise(
    "duplicate Add for same id produces no events (CategoryAlreadyExists error path)",
    async () => {
      let ops = await agg->CategoryAgg.operations->ReventlessInMemory.TestRunner.resolve
      let commandJson =
        Category.Add({name: "Duplicate"})->Message.encode(
          Category.commandSchema,
        )
      await ops.publishJsons([{Message.id: "cat-1", meta: testMeta, commandJson}])
