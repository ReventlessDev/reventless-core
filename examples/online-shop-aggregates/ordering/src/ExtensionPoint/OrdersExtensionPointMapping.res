...
let mapOutgoingEvent = Some((_id, event, _meta, _queryEngine) =>
  ...
  | Order.Placed({customerId, productIds}) =>
    productIds->Array.map(productId =>
      PublishEvent(
        productId,
        OrderingSpec.OrdersExtensionPoint.ItemOrdered({productId, orderId: _id, customerId}),
      )
    )
  | Order.Cancelled({productIds}) =>
    productIds->Array.map(productId =>
      PublishEvent(
        productId,
        OrderingSpec.OrdersExtensionPoint.ItemOrderCancelled({productId, orderId: _id}),
      )
    )
  | _ => []
  }
