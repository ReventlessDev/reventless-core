...
describe("Order E2E:", () => {
  ...
  testPromise("Place command publishes 1 event to the event topic", async () => {
  ...
  testPromise("Ship on placed order publishes 1 event", async () => {
    let ops = await agg->OrderAgg.operations->ReventlessInMemory.TestRunner.resolve
    let commandJson =
      Order.Ship->Message.encode(Order.commandSchema)
    await ops.publishJsons([{Message.id: "ord-1", meta: testMeta, commandJson}])
    expect(capturedEventCount.contents)->toBe(1)
  })

  testPromise(
    "duplicate Place for same id produces no events (OrderAlreadyPlaced error path)",
    async () => {
      let ops = await agg->OrderAgg.operations->ReventlessInMemory.TestRunner.resolve
      let commandJson =
        Order.Place({
