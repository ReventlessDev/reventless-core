...
Background
----------
The online-shop-aggregates stack is split into three Pulumi projects:
  examples/online-shop-aggregates/platform-aws/   (admin, scheduler, shared API)
  examples/online-shop-aggregates/catalog-aws/    (Catalog plugin infra)
  examples/online-shop-aggregates/ordering-aws/   (Ordering plugin infra)

Lambda replacements driven by IAM-role changes (and other Pulumi-side
identity flips) periodically leave orphan resources behind. This script
identifies them and emits delete commands.

Usage
-----
  # 1. Export each stack's state
  cd examples/online-shop-aggregates/platform-aws && pulumi stack export --file /tmp/state-platform.json
  cd examples/online-shop-aggregates/catalog-aws  && pulumi stack export --file /tmp/state-catalog.json
  cd examples/online-shop-aggregates/ordering-aws && pulumi stack export --file /tmp/state-ordering.json
