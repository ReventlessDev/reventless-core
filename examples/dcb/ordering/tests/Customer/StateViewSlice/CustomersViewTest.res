...
describe("CustomersView.project:", () => {
  ...
  test("EmailUpdated Update function changes email", () =>
    expect(
      CustomersView.project(
        None,
        OrderingEventLog.EmailUpdated({customerId: "cust-1", email: "alice2@example.com"}),
      )->applyFirstUpdate(baseCustomer),
    )->toEqual({...baseCustomer, email: "alice2@example.com"})
  )

  test("AddressUpdated Update function changes address", () =>
    expect(
      CustomersView.project(
        None,
        OrderingEventLog.AddressUpdated({customerId: "cust-1", address: "789 Pine Rd"}),
      )->applyFirstUpdate(baseCustomer),
