...
describe("QueryDbStorage_InMemory", () => {
  describe("save and load", () => {
    test("save stores state; loadStream retrieves it by id", async () => {
      ...
      let items = await Effect.Effect.runPromise(Effect.Effect.catchAll(Stream.runCollect(ops.loadStream("id1")), param => Effect.Effect.succeed([])));
    ...
    test("loadStream returns empty for unknown id", async () => {
      ...
      let ops = await TestRunner$ReventlessInMemory.resolve(s.operations);
      let items = await Effect.Effect.runPromise(Effect.Effect.catchAll(Stream.runCollect(ops.loadStream("unknown")), param => Effect.Effect.succeed([])));
    ...
    test("load delegates to loadStream (backward-compat)", async () => {
      ...
      let streamed = await Effect.Effect.runPromise(Stream.runCollect(ops.loadStream("compat-id")));
  ...
  describe("saveBatch", () => {
    test("stores multiple items; loadStream retrieves each by id", async () => {
      ...
      let itemsA = await Effect.Effect.runPromise(Effect.Effect.catchAll(Stream.runCollect(ops.loadStream("a")), param => Effect.Effect.succeed([])));
      let itemsB = await Effect.Effect.runPromise(Effect.Effect.catchAll(Stream.runCollect(ops.loadStream("b")), param => Effect.Effect.succeed([])));
  ...
  describe("delete", () => {
    test("removes item; loadStream returns empty", async () => {
      ...
      let items = await Effect.Effect.runPromise(Effect.Effect.catchAll(Stream.runCollect(ops.loadStream("del-id")), param => Effect.Effect.succeed([])));
      expect(items.length).toBe(0);
    });
    test("deleteBatch removes multiple items", async () => {
      ...
      let rx = await Effect.Effect.runPromise(Effect.Effect.catchAll(Stream.runCollect(ops.loadStream("x")), param => Effect.Effect.succeed([])));
      let ry = await Effect.Effect.runPromise(Effect.Effect.catchAll(Stream.runCollect(ops.loadStream("y")), param => Effect.Effect.succeed([])));
