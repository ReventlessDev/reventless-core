...
import * as CommandGeneratorResolvers_GraphQL$ReventlessInMemory from "../../src/adapter/CommandGenerator/CommandGeneratorResolvers_GraphQL.res.mjs";
...
function buildFixture(namespace) {
  ...
  CommandGeneratorResolvers_GraphQL$ReventlessInMemory.register([
    addField,
    renameField,
    archiveField
  ], commandSchema, commandAuthorization, DomainGraphQL_Server$ReventlessInMemory.asInterface);
  ...
  [
    ...
    renameField,
    archiveField
  ].forEach(field => CommandGeneratorResolvers_GraphQL$ReventlessInMemory.bindHandler(field, stubGenerateCommand));
  let resolverFor = field => {
    let r = DomainGraphQL_Server$ReventlessInMemory.asInterface.getMutationResolver(field);
    if (r !== undefined) {
      return r;
    } else {
...
describe("CommandGeneratorResolvers_GraphQL — per-constructor authorization", () => {
  beforeEach(() => DomainGraphQL_Server$ReventlessInMemory.asInterface.reset());
  ...
  test("payload-less Archive registered as a GraphQL mutation field (no longer filtered)", async () => {
    buildFixture("Cat7");
    let sdl = DomainGraphQL_Server$ReventlessInMemory.asInterface.buildSdl();
