...
beforeAllAsync(async () => {
  ...
  Auth_InMemory.Login.setTokenSecret("http-test-secret")
  Auth_InMemory.Login.setCredentials(
    ~username="alice",
    ~password="alice-pw",
    ~identity={
      userId: "u-alice",
...
testPromise("POST /__inmemory/login returns {token, identity} for valid credentials", async () => {
  ...
  // The echoed identity matches what Auth_InMemory.lookupUser returns.
...
testPromise("Bearer token issued via HTTP round-trips through authenticate", async () => {
  ...
  let token = getString(body, "token")
  let result =
    await Auth_InMemory.authenticate({
