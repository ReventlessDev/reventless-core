...
let PlaceMaker = StateChangeSlice_Builder$ReventlessInMemory.Make({
  name: name,
  moduleUrl: moduleUrl,
  Id: Id$Reventless.$$String,
  consumedEventSchema: consumedEventSchema,
...
let ShipMaker = StateChangeSlice_Builder$ReventlessInMemory.Make({
  name: name$1,
  moduleUrl: moduleUrl$2,
  Id: Id$Reventless.$$String,
  consumedEventSchema: consumedEventSchema$1,
...
async function readEventTypes(orderId) {
  let logOps = await TestRunner$ReventlessInMemory.resolve(DcbLogMaker.operations(dcbEventLog));
