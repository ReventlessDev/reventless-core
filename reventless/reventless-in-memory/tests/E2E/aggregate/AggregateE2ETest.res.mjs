...
describe("Aggregate E2E", () => {
  beforeAll(async () => {
    await TestRunner$ReventlessInMemory.resolve(AggregateE2EFixtures$ReventlessInMemory.ItemAgg.operations(AggregateE2EFixtures$ReventlessInMemory.agg));
  });
  beforeEach(() => {
  ...
  test("CreateItem command produces ItemCreated event on event topic", async () => {
    ...
    expect(AggregateE2EFixtures$ReventlessInMemory.capturedEventCount.contents).toBe(1);
  });
  test("second CreateItem for same id produces no events (AlreadyExists)", async () => {
    ...
    expect(AggregateE2EFixtures$ReventlessInMemory.capturedEventCount.contents).toBe(0);
  });
