  ...
  return Behavior => {
    ...
    let exec = (history, context, command) => {
      ...
      try {
        ...
        if (exn.RE_EXN_ID === Message$ReventlessSpec.InvalidEvent) {
          return [];
        }
        throw exn;
      }
    ...
    let listErrors = () => "Errors occured: " + Stdlib_Array.reduce(errors.contents.map(err => Stdlib_Option.getOrThrow(Stdlib_JSON.Decode.string(Stdlib_Option.getOrThrow(Stdlib_JSON.Decode.array(Message$ReventlessSpec.encode(err, Spec.errorSchema)), undefined)[0]), undefined)), "", (a, b) => a + (b + " "));
