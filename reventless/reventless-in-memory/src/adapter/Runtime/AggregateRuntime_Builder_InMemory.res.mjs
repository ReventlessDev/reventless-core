...
function Make(Bus) {
  return CommandTopicChannel => (EventCollectorChannel => {
    let forCommandGenerator = (handler, connect, param, param$1, commandGenerator) => {
      ...
      return connect(RuntimeEnvironment_InMemory$ReventlessInMemory.make(ComponentType$ReventlessCore.nameOpt(resource.__name, CommandGenerator$ReventlessCore.componentType), handler.apply(Runtime$ReventlessCore.runEffectHandler), undefined, undefined, undefined));
    };
    let forCommandTopic = (handler, connect, param, param$1, commandTopic) => {
      ...
      return connect(RuntimeEnvironment_InMemory$ReventlessInMemory.make(ComponentType$ReventlessCore.nameOpt(resource.__name, CommandTopic$ReventlessCore.componentType), handler.apply(Runtime$ReventlessCore.runEffectHandler), undefined, undefined, undefined));
    };
