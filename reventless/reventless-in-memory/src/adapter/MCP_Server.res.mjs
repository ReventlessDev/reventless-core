...
import * as McpSdk from "@reventlessdev/rescript-mcp-sdk/src/McpSdk.res.mjs";
...
function registerToolsFromEntries(pluginName, mutationEntries, commandHandler) {
  ...
  toolDefs.forEach(def => {
    let handler = async args => {
      try {
        return McpSdk.toolResult(await commandHandler(def.name, args));
      } catch (raw_exn) {
        let exn = Primitive_exceptions.internalToException(raw_exn);
        return McpSdk.toolError(Stdlib_Option.getOr(Stdlib_Option.flatMap(Stdlib_JsExn.fromException(exn), Stdlib_JsExn.message), "Command failed"));
      }
    };
...
function createServerInstance() {
  ...
  McpSdk.onListTools(server, async () => {
  ...
  McpSdk.onCallTool(server, async req => {
    ...
    if (match !== undefined) {
      ...
      return McpSdk.toolError(`Unknown tool: ` + toolName);
    }
  });
  McpSdk.onListResources(server, async () => {
