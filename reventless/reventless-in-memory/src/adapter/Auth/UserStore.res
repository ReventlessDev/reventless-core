// In-memory user store loader. Hydrates `Auth_InMemory.Login.store` from
...
// file exists, the store stays empty and `Auth_InMemory.Login.issue` rejects
...
let parseString = (yamlText: string): result<array<entry>, string> =>
  try {
    ...
    Ok(S.parseOrThrow(~to=json, _entriesSchema))
  } catch {
...
let _registerEntries = (entries: array<entry>): unit =>
  entries->Array.forEach(({username, password, groups, ?userId}) => {
    ...
    Auth_InMemory.Login.setCredentials(~username, ~password, ~identity=id)
  })
  ...
  | None =>
    ...
    | None =>
      ...
      if _existsSync(defaultPath) {
        ...
        Console.log(
          "[Auth_InMemory] no users configured — POST /__inmemory/login will reject all. " ++
