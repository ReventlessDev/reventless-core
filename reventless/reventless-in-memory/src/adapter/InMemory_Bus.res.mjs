...
function Impl(C) {
  ...
  let dispatchCommand = async (channelName, json) => {
    ...
    if (handler !== undefined) {
      ...
      Effect.Effect.runSync(C.logger.warn("InMemory_Bus: no command handler for channel: " + channelName));
      return;
    }
  };
...
function Make($star) {
  ...
  let dispatchCommand = async (channelName, json) => {
    ...
    if (handler !== undefined) {
      ...
      Effect.Effect.runSync(Logger.consoleLogger.warn("InMemory_Bus: no command handler for channel: " + channelName));
      return;
    }
  };
  let registerCommandHandler = (channelName, handler) => {
