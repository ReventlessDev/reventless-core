...
function finish() {
  ...
  if (specs.length !== 0) {
    specs.forEach(spec => {
      ...
      if (info === undefined) {
        return log.warn("AggregateRuntime_Builder_Micro", undefined, `no handler registered for ` + spec.aggregateName);
      }
      ...
      if (match$3 === undefined) {
        return log.warn("AggregateRuntime_Builder_Micro", undefined, `eventCollector registered for ` + spec.aggregateName + ` but no mappingsModulePath — skipping EventMapper Lambda`);
      }
