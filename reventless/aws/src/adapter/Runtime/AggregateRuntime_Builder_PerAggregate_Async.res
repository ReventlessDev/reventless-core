// Async variant of AggregateRuntime_Builder_PerAggregate.
...
> = (~handler as _, ~connect, ~memorySize=1024, ~timeout=30, commandTopic) => {
  ...
  | None =>
    ...
    JsError.throwWithMessage(
      `forCommandTopic(per-aggregate-async): commandTopic ${name} has no Aggregate parent`,
    )
  }
}
...
> = (~handler as _, ~eventTopics, ~resources, ~memorySize=1024, ~timeout=30, eventCollector) => {
  ...
  | None =>
    ...
    JsError.throwWithMessage(
      `forEventCollector(per-aggregate-async): eventCollector ${name} has no Aggregate parent`,
    )
  }
}
    ...
    if specs->Array.length > 0 {
      specs->Array.forEach(spec => {
        ...
        | Some(info) =>
          ...
          let channelSpecs = spec.eventCollectorChannelSpec->Option.mapOr([], cs => [cs])
        ...
        | None =>
          log.warn(
            ~comp="AggregateRuntime_Builder_PerAggregate_Async",
