// Generated by ReScript, PLEASE EDIT WITH CARE

import * as Util_SQS$ReventlessAws from "../../util/Util_SQS.res.mjs";
import * as Util_SQS_Runtime$ReventlessAws from "../../util/Util_SQS_Runtime.res.mjs";
import * as CommandTopicChannel_SQS_Runtime$ReventlessAws from "./CommandTopicChannel_SQS_Runtime.res.mjs";

function make(resources) {
  return {
    resources: resources,
    remotePublish: CommandTopicChannel_SQS_Runtime$ReventlessAws.publishJsons(Util_SQS_Runtime$ReventlessAws.toResolvedQueue(Util_SQS$ReventlessAws.findResolvedResource(resources)), "SQS")
  };
}

export {
  make,
}
/* Util_SQS-ReventlessAws Not a pure module */
