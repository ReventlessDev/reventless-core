...
import * as AggregateRuntime_Builder_Micro$ReventlessAws from "../adapter/Runtime/AggregateRuntime_Builder_Micro.res.mjs";
...
function Make(Spec) {
  return Behavior => (EventMappings => (MappingsConfig => {
    let Inner = Aggregate_Builder$ReventlessCore.Make(Spec)(Behavior)({
      ...
      forCommandGenerator: AggregateRuntime_Builder_Micro$ReventlessAws.forCommandGenerator,
      forCommandTopic: AggregateRuntime_Builder_Micro$ReventlessAws.forCommandTopic,
    ...
    let make = (api, runtime, opts) => {
      ...
      AggregateRuntime_Builder_Micro$ReventlessAws.registerAggregate(Spec.name, Util_Bundle$ReventlessAws.getModuleSpecifier(Spec.moduleUrl), Util_Bundle$ReventlessAws.getModuleSpecifier(Behavior.moduleUrl), eventLogTableName, MappingsConfig.mappingsModulePath);
      return aggregate;
    };
    let finish = () => AggregateRuntime_Builder_Micro$ReventlessAws.finish();
    return {
      Spec: Spec,
      make: make,
