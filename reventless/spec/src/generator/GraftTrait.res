...
/** The trait, where its graft goes, and every other `--key value` as a field of
    the trait's config — a string, or `true` for a key given no value. */
type args = {
  traitPackage: string,
  into: string,
  tests: string,
...
let parseArgs = (argv: array<string>): result<args, string> =>
  CliArgs.parse(~strings=["into", "tests"], ~bools=["dry-run"], ~pairs=true, argv)
  ->Result.flatMap(a => a->CliArgs.atMost(1))
  ->Result.flatMap(a =>
    switch (
      a->CliArgs.positionals->Array.get(0),
      a->CliArgs.string("into"),
      a->CliArgs.string("tests"),
    ) {
    | (None | Some(""), _, _) => Error("<trait-package> is required.")
    | (Some(traitPackage), Some(into), Some(tests)) =>
      // A key given no value reads as `true`, so the config schema can take it
      // as a boolean without the tool knowing which keys are flags.
      ...
      ->Array.forEach(((key, value)) =>
        fields->Dict.set(
          key,
          switch value {
          | Text(text) => JSON.Encode.string(text)
          | Present => JSON.Encode.bool(true)
          },
        )
      )
      Ok({traitPackage, into, tests, dryRun: a->CliArgs.bool("dry-run"), fields})
    | (Some(_), _, _) => Error("--into and --tests are both required.")
    }
  )

let cli: CliArgs.cli<args> = {bin: "graft-trait", usage, parse: parseArgs}

let main = () =>
  CliArgs.run(cli, async ({traitPackage, into, tests, dryRun, fields}) => {
...
// No top-level call: `../../run-graft-trait.mjs` invokes [main], so a test can
// import this module.
