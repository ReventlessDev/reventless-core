...
describe("Uploadable:", () => {
  ...
  describe("UploadableImage:", () => {
    ...
    testSync(
      "declares its store through the shared StoredIn payload",
      () =>
    ...
    testSync(
      "carries a qualified store's plugin",
      () =>
        expect(
    ...
    testSync(
      "validates as a storage ref does",
      () =>
        expect(
          [
  ...
  describe("ImageRef / FileRef:", () => {
    ...
    testSync(
      "accept a web address and an origin-relative path",
      () =>
        expect(
          [
    ...
    testSync(
      "refuse a data: URI, a protocol-relative host and the empty string",
      () =>
        expect(
          ["data:image/png;base64,AAAA", "//evil.example/x.png", "", "photo.jpg"]->Array.map(
            raw =>
