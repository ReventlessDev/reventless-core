...
describe("OwnerScope:", () => {
  ...
  describe("an ordinary authenticated caller owns their rows:", () => {
    ...
    testSync(
      "they are classified as an owner",
      () => expect(scope)->toEqual(OwnerScope.Owned({userId: "u-1"})),
    )
    testSync(
      "their id is what gets stamped and matched",
  ...
  describe("a caller in an elevated group is exempt:", () => {
    ...
    testSync(
      "they are classified as elevated, not as an owner",
    ...
    testSync("they contribute no owner id", () => expect(OwnerScope.ownerId(scope))->toEqual(None))
  ...
  describe("the IAM caller — the shape that has no userId at all:", () => {
    ...
    testSync(
      "it is System, not a non-elevated owner",
  ...
  describe("callers that cannot be identified are refused, not guessed:", () => {
    ...
    testSync(
      "an anonymous identity is unidentified",
      () =>
        expect(
          Identity.anonymous->OwnerScope.resolve(~elevated=["Admin"])->reasonOf->Option.isSome,
        )->toBe(true),
    )

    testSync(
      "a Cognito identity missing its userId is unidentified",
      () =>
        expect(
    ...
    testSync(
      "an empty userId is unidentified",
      () =>
        expect(
          cognito(~userId="", ~groups=[])
          ->OwnerScope.resolve(~elevated=[])
          ->reasonOf
          ->Option.isSome,
        )->toBe(true),
    ...
    testSync(
      "an unrecognised provider is unidentified, NOT System",
      () => {
        let scope = rawIdentity({
          "userId": "u-9",
          "username": "u-9",
          "groups": [],
          "provider": "Kerberos",
    ...
    testSync(
      "the reason names the offending provider",
      () =>
        expect(
    ...
    testSync(
      "an unidentified caller contributes no owner id",
      () =>
        expect(OwnerScope.ownerId(Identity.anonymous->OwnerScope.resolve(~elevated=[])))->toBe(
          None,
        ),
    )
    ...
    testSync(
      "a wholly absent identity is unidentified, not a crash",
      () => {
  ...
  describe("a Custom provider is a person, not a machine:", () => {
    ...
    testSync(
      "it resolves by group like any other user",
  ...
  describe("the elevated list resolves from code, then the environment:", () => {
    ...
    testSync(
      "nothing set anywhere resolves to empty",
      () => expect(withEnv(None, () => OwnerScope.elevatedGroups()))->toEqual([]),
    )

    testSync(
      "the environment is read when no code set a list",
      () => expect(withEnv(Some("Admin"), () => OwnerScope.elevatedGroups()))->toEqual(["Admin"]),
    )

    testSync(
      "a comma-separated list is split and trimmed",
      () =>
    ...
    testSync(
      "empty entries are dropped rather than kept as blank groups",
      () =>
        expect(withEnv(Some("Admin,,  ,"), () => OwnerScope.elevatedGroups()))->toEqual(["Admin"]),
    )

    testSync(
      "an explicit call wins over the environment",
      () =>
        expect(
          withEnv(
            Some("Admin"),
    ...
    testSync(
      "an explicit empty list wins over the environment",
      () =>
        expect(
          withEnv(
            Some("Admin"),
    ...
    testSync(
      "classification honours an environment-only configuration",
      () =>
        expect(
          withEnv(
            Some("Admin"),
  ...
  describe("retirement narrowing:", () => {
    ...
    testSync(
      "a view that declares no retirement flag narrows nothing",
      () =>
        expect(
    ...
    testSync(
      "an ordinary caller never sees retired rows",
    ...
    testSync(
      "an elevated caller is excluded too until they ask",
      () =>
        expect(retiredOf(cognito(~userId="ops-1", ~groups=["Admin"])))->toEqual(Some("archived")),
    )

    testSync(
      "an elevated caller who asks gets them",
      () =>
    ...
    testSync(
      "an ordinary caller asking is ignored, not obeyed",
      () =>
    ...
    testSync(
      "an unidentified caller is excluded rather than refused",
  ...
  describe("the configured default is used when no list is passed:", () => {
    testSync(
      "setElevatedGroups changes how an unqualified resolve classifies",
      () => {
