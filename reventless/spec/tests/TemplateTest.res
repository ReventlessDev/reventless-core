...
describe("Template:", () => {
  describe("parsing refuses what it cannot render:", () => {
    ...
    testSync(
      "a block left open is named",
      () =>
        expect(
          parseError("{{# if expedited }}fast"),
        )->toBe(`"{{# if expedited }}" was never closed`),
    )

    testSync(
      "a close that does not match what is open",
      () =>
        expect(
          parseError("{{# if expedited }}fast{{/ each }}"),
        )->toBe(`"{{/ each }}" closes a "if"`),
    )
    ...
    testSync(
      "an each inside an each",
      () =>
        expect(
          parseError("{{# each items }}{{# each items }}x{{/ each }}{{/ each }}"),
        )->toBe(`an "each" inside an "each" — iteration is one level deep`),
    )

    testSync(
      "an item-relative path where there is no item",
      () =>
        expect(
          parseError("{{ .name }}"),
        )->toBe(`".name" is item-relative, and there is no item outside an "each"`),
    )

    testSync(
      "a formatter outside the vocabulary names the vocabulary",
      () =>
        expect(
          parseError("{{ total | dollars }}"),
        )->toBe(`"dollars" is not a formatter — one of raw, money, percent, bytes, duration, dateRange, geoPoint`),
    )
  ...
  describe("a value is formatted by its own semantic, with nothing in the template:", () => {
    ...
    testSync(
      "a field with no semantic is its plain value",
      () => expect(render("Order {{ orderId }}."))->toBe("Order o-1."),
    )

    testSync(
      "`| raw` renders what the payload holds",
    ...
    testSync(
      "an override that does not apply falls back to the value",
  ...
  describe("a sensitive value never reaches the output:", () => {
    testSync(
      "an annotated field is withheld",
    ...
    testSync(
      "an optional annotated field is withheld",
      () => expect(render("{{ resetToken }}"))->toBe("[withheld: resetToken]"),
    )

    testSync(
      "an email is withheld with no annotation at all",
    ...
    // drop the body.
    testSync(
      "a guard on a sensitive field still renders its body",
      () =>
    ...
    testSync(
      "one arm of an event union answers for its own fields",
      () =>
        expect(
  ...
  describe("an unresolved path is visible, never an exception:", () => {
    ...
    testSync(
      "a path through a field that is not an object",
      () => expect(render("{{ orderId.deeper }}"))->toBe("[missing: orderId.deeper]"),
    )

    testSync(
      "an each over something that is not a list",
  ...
  describe("guards and iteration:", () => {
    testSync(
      "a guard renders its body when the value is there",
      () => expect(render("{{# if expedited }}Express.{{/ if }}"))->toBe("Express."),
    )

    testSync(
      "an empty string is not a reason to render",
      () => expect(render("{{# if note }}{{ note }}{{/ if }}done"))->toBe("done"),
    )

    testSync(
      "an absent field is not a reason to render",
      () => expect(render("{{# if trackingNumber }}shipped{{/ if }}ok"))->toBe("ok"),
    )

    testSync(
      "each item is rendered with item-relative paths",
      () =>
    ...
    testSync(
      "the root is still reachable from inside an each",
      () =>
    ...
    testSync(
      "an empty list renders nothing rather than a placeholder",
      () =>
        expect(
          Template.renderSource(
    ...
    testSync(
      "iteration stops at the bound and says what it left",
      () => {
            ...
            (
              ...
              JSON.Encode.array(
                Array.fromInitializer(
                  ~length=Template.maxItems + 3,
                  _ => JSON.Encode.object(Dict.fromArray([("name", JSON.Encode.string("x"))])),
                ),
              ),
            ),
          ]),
