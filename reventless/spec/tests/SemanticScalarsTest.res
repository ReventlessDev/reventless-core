...
describe("semantic scalars:", () => {
  ...
  describe("every scalar marks itself:", () => {
    ...
    testSync(
      "and none of them is DCB-tagged or a reference",
      () =>
        expect(
          [
  ...
  describe("an optional field keeps its semantic:", () => {
    ...
    testSync(
      "undefined-wrapped (the `field?: t` shape)",
    ...
    testSync(
      "an optional storage ref still declares its store",
      () =>
        expect(
    ...
    testSync(
      "an optional reference still names its target",
      () =>
        expect(
    ...
    testSync(
      "a multi-variant union carries no semantic of its own",
  ...
  describe("Email:", () => {
    ...
    testSync(
      "says what it wanted and shows the value",
      () =>
        expect(
  ...
  describe("Url:", () => {
    ...
    testSync(
      "accepts http with a query",
      () => expect(accepts("http://example.com/a?b=1"))->toBe(true),
    )
    testSync(
      "accepts an uppercase scheme",
      () => expect(accepts("HTTPS://example.com"))->toBe(true),
    )
  ...
  describe("Percent:", () => {
    ...
    testSync(
      "rejects infinity",
      () => expect(accepts(Float.Constants.positiveInfinity))->toBe(false),
    )

    testSync(
      "names the scale it is not",
      () =>
        expect(
  ...
  describe("Bytes:", () => {
    ...
    testSync(
      "accepts the largest exact integer",
  ...
  describe("Duration:", () => {
    ...
    testSync(
      "accepts zero — an instant timeout is a duration",
  ...
  describe("the schema agrees with the constructor:", () => {
    ...
    testSync(
      ...
      () =>
        expect((
          parses(Email.schema, "buyer@example.com"),
          parses(Email.schema, "buyer"),
        ))->toEqual((true, false)),
    )
    testSync(
      "Url",
    ...
    testSync(
      "Percent",
      () =>
        expect((parses(Percent.schema, 42.5), parses(Percent.schema, 101.0)))->toEqual((
          true,
          false,
        )),
    )
    ...
    testSync(
      "Duration",
      () =>
        expect((parses(Duration.schema, 3660), parses(Duration.schema, -1)))->toEqual((
          true,
          false,
        )),
    )
