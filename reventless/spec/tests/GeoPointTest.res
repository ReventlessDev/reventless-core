...
describe("GeoPoint:", () => {
  ...
  describe("validateLat is the single statement of the latitude range:", () => {
    testSync(
      "the equator is accepted",
      () => expect(GeoPoint.validateLat(0.0)->Result.isOk)->toBe(true),
    )
    testSync(
      "the north pole is accepted",
      () => expect(GeoPoint.validateLat(90.0)->Result.isOk)->toBe(true),
    )
    testSync(
      "the south pole is accepted",
      () => expect(GeoPoint.validateLat(-90.0)->Result.isOk)->toBe(true),
    )
    testSync(
      "beyond the pole is refused",
    ...
    testSync(
      "a longitude in the latitude's place is refused",
      () => expect(GeoPoint.validateLat(151.2093)->Result.isOk)->toBe(false),
    )
    testSync(
      "the refusal quotes the offending number",
      () =>
        expect(
    ...
    testSync(
      "a non-finite latitude is refused",
  ...
  describe("validateLng runs to ±180, not ±90:", () => {
    testSync(
      "the antimeridian is accepted",
      () => expect(GeoPoint.validateLng(180.0)->Result.isOk)->toBe(true),
    )
    testSync(
      "its western twin is accepted",
      () => expect(GeoPoint.validateLng(-180.0)->Result.isOk)->toBe(true),
    )
    testSync(
      "a longitude past ±90 is fine — this is what makes it a longitude",
      () => expect(GeoPoint.validateLng(151.2093)->Result.isOk)->toBe(true),
    )
    testSync(
      "beyond the antimeridian is refused",
  ...
  describe("distanceTo is the great-circle distance in metres:", () => {
    ...
    testSync(
      "Vienna to Bratislava is about 55 km",
      () => {
  ...
  describe("the GeoJSON codec is the one place the order is positional:", () => {
    ...
    testSync(
      "and it round-trips",
      () => expect(GeoPoint.fromGeoJson(geometry))->toEqual(Ok(vienna)),
    )
    ...
    testSync(
      "a swapped array is refused when the latitude cannot be one",
      () =>
        expect(
    ...
    testSync(
      "a non-point geometry is refused",
      () =>
        expect(
          GeoPoint.fromGeoJson(
    ...
    testSync(
      "a geometry without coordinates is refused",
      () =>
        expect(
          GeoPoint.fromGeoJson(
  ...
  describe("the schema:", () => {
    ...
    testSync(
      "decode REFUSES an out-of-range latitude",
      () => expect(parses(point(91.0, 0.0)))->toBe(false),
    )
    testSync(
      "decode REFUSES a negative out-of-range latitude",
      () => expect(parses(point(-91.0, 0.0)))->toBe(false),
    )
    testSync(
      "decode REFUSES an out-of-range longitude",
      () => expect(parses(point(0.0, 181.0)))->toBe(false),
    )
    testSync(
      "decode REFUSES a negative out-of-range longitude",
      () => expect(parses(point(0.0, -181.0)))->toBe(false),
    )

    testSync(
      "serializes as {lat, lng}",
      () => expect(vienna->Util_Sury.toJson(GeoPoint.schema))->toEqual(point(48.2082, 16.3738)),
    )
    ...
    testSync(
      "JSON from a hand-rolled {lat, lng} record parses unchanged",
      () => {
        let legacy: {"lat": float, "lng": float} = {"lat": 48.2082, "lng": 16.3738}
        expect(legacy->Obj.magic->S.parseOrThrow(~to=GeoPoint.schema))->toEqual(vienna)
      },
    )
  })
})
