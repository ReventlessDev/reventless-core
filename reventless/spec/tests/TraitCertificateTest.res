...
describe("TraitCertificate", () => {
  describe("fromReport", () => {
    testSync(
      "counts are derived, not asserted by the caller",
      () => {
    ...
    testSync(
      "every assertion is carried by name, in report order",
      () =>
  ...
  describe("verified", () => {
    ...
    testSync(
      "an empty suite is not verified, however few failures it has",
  ...
  describe("render", () => {
    testSync(
      "is byte-stable and newline-terminated",
      () => {
        let c = cert([("a", true)])
        expect((
  ...
  describe("summarize", () => {
    testSync(
      "says the verdict, not only the numbers",
      () =>
        expect(
          cert([("a", false)])->TraitCertificate.summarize->String.includes("NOT verified"),
        )->toBe(true),
    )
  })
})
