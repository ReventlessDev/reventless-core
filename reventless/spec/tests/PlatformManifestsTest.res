...
describe("PlatformManifests.resolve", () => {
  describe("arm 1 — the entry is itself a composition package", () =>
    testSync(
      "its own src/capabilities.json is the manifest",
      () => {
  ...
  describe("arm 2 — an -aws root generated from a composition src/ in the same repo", () => {
    ...
    testSync(
      "the `generate` script leads to the composition's manifest",
      () => {
        let fs = fakeFs([
    ...
    testSync(
      "an unbuilt composition is Unbuilt, not NotAPlugin",
      () =>
  ...
  describe("arm 3 — an -aws root that installs a published composition package", () => {
    ...
    testSync(
      "the dependency's manifest resolves, naming the package",
      () => {
        let fs = fakeFs([
    ...
    testSync(
      "a hoisted dependency is found by walking up to the parent node_modules",
      () => {
        let fs = fakeFs([
    ...
    testSync(
      "two plugin dependencies both contribute",
      () => {
        let fs = fakeFs([
          (
    ...
    testSync(
      "an installed but unbuilt plugin dependency is Unbuilt",
      () =>
        switch PlatformManifests.resolve(
          ~fs=fakeFs([
            ("/repo/catalog-aws/package.json", installingRoot),
    ...
    testSync(
      "an unbuilt plugin dependency fails even when a sibling resolved",
      () =>
        switch PlatformManifests.resolve(
          ~fs=fakeFs([
            (
  ...
  describe("arm 4 — the entry is not a plugin", () => {
    ...
    testSync(
      "a stack depending on no plugin package contributes nothing",
      () => {
        let fs = fakeFs([
          (
            "/repo/sdk-service/package.json",
            `{"dependencies": {"@reventlessdev/reventless-aws": "^3"}}`,
          ),
          (
    ...
    testSync(
      "a stack with no package.json at all contributes nothing",
      () =>
        expect(PlatformManifests.resolve(~fs=fakeFs([]), ~pluginDir="/repo/spa"))->toEqual(
          NotAPlugin,
        ),
    )
    ...
    testSync(
      "a dependency that only provides the emit-capabilities bin is not a plugin",
      () => {
        let fs = fakeFs([
          (
            "/repo/tool/package.json",
            `{"dependencies": {"@reventlessdev/reventless-local": "^3"}}`,
          ),
          (
  ...
  describe("resolution order", () =>
    ...
    // the wrong file.
    testSync(
      "its own src/ wins over a `generate` script pointing elsewhere",
      () => {
        let fs = fakeFs([
          ("/repo/catalog/src/capabilities.json", emptyManifest),
          (
