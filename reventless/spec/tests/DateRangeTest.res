...
describe("DateRange:", () => {
  ...
  describe("duration is the length in whole seconds:", () => {
    ...
    testSync(
      "a zero-length range is zero seconds",
  ...
  describe("contains is [start, end) — end exclusive:", () => {
    testSync(
      "the start instant is contained",
      () => expect(DateRange.contains(dr(nine, thirteen), nine))->toBe(true),
    )
    testSync(
      "an instant inside is contained",
    ...
    testSync(
      "the end instant is NOT contained",
      () => expect(DateRange.contains(dr(nine, thirteen), thirteen))->toBe(false),
    )
    testSync(
      "a zero-length range contains nothing, not even its own instant",
  ...
  describe("overlaps is end-exclusive too:", () => {
    ...
    testSync(
      "ranges that share an interior instant overlap",
  ...
  describe("validate is the single statement of the ordering rule:", () => {
    ...
    testSync(
      "the refusal shows both instants",
      () =>
        expect(
  ...
  describe("format is the two instants, locale-independent:", () =>
    testSync(
      "an en dash between start and end",
  ...
  describe("the schema:", () => {
    ...
    testSync(
      "decode does NOT enforce ordering — a reversed range still parses",
      () => expect(parses(range(eleven, nine)))->toBe(true),
    )
    testSync(
      "but validate refuses the same reversed range",
      () => expect(DateRange.validate(dr(eleven, nine))->Result.isOk)->toBe(false),
    )
    ...
    testSync(
      "serializes end under the wire key \"end\"",
      () =>
        expect(dr(nine, eleven)->Util_Sury.toJson(DateRange.schema))->toEqual(range(nine, eleven)),
    )
  })
})
