...
describe("Money:", () => {
  ...
  describe("the currency table is generated from the standard:", () => {
    testSync(
      "every currency has an exponent — the whole point of closing the type",
      () => expect(Currency.all->Array.every(c => Currency.exponent(c) >= 0))->toBe(true),
    )

    testSync(
      "the codes the framework admits are there",
      () =>
        expect(
          ["EUR", "USD", "GBP", "JPY", "CHF", "CNY"]->Array.every(
            code => Currency.fromString(code)->Result.isOk,
          ),
        )->toBe(true),
    )
    ...
    testSync(
      "a dormant ISO code is not a currency here",
      () =>
        expect(
    ...
    testSync(
      "the metals and the sentinels are not currencies",
      () =>
        expect(
          ["XAU", "XAG", "XDR", "XXX", "XTS"]->Array.some(
            code => Currency.fromString(code)->Result.isOk,
          ),
        )->toBe(false),
    )
    ...
    testSync(
      "the exponents match ISO 4217",
      () =>
        expect(
          ["EUR", "JPY", "SEK"]->Array.map(
            code =>
    ...
    testSync(
      "a lower-case code is rejected, not repaired",
      () => expect(Currency.fromString("eur")->Result.isOk)->toBe(false),
    )

    testSync(
      "the rejection says what it wanted and shows the value",
      () =>
        expect(
    ...
    testSync(
      "every listed currency round-trips through its code",
      () =>
        expect(
          Currency.all->Array.every(
            c =>
  ...
  describe("format places the decimal point from the currency:", () => {
    ...
    testSync(
      "an amount under one unit keeps its leading zero",
  ...
  describe("ofMajor converts a decimal at the boundary:", () => {
    ...
    testSync(
      "what it produces is always valid money",
      () =>
        expect(
          Money.validateAmount(Money.ofMajor(~amount=0.1 +. 0.2, ~currency=EUR).amount),
        )->toEqual(Ok(30.0)),
    )

    testSync(
      "toMajor is its inverse for a whole major amount",
      () => expect(Money.toMajor(Money.ofMajor(~amount=1234.56, ~currency=EUR)))->toBe(1234.56),
    )

    testSync(
      "and gives a form the figure it shows",
      () =>
        expect((Money.toMajor(m(1050.0, EUR)), Money.toMajor(m(1000.0, JPY))))->toEqual((
          10.5,
          1000.0,
        )),
    )
  ...
  describe("add checks the currency rather than assuming it:", () => {
    ...
    testSync(
      "exactness is why the amount is in minor units",
      () =>
    ...
    testSync(
      "and it stays exact over a long run",
      () =>
        expect(Money.sum(Array.make(~length=10, m(10.0, EUR))))->toEqual(Some(Ok(m(100.0, EUR)))),
    )

    testSync(
      "sum refuses at the first currency that does not match",
      () =>
        expect(
    ...
    testSync(
      "the sum of no amounts has no currency to be in",
  ...
  describe("times multiplies by a count:", () => {
    testSync(
      "a line item is its unit price times its quantity",
      () => expect(Money.times(m(1050.0, EUR), ~by=3))->toEqual(Ok(m(3150.0, EUR))),
    )

    testSync(
      "none of something is nothing, in the same currency",
      () => expect(Money.times(m(1050.0, EUR), ~by=0))->toEqual(Ok(m(0.0, EUR))),
    )

    testSync(
      "a negative count reverses the sign",
    ...
    testSync(
      "a product past the exact-integer range is refused",
      () => expect(Money.times(m(9_000_000_000_000_000.0, EUR), ~by=3)->Result.isOk)->toBe(false),
    )

    testSync(
      "and the refusal says what it could not stay inside",
      () =>
        expect(
  ...
  describe("allocate splits an amount without losing a unit:", () => {
    ...
    testSync(
      "€10.00 into three is 3.34, 3.33, 3.33",
      () =>
        expect(amounts(Money.allocate(m(1000.0, EUR), ~into=3)))->toEqual([334.0, 333.0, 333.0]),
    )

    testSync(
      "and those add back up to what was split",
      () =>
        expect(Money.allocate(m(1000.0, EUR), ~into=3)->Result.getOr([])->Money.sum)->toEqual(
          Some(Ok(m(1000.0, EUR))),
        ),
    )

    // The property, over sizes where the remainder lands differently each time.
    testSync(
      "a split always adds back up to the whole",
      () =>
        expect(
          [1.0, 2.0, 7.0, 1000.0, 1001.0, 999999.0]->Array.every(
            amount =>
              [1, 2, 3, 7, 11]->Array.every(
                into =>
    ...
    testSync(
      "every part is a whole minor unit",
      () =>
        expect(
    ...
    testSync(
      "a split into one is the amount itself",
      () => expect(amounts(Money.allocate(m(1000.0, EUR), ~into=1)))->toEqual([1000.0]),
    )

    testSync(
      "more parts than units leaves the tail at zero",
    ...
    testSync(
      "a negative amount splits away from zero",
      () =>
    ...
    testSync(
      "yen split three ways are whole yen",
      () => expect(amounts(Money.allocate(m(10.0, JPY), ~into=3)))->toEqual([4.0, 3.0, 3.0]),
    )

    testSync(
      "a split into no parts at all is refused",
      () =>
        expect((
  ...
  describe("the schema:", () => {
    ...
    testSync(
      "accepts an amount in minor units",
    ...
    testSync(
      "rejects a fraction of a minor unit",
      () => expect(parses(money(10.5, "EUR")))->toBe(false),
    )
    testSync(
      "rejects a non-finite amount",
    ...
    testSync(
      "accepts an amount an int32 could not hold",
      () => expect(parses(money(9_000_000_000.0, "EUR")))->toBe(true),
    )

    testSync(
      "rejects an unknown currency code",
      () => expect(parses(money(1000.0, "QQQ")))->toBe(false),
    )
    testSync(
      "rejects a currency the framework has not admitted",
      () => expect(parses(money(1000.0, "AED")))->toBe(false),
    )
    testSync(
      "rejects a lower-case currency code",
    ...
    testSync(
      "serializes the currency as its three-letter code",
