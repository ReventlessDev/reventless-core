...
describe("CapabilityNeed", () => {
  describe("round trip", () => {
    ...
    testSync(
      "an unknown name is reported as unknown, not folded into an arm",
  ...
  describe("unmet", () => {
    ...
    testSync(
      "a declared need the platform provisions is met",
      () => expect(CapabilityNeed.unmet(~declared, ~provisioned=[Geocoding]))->toEqual([]),
    )

    testSync(
      "a declared need the platform provisions nothing for is unmet",
      () =>
    ...
    testSync(
      "a plugin that declares nothing is never unmet",
      () => expect(CapabilityNeed.unmet(~declared=[], ~provisioned=[]))->toEqual([]),
    )
    ...
    testSync(
      "a capability this build does not recognise is skipped",
      () =>
        expect(
    ...
    testSync(
      "every declaring component is named, not just the first",
      () =>
        expect(
    ...
    testSync(
      "one provisioned capability does not cover another",
      () =>
        expect(
          CapabilityNeed.unmet(
            ~declared=[
              ("Geocoding", "GeocodeCustomerAddress"),
              ("Messaging", "SendOrderConfirmation"),
            ],
  ...
  describe("unmetMessage", () => {
    ...
    testSync(
      "names the capability and the component that declared it",
    ...
    testSync(
      "states the silent outcome it is preventing",
