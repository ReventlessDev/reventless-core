...
describe("Lifecycle:", () => {
  describe("a declared trail field:", () => {
    testSync(
      "resolves to a schema from the type alone",
      () =>
        expect(
          {
            orderId: "o1",
        ...
        )->toBe(`{"orderId":"o1","lifecycle":"Placed","trail":[{"state":"Placed","at":"2026-03-02T09:00:00Z"}]}`),
    )

    testSync(
      "is found by name on the state schema",
    ...
    testSync(
      "refuses an entry whose instant is not one",
      () =>
        expect(
  ...
  describe("the lifecycle field:", () => {
    testSync(
      "is the enum field named lifecycle",
    ...
    testSync(
      "is not a string field of the same name",
      () => expect(Lifecycle.fieldName(freeTextSchema->S.castToUnknown))->toEqual(None),
    )

    testSync(
      "is not guessed from another name",
  ...
  describe("recording an entry:", () => {
    testSync(
      ...
      () => {
        ...
        expect(
          state->JSON.Encode.object->JSON.stringify,
        )->toBe(`{"lifecycle":"Placed","trail":[{"state":"Placed","at":"2026-03-02T09:00:00Z"}]}`)
      },
    )
    ...
    testSync(
      "keeps a repeated state as a second entry",
      () => {
        let state = dict(`{"trail":[{"state":"Placed","at":"2026-03-02T09:00:00Z"},{"state":"Cancelled","at":"2026-03-02T10:00:00Z"}]}`)
        state->Lifecycle.Trail.record(
          ~field="trail",
          ~state=JSON.Encode.string("Placed"),
        ...
        expect(
          state->JSON.Encode.object->JSON.stringify,
        )->toBe(`{"trail":[{"state":"Placed","at":"2026-03-02T09:00:00Z"},{"state":"Cancelled","at":"2026-03-02T10:00:00Z"},{"state":"Placed","at":"2026-03-02T11:00:00Z"}]}`)
      },
    )
  })
})
