...
describe("Compat.validateAndProject", () => {
  describe("required field validation", () => {
    testSync(
      "succeeds when all required fields are in the manifest",
      () => {
    ...
    testSync(
      "succeeds when requiredFields is empty",
      () => {
    ...
    testSync(
      "fails with MissingRequiredField when a required field is absent",
      () => {
    ...
    testSync(
      "fails when manifest for outputName is absent entirely",
      () => {
    ...
    testSync(
      "includes the first missing field in the error (not all missing fields)",
      () => {
  ...
  describe("fromJson decoding", () => {
    testSync(
      "calls fromJson when all required fields are present",
      () => {
        let meta = makeMeta([("tasks", ["name"])])
    ...
    testSync(
      "does not call fromJson when a required field is missing",
      () => {
        let meta = makeMeta([("tasks", [])])
        let called = ref(false)
        let trackingFromJson = (json: JSON.t) => {
    ...
    testSync(
      "wraps fromJson error in DecodeFailed",
      () => {
        let meta = makeMeta([("tasks", ["name"])])
        let result = Compat.validateAndProject(
          ~stackName="org/plugin/prod",
        ...
        expect(result)->toEqual(
          Error(Compat.DecodeFailed({stackName: "org/plugin/prod", reason: "decode failed"})),
        )
      },
    )

    testSync(
      "passes the raw JSON value to fromJson unchanged",
      () => {
  ...
  describe("optional fields", () => {
    testSync(
      "does not require optional fields to be in the manifest",
      () => {
  ...
  describe("stackName in errors", () => {
    testSync(
      "uses the provided stackName in MissingRequiredField error",
      () => {
        let meta = makeMeta([("tasks", [])])
        let result = Compat.validateAndProject(
    ...
    testSync(
      "uses the provided stackName in DecodeFailed error",
      () => {
        let meta = makeMeta([("tasks", ["name"])])
        let result = Compat.validateAndProject(
          ~stackName="myorg/my-plugin/staging",
  ...
  describe("field manifest boundary cases", () => {
    testSync(
      "succeeds when manifest has more fields than required",
      () => {
    ...
    testSync(
      "succeeds with empty required fields even when manifest is empty",
      () => {
        let meta = makeMeta([("tasks", [])])
        let result = Compat.validateAndProject(
          ~stackName="org/plugin/prod",
