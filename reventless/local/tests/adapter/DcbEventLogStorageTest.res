...
describe("DcbEventLogStorage_InMemory", () => {
  describe("append", () => {
    testPromise(
      "stores events and returns incrementing position",
      async () => {
    ...
    testPromise(
      "appending multiple events at once returns position of last event",
      async () => {
        let storage = makeStorage()
        let ops = await storage.operations->TestRunner.resolve
  ...
  describe("read", () => {
    ...
    testPromise(
      ...
      async () => {
        ...
        let result = await ops.read(~query=[{Reventless.DcbTag.eventTypes: ["Created"]}])
        expect(result.events->Array.length)->toBe(2)
    ...
    testPromise(
      "multi-tag clause matches only events carrying ALL tags",
      async () => {
        let storage = makeStorage()
        let ops = await storage.operations->TestRunner.resolve
        let _ = await ops.append([
          ...
          makeEvent(
            ~eventType="Evt",
            ~data=JSON.Null,
            ~tags=[{Reventless.DcbTag.key: "region", value: "eu"}],
          ),
        ])
    ...
    testPromise(
      "OR of clauses unions matches across clauses in position order",
      async () => {
        let storage = makeStorage()
        let ops = await storage.operations->TestRunner.resolve
        let _ = await ops.append([
          ...
          makeEvent(
            ~eventType="B",
            ~data=JSON.Null,
            ~tags=[{Reventless.DcbTag.key: "x", value: "1"}],
          ),
    ...
    testPromise(
      "an empty tag value appends and stays readable",
      async () => {
        let storage = makeStorage()
        let ops = await storage.operations->TestRunner.resolve
        let tags = [
    ...
    testPromise(
      "after parameter skips events at or before that position",
      async () => {
        let storage = makeStorage()
        let ops = await storage.operations->TestRunner.resolve
        let _ = await ops.append([
  ...
  describe("headPosition", () => {
    testPromise(
      "equals the position of the last appended event",
      async () => {
        let storage = makeStorage()
        let ops = await storage.operations->TestRunner.resolve
        let _ = await ops.append([
    ...
    testPromise(
      "headPosition is absent when no events have been stored",
      async () => {
        let storage = makeStorage()
        let ops = await storage.operations->TestRunner.resolve
        let result = await ops.read(~query=[])
  ...
  describe("conditional append", () => {
    testPromise(
      "append with matching condition query returns Error (conflict)",
      async () => {
        let storage = makeStorage()
        let ops = await storage.operations->TestRunner.resolve
    ...
    testPromise(
      "append with condition that matches no events succeeds",
      async () => {
        let storage = makeStorage()
        let ops = await storage.operations->TestRunner.resolve
    ...
    testPromise(
      "condition with after only checks events after that position",
      async () => {
        let storage = makeStorage()
        let ops = await storage.operations->TestRunner.resolve
