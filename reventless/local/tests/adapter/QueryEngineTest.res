...
describe("LocalQueryEngine", () => {
  describe("query", () => {
    ...
    testPromise(
      "query with explicit key overrides the id parameter",
      async () => {
        module TestBus = LocalBus.Make()
        module Storage = LocalQueryDbStorage.Make(TestBus)
        module QE = LocalQueryEngine.Make(TestBus)
    ...
    testPromise(
      "query with Int id converts to string key for lookup",
      async () => {
        module TestBus = LocalBus.Make()
        module Storage = LocalQueryDbStorage.Make(TestBus)
        module QE = LocalQueryEngine.Make(TestBus)
  ...
  describe("scan with ~limit", () => {
    testPromise(
      "~limit=2 on a 5-item QueryDb returns exactly 2 items",
      async () => {
        module TestBus = LocalBus.Make()
        module Storage = LocalQueryDbStorage.Make(TestBus)
        module QE = LocalQueryEngine.Make(TestBus)
    ...
    testPromise(
      "~limit larger than total returns all items (no padding)",
      async () => {
        module TestBus = LocalBus.Make()
        module Storage = LocalQueryDbStorage.Make(TestBus)
        module QE = LocalQueryEngine.Make(TestBus)
    ...
    testPromise(
      "~limit=3 on a 3-item QueryDb returns all 3 items",
      async () => {
        module TestBus = LocalBus.Make()
        module Storage = LocalQueryDbStorage.Make(TestBus)
        module QE = LocalQueryEngine.Make(TestBus)
