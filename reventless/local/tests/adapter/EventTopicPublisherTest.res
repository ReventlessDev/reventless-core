...
describe("LocalEventTopicPublisher", () => {
  describe("make", () => {
    testPromise(
      "returns one resource whose name resolves to the topic name",
      async () => {
  ...
  describe("publishJson", () => {
    testPromise(
      "delivers event to subscriber on the same topic",
      async () => {
        module TestBus = LocalBus.Make()
        module TestPublisher = LocalEventTopicPublisher.Make(TestBus)
    ...
    testPromise(
      "publishing to topic-a does not trigger topic-b subscriber",
      async () => {
        module TestBus = LocalBus.Make()
        module TestPublisher = LocalEventTopicPublisher.Make(TestBus)
    ...
    testPromise(
      "passes service name from publishJson call to subscriber",
      async () => {
        module TestBus = LocalBus.Make()
        module TestPublisher = LocalEventTopicPublisher.Make(TestBus)
