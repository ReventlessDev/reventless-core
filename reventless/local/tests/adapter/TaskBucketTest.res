...
describe("LocalTaskBucket", () => {
  describe("makeHandler", () => {
    testPromise(
      "calls callback with eventName and key extracted from JSON object",
      async () => {
        ...
        let handler = LocalTaskBucket.makeHandler(callback)
    ...
    testPromise(
      ...
      async () => {
        ...
        let handler = LocalTaskBucket.makeHandler(callback)
        let json = JSON.Encode.object(Dict.fromArray([("key", JSON.Encode.string("some/path"))]))
        let _ = await handler(json, ())
    ...
    testPromise(
      "non-object JSON uses 'ObjectCreated' and empty string for key",
      async () => {
        let receivedEventName: ref<string> = ref("")
        let receivedKey: ref<string> = ref("initial")
        let callback: ReventlessCore.Task.bucketCallback = async (~eventName, ~key) => {
    ...
    testPromise(
      "non-string eventName field falls back to ObjectCreated",
      async () => {
        let receivedEventName: ref<string> = ref("")
        let callback: ReventlessCore.Task.bucketCallback = async (~eventName, ~key as _) => {
          receivedEventName := eventName
        ...
        let handler = LocalTaskBucket.makeHandler(callback)
        let json = JSON.Encode.object(Dict.fromArray([("eventName", JSON.Encode.int(42))]))
        let _ = await handler(json, ())
        expect(receivedEventName.contents)->toBe("ObjectCreated")
      },
    )
    ...
    testPromise(
      "returns task actions from callback",
      async () => {
  ...
  describe("make", () => {
    testPromise(
      ...
      async () => {
        let bucket = LocalTaskBucket.make(~name="my-bucket", ~opts={})
