...
describe("EventLog.replayStream (in-memory adapter)", () => {
  ...
  describe("basic streaming", () => {
    testPromise(
      "replayStream returns all appended events",
      async () => {
        let ops = await eventLog->ReventlessCore.Component.operations->TestRunner.resolve
        let _ = await ops.append(
          0,
          "stream-1",
          [makeEvent'("stream-1", ItemEventLogSpec.ItemCreated({name: "Widget"}))],
        )
        let _ = await ops.append(
          1,
          "stream-1",
          [makeEvent'("stream-1", ItemEventLogSpec.ItemDeleted({id: "stream-1"}))],
        )
    ...
    testPromise(
      "replayStream for unknown id returns empty stream",
      async () => {
        let ops = await eventLog->ReventlessCore.Component.operations->TestRunner.resolve
    ...
    testPromise(
      "replayStream emits events in append order",
      async () => {
        let ops = await eventLog->ReventlessCore.Component.operations->TestRunner.resolve
        let _ = await ops.append(
          0,
          "stream-2",
          [makeEvent'("stream-2", ItemEventLogSpec.ItemCreated({name: "First"}))],
        )
        let _ = await ops.append(
          1,
          "stream-2",
          [makeEvent'("stream-2", ItemEventLogSpec.ItemDeleted({id: "stream-2"}))],
        )
    ...
    testPromise(
      "separate aggregate IDs have independent streams",
      async () => {
        let ops = await eventLog->ReventlessCore.Component.operations->TestRunner.resolve
        let _ = await ops.append(
          0,
          "agg-X",
          [makeEvent'("agg-X", ItemEventLogSpec.ItemCreated({name: "X"}))],
        )
        let _ = await ops.append(
          0,
          "agg-Y",
          [
            makeEvent'("agg-Y", ItemEventLogSpec.ItemCreated({name: "Y1"})),
            makeEvent'("agg-Y", ItemEventLogSpec.ItemDeleted({id: "agg-Y"})),
          ],
        )
    ...
    testPromise(
      "take(2) on a 5-event log returns only 2 events",
      async () => {
        let ops = await eventLog->ReventlessCore.Component.operations->TestRunner.resolve
        let _ = await ops.append(
          0,
  ...
  describe("tuple fold — (state, count) pattern used in Aggregate_Callback", () => {
    testPromise(
      "runFold produces correct count for sequenceNr",
      async () => {
        let ops = await eventLog->ReventlessCore.Component.operations->TestRunner.resolve
        let _ = await ops.append(
          0,
          "fold-1",
          [
            makeEvent'("fold-1", ItemEventLogSpec.ItemCreated({name: "e1"})),
            makeEvent'("fold-1", ItemEventLogSpec.ItemDeleted({id: "e2"})),
          ],
        )
        let _ = await ops.append(
          2,
          "fold-1",
          [makeEvent'("fold-1", ItemEventLogSpec.ItemCreated({name: "e3"}))],
        )
    ...
    testPromise(
      "runFold on empty stream returns initial accumulator",
      async () => {
        let ops = await eventLog->ReventlessCore.Component.operations->TestRunner.resolve
