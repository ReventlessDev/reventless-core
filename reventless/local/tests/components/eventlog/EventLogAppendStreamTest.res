...
describe("EventLog.appendStream (in-memory adapter)", () => {
  ...
  describe("basic append", () => {
    testPromise(
      "appendStream writes all events to storage",
      async () => {
    ...
    testPromise(
      "appendStream on empty stream writes nothing",
      async () => {
        let ops = await eventLog->ReventlessCore.Component.operations->TestRunner.resolve
    ...
    testPromise(
      "appendStream preserves event order",
      async () => {
        let ops = await eventLog->ReventlessCore.Component.operations->TestRunner.resolve
        let stream =
          [
    ...
    testPromise(
      "appendStream after array append continues from correct sequenceNr",
      async () => {
        let ops = await eventLog->ReventlessCore.Component.operations->TestRunner.resolve
  ...
  describe("replayStream → appendStream pipeline", () => {
    testPromise(
      "can copy events from one aggregate to another via streams",
      async () => {
        let ops = await eventLog->ReventlessCore.Component.operations->TestRunner.resolve
        let _ = await ops.append(
          0,
    ...
    testPromise(
      "copied events round-trip through decode correctly",
      async () => {
        let ops = await eventLog->ReventlessCore.Component.operations->TestRunner.resolve
        let _ = await ops.append(
          0,
