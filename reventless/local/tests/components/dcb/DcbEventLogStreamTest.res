...
describe("DcbEventLog.readStream (in-memory adapter)", () => {
  ...
  describe("basic streaming", () => {
    testPromise(
      "readStream for tag with no matching events returns empty stream",
      async () => {
    ...
    testPromise(
      "readStream emits all events matching a tag query",
      async () => {
        let ops = await eventLog->ReventlessCore.Component.operations->TestRunner.resolve
    ...
    testPromise(
      "readStream for distinct IDs returns independent streams",
      async () => {
        let ops = await eventLog->ReventlessCore.Component.operations->TestRunner.resolve
    ...
    testPromise(
      "take(1) limits stream without loading all",
      async () => {
        let ops = await eventLog->ReventlessCore.Component.operations->TestRunner.resolve
  ...
  describe("fold patterns — (state, headPosition) used in StateChangeSlice_Callback", () => {
    testPromise(
      "runFold extracts last headPosition for append condition",
      async () => {
        let ops = await eventLog->ReventlessCore.Component.operations->TestRunner.resolve
  ...
  describe(
    "StateChangeSlice regression — second command sees first event via stream fold",
    () => {
      testPromise(
        "second AddItem with same ID is rejected (state sees existing event)",
        async () => {
      ...
      testPromise(
        "second AddItem with different ID succeeds",
        async () => {
          let id1 = "distinct-1"
          let id2 = "distinct-2"
