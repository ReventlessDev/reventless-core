...
describe("StateChangeSlice_Callback:", () => {
  describe("handleCommands - happy path", () => {
    testPromise(
      "CreateItem on empty log succeeds",
      async () => {
    ...
    testPromise(
      "stored event carries only its entity tags (no framework provenance tag)",
      async () => {
  ...
  describe("handleCommands - decide returns Ok([])", () => {
    testPromise(
      "NoOp returns Ok without storing events",
      async () => {
        let results = await TestHandler.handleCommands(
          testDcbEventLog,
  ...
  describe("handleCommands - decide returns Error", () => {
    testPromise(
      "CreateItem when item exists returns Ok (business-rule violations ACK, not NACK)",
      async () => {
  ...
  describe("handleCommands - retry on conflict", () => {
    testPromise(
      "retries and succeeds after 1 append failure",
      async () => {
    ...
    testPromise(
      "returns Error after retries exhausted (4 failures)",
      async () => {
  ...
  describe("handleCommands - read consistency (eventual-first, strong-on-retry)", () => {
    testPromise(
      "a conflict-free command reads eventually-consistent (no strong read)",
      async () => {
        let _ = await TestHandler.handleCommands(
          testDcbEventLog,
          Stream.fromIterable([
    ...
    testPromise(
      "escalates to strong consistency on retry after a conflict",
      async () => {
        mock.failNextAppends := 1
        let _ = await TestHandler.handleCommands(
          testDcbEventLog,
    ...
    testPromise(
      "every retry after the first is strong (2 failures)",
      async () => {
  ...
  describe("handleCommands - read consistency (per-slice override)", () => {
    testPromise(
      "AlwaysStrong reads strong on the first attempt",
      async () => {
    ...
    testPromise(
      "AlwaysStrong stays strong across retries (2 failures)",
      async () => {
        mock.failNextAppends := 2
        let _ = await StrongHandler.handleCommands(
          testDcbEventLog,
    ...
    testPromise(
      "AlwaysEventual never reads strong, even on retry (2 failures)",
      async () => {
        mock.failNextAppends := 2
  ...
  describe("handleCommands - conditional append", () => {
    testPromise(
      "RenameItem after CreateItem uses headPosition in condition",
      async () => {
        let _ = await TestHandler.handleCommands(
          testDcbEventLog,
          Stream.fromIterable([
  ...
  describe("handleCommands - batch", () => {
      ...
      async () => {
        let results = await TestHandler.handleCommands(
          ...
          Stream.fromIterable([
            makeTopicItem(
              "ref-1",
              DcbFixtures.TestCommandSpec.CreateItem({itemId: "item-1", name: "First"}),
            ),
            makeTopicItem(
              "ref-2",
              DcbFixtures.TestCommandSpec.CreateItem({itemId: "item-2", name: "Second"}),
            ),
          ]),
        )->Effect.runPromise
    ...
    testPromise(
      "mixed: new item succeeds, duplicate item ACKs as Ok (business-rule violation)",
      async () => {
        // Create item-1 first
        let _ = await TestHandler.handleCommands(
          testDcbEventLog,
          Stream.fromIterable([
            makeTopicItem(
              "ref-0",
              DcbFixtures.TestCommandSpec.CreateItem({itemId: "item-1", name: "Existing"}),
            ),
          ]),
        )->Effect.runPromise
        ...
        let results = await TestHandler.handleCommands(
          ...
          Stream.fromIterable([
            makeTopicItem(
              "ref-1",
              DcbFixtures.TestCommandSpec.CreateItem({itemId: "item-2", name: "New"}),
            ),
            makeTopicItem(
              "ref-2",
              DcbFixtures.TestCommandSpec.CreateItem({itemId: "item-1", name: "Duplicate"}),
  ...
  describe("handleCommands - projection cache", () => {
    ...
    testPromise(
      "a third same-entity command reads only the delta",
      async () => {
        ...
        let _ = await run(
          "ref-1",
          DcbFixtures.TestCommandSpec.CreateItem({itemId: "item-1", name: "A"}),
        )
        let _ = await run(
          "ref-2",
          DcbFixtures.TestCommandSpec.RenameItem({itemId: "item-1", newName: "B"}),
        )
        let r3 = await run(
          "ref-3",
          DcbFixtures.TestCommandSpec.RenameItem({itemId: "item-1", newName: "C"}),
        )
    ...
    testPromise(
      "a different entity does not hit another entity's cache (full read)",
      async () => {
        let run = (reference, command) =>
          TestHandler.handleCommands(
            testDcbEventLog,
        ...
        let _ = await run(
          "ref-2",
          DcbFixtures.TestCommandSpec.CreateItem({itemId: "item-2", name: "B"}),
        )
    ...
    testPromise(
      "conflict re-seeds the cache so the retry reads the delta",
      async () => {
        let run = (reference, command) =>
          TestHandler.handleCommands(
            testDcbEventLog,
