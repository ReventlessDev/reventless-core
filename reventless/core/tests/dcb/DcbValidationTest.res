...
@schema
type producedB = ItemArchived({itemId: @s.matches(Reventless.DcbTag.string) string})
...
@schema
type producedAMismatchFields = ItemCreated({itemId: @s.matches(Reventless.DcbTag.string) string})

// A second producer for ItemCreated with mismatched tag annotations
@schema
type producedAMismatchTags = ItemCreated({itemId: string, name: string})
...
@schema
type consumedPartial = ItemCreated({itemId: string})
...
@schema
type consumedDangling = ItemDeleted({itemId: string})

// Consumed events — field not in produced shape
@schema
type consumedBadField = ItemCreated({itemId: string, rating: float})

// Consumed events — type mismatch
@schema
type consumedBadType = ItemCreated({itemId: string, name: float})
...
@schema
type touchDemandCommand = TouchDemand({productId: @s.matches(Reventless.DcbTag.string) string})
...
describe("DcbValidation:", () => {
  describe("validateProducedAndConsumed", () => {
    testSync(
      "passes when all consumed events match produced events",
      () => {
        let result = validate(
    ...
    testSync(
      "passes with partial field projection",
      () => {
        let result = validate(
    ...
    testSync(
      "passes with payload-less consumed events",
      () => {
        let result = validate(
          ~produced=[("AddItem", producedASchema->u), ("ArchiveItem", producedBSchema->u)],
    ...
    testSync(
      "passes when multiple producers have identical shapes and tags",
      () => {
        let result = validate(
    ...
    testSync(
      "fails when consumed event has no producer",
      () => {
        let result = validate(
          ~produced=[("AddItem", producedASchema->u)],
    ...
    testSync(
      "fails when consumed event references field not in produced shape",
      () => {
        let result = validate(
          ~produced=[("AddItem", producedASchema->u)],
    ...
    testSync(
      "fails when consumed field type mismatches produced field type",
      () => {
        let result = validate(
          ~produced=[("AddItem", producedASchema->u)],
    ...
    testSync(
      "fails when producers have different fields for same TAG",
      () => {
        let result = validate(
          ~produced=[
            ("AddItem", producedASchema->u),
            ("ImportItem", producedAMismatchFieldsSchema->u),
          ],
          ~consumed=[],
        )
        switch result {
    ...
    testSync(
      "fails when producers have different tag annotations for same TAG",
      () => {
        let result = validate(
          ~produced=[
            ("AddItem", producedASchema->u),
            ("UntaggerItem", producedAMismatchTagsSchema->u),
          ],
          ~consumed=[],
        )
        switch result {
        | Error(errors) =>
          expect(errors->Array.length)->toBeGreaterThanOrEqual(1)->ignore
          let hasTagError =
            errors->Array.some(err => err.message->String.includes("tag annotations"))
    ...
    testSync(
      "fails when producers have different field types for same TAG",
      () => {
        let result = validate(
          ~produced=[
            ("AddItem", producedASchema->u),
            ("BadTypeItem", producedAMismatchTypeSchema->u),
          ],
          ~consumed=[],
        )
        switch result {
    ...
    testSync(
      "passes with empty produced and consumed",
      () => {
    ...
    testSync(
      "collects multiple errors at once",
      () => {
        let result = validate(
          ~produced=[("AddItem", producedASchema->u)],
          ~consumed=[
            ("DeleteHandler", consumedDanglingSchema->u),
            ("BadTypeView", consumedBadTypeSchema->u),
          ],
        )
        switch result {
  ...
  describe("validateCompositeReads (Issue 5)", () => {
    let check = slices => Reventless.DcbValidation.validateCompositeReads(~slices, ~producedTagKeys)

    testSync(
      "warns when a composite read's consumed type carries an extra tag",
      () => {
        let warnings = check([
          ("RecordDemand", recordDemandCommandSchema->u, demandConsumedSchema->u),
        ])
    ...
    testSync(
      "does not warn for an exact-match consumed type",
      () => {
        ...
        expect(
          warnings->Array.some(w => w.message->String.includes("ProductDemandRecorded")),
        )->toBe(false)
      },
    )

    testSync(
      "ignores single-tag commands (partition read, not composite)",
      () => {
        let warnings = check([
          ("TouchDemand", touchDemandCommandSchema->u, demandConsumedSchema->u),
        ])
    ...
    testSync(
      "ignores tagged-array commands (per-element OR, not composite)",
      () => {
    ...
    testSync(
      "no warnings when nothing reads compositely",
      () => {
  ...
  describe("validateCrossPartitionScope (Issue 13)", () => {
    ...
    testSync(
      "no warning when every carrier of a key agrees on cross-partition scope",
      () => {
        let warnings = check([
    ...
    testSync(
      "warns when one producer leaves a cross-partition key partition-scoped",
      () => {
        let warnings = check([
          ("Subscribe", subscribedCrossPartitionSchema->u),
    ...
    testSync(
      "no warning when no key is cross-partition anywhere",
      () => {
