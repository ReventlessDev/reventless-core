...
describe("SuryToJsonSchema:", () => {
  describe("deriveObjectSchema with no annotations:", () => {
    testSync(
      "emits plain JSON Schema when no metadata is attached",
      () => {
        let schema = S.schema(
          s =>
            {
        ...
        expect(idSchema->Option.flatMap(s => getProperty(s, "x-reventless-id")))->toBe(None)
      },
    )

    testSync(
      "emits format:\"date-time\" for a DateTime-marked string field",
      () => {
        let schema = S.schema(
          s =>
            {
        ...
        expect(getPropertyOf(json, "name")->Option.flatMap(s => getProperty(s, "format")))->toBe(
          None,
        )
      },
    )
    ...
    testSync(
      "a DateTime field emits format and no semantic key",
      () => {
    ...
    testSync(
      "a CalendarDate field emits format:\"date\" AND the semantic key",
      () => {
        let json = SuryToJsonSchema.deriveObjectSchema(
  ...
  describe("deriveObjectSchema with an owner field:", () => {
    ...
    testSync(
      "the marked field carries x-reventless-owner",
    ...
    testSync(
      "an unmarked string field carries nothing",
      () => expect(ownerOf(json, "note"))->toBe(None),
    )

    testSync(
      "the field is still a plain string on the wire",
      () =>
        expect(
    ...
    testSync(
      "an optional owner field is still recognised",
      () => {
  ...
  describe("deriveObjectSchema with a sensitive field:", () => {
    ...
    testSync(
      "the marked field carries x-reventless-sensitive",
    ...
    testSync(
      "an email field is sensitive with no annotation",
    ...
    testSync(
      "an optional sensitive field is still recognised",
      () => {
        let optJson = SuryToJsonSchema.deriveObjectSchema(
          S.schema(
            s =>
    ...
    testSync(
      "sensitivity composes with ownership on one field",
      () => {
  ...
  describe("deriveObjectSchema with stateAnnotations metadata:", () => {
    ...
    testSync(
      "emits x-reventless-id on field listed in ids",
      () => {
        let schema = S.schema(
          s =>
            {
    ...
    testSync(
      "emits x-reventless-compositeId on each compositeId field",
      () => {
        let schema = S.schema(
          s =>
            {
    ...
    testSync(
      "emits x-reventless-subId on field listed in subIds",
      () => {
        let schema = S.schema(
          s =>
            {
    ...
    testSync(
      "emits x-reventless-index as the index name when named",
      () => {
        let schema = S.schema(
          s =>
            {
    ...
    testSync(
      "emits x-reventless-index as true for unnamed @index",
      () => {
        let schema = S.schema(
          s =>
            {
    ...
    testSync(
      "does not emit x-reventless-* on unannotated fields",
      () => {
        let schema = S.schema(
          s =>
            {
        ...
        expect(nameSchema->Option.flatMap(s => getProperty(s, "x-reventless-id")))->toBe(None)
      },
    )

    testSync(
      "emits x-reventless-hidden on field listed in hidden",
      () => {
        let schema = S.schema(
          s =>
            {
    ...
    testSync(
      "drops a field listed in internal rather than annotating it",
      () => {
        let schema = S.schema(
          s =>
            {
    ...
    testSync(
      "keeps a dropped field out of required",
      () => {
        let schema = S.schema(
          s =>
            {
    ...
    testSync(
      "emits x-reventless-summary on field listed in summary",
      () => {
        let schema = S.schema(
          s =>
            {
    ...
    testSync(
      "emits x-reventless-drillTarget as the slice name",
      () => {
        let schema = S.schema(
          s =>
            {
    ...
    testSync(
      "emits x-reventless-drillTargetKey as the key path",
      () => {
        let schema = S.schema(
          s =>
            {
    ...
    testSync(
      "does not emit x-reventless-drillTargetKey when no key was supplied",
      () => {
        let schema = S.schema(
          s =>
            {
    ...
    testSync(
      "emits x-reventless-scan on field listed in scan",
      () => {
        let schema = S.schema(
          s =>
            {
    ...
    testSync(
      "emits x-reventless-scanSort on field listed in scanSort",
      () => {
        let schema = S.schema(
          s =>
            {
    ...
    testSync(
      "emits x-reventless-semantic on the field listed in semantic",
      () => {
        let schema = S.schema(
          s =>
            {
    ...
    testSync(
      "emits x-reventless-metric {aggregate,label} on the field listed in metric",
      () => {
        let schema = S.schema(
          s =>
            {
        ...
        let schema' = schema->withSpec({
          ...emptySpec,
          metric: [("total", {aggregate: "sum", label: "Revenue"})],
        })
        let json = SuryToJsonSchema.deriveObjectSchema(schema')
        let metricObj =
          getPropertyOf(json, "total")->Option.flatMap(s => getProperty(s, "x-reventless-metric"))
        expect((
          metricObj
    ...
    testSync(
      "omits the metric label when empty (UI derives one from the field name)",
      () => {
        let schema = S.schema(s => {"total": s.matches(S.float)})->S.castToUnknown
        let schema' =
          schema->withSpec({...emptySpec, metric: [("total", {aggregate: "sum", label: ""})]})
        let json = SuryToJsonSchema.deriveObjectSchema(schema')
        let metricObj =
          getPropertyOf(json, "total")->Option.flatMap(s => getProperty(s, "x-reventless-metric"))
        expect((
    ...
    testSync(
      "emits x-reventless-retired {label,showWhenFalse} on the retired field",
      () => {
        let schema = S.schema(
          s =>
            {
        ...
        let json = SuryToJsonSchema.deriveObjectSchema(schema')
        let retiredObj =
          getPropertyOf(json, "archived")->Option.flatMap(
            s => getProperty(s, "x-reventless-retired"),
          )
        expect((
          retiredObj
          ->Option.flatMap(r => getProperty(r, "label"))
          ->Option.flatMap(JSON.Decode.string),
          retiredObj
    ...
    testSync(
      "carries one retirement state as a one-element set",
      () => {
        expect(
    ...
    testSync(
      "carries every retirement state a lifecycle declares",
      () => {
        expect(
    ...
    testSync(
      "emits the singular value beside a one-element set",
      () => {
        expect(
    ...
    testSync(
      "omits the singular value for a set of two",
      () => {
        expect(
          stateFormJson(~values=["Archived", "Discontinued"])
          ->retiredOn("accountStatus")
    ...
    testSync(
      "omits both value and values entirely on the boolean form",
      () => {
    ...
    testSync(
      "carries namedWhenRetired only when the record opted in",
      () => {
    ...
    testSync(
      "omits the retired label when empty but always states showWhenFalse",
      () => {
        ...
        let json = SuryToJsonSchema.deriveObjectSchema(schema')
        let retiredObj =
          getPropertyOf(json, "deactivated")->Option.flatMap(
            s => getProperty(s, "x-reventless-retired"),
          )
        expect((
    ...
    testSync(
      "emits nothing for a schema with no retired annotation",
      () => {
        let schema = S.schema(s => {"archived": s.matches(S.bool)})->S.castToUnknown
    ...
    testSync(
      "emits x-reventless-group-by on the field named by groupBy",
      () => {
        let schema = S.schema(
          s =>
            {
    ...
    testSync(
      "emits x-reventless-collapsed on field listed in collapsed",
      () => {
        let schema = S.schema(
          s =>
            {
    ...
    testSync(
      "emits x-reventless-visibility at top level when visibility is Internal",
      () => {
    ...
    testSync(
      "omits x-reventless-visibility when visibility is None (default Public)",
      () => {
        let schema = S.schema(s => {"id": s.matches(S.string)})->S.castToUnknown
    ...
    testSync(
      "emits x-reventless-live at top level when live is declared",
      () => {
        let schema = S.schema(s => {"id": s.matches(S.string)})->S.castToUnknown
    ...
    testSync(
      "omits x-reventless-live when live is None (no annotation)",
      () => {
        let schema = S.schema(s => {"id": s.matches(S.string)})->S.castToUnknown
    ...
    testSync(
      "emits x-reventless-live and x-reventless-visibility together",
      () => {
        let schema = S.schema(s => {"id": s.matches(S.string)})->S.castToUnknown
        let schema' = schema->withSpec({
          ...emptySpec,
  ...
  describe("reference / DCB-tag decoupling:", () => {
    ...
    testSync(
      "a DCB-tagged field with no reference is still an entity id",
      () => {
        let schema = S.schema(
          s =>
            {
    ...
    testSync(
      "a reference without a DCB tag is still an entity id",
      () => {
    ...
    testSync(
      "Reference.to_ carries both facts",
      () => {
    ...
    testSync(
      "neither reference nor tag emits x-reventless-semantic on the wire",
      () => {
        ...
        let semanticOf = fieldName =>
          getPropertyOf(json, fieldName)->Option.flatMap(
            s => getProperty(s, "x-reventless-semantic"),
          )
  ...
  describe("type-carried semantics on a command field:", () => {
    ...
    testSync(
      "a storage-ref command field emits the semantic id",
      () => {
        let schema = S.schema(
          s =>
            {
    ...
    testSync(
      "it carries the store identity and marks the type as its source",
      () => {
        let schema = S.schema(
          s =>
            {
    ...
    testSync(
      "the underlying string shape is unchanged",
      () => {
        ...
        let schema =
          S.schema(
            s => {"imageUrl": s.matches(Reventless.StorageRef.forStore(~store="productImages"))},
          )->S.castToUnknown
        let json = SuryToJsonSchema.deriveObjectSchema(schema)
        expect(
    ...
    testSync(
      "an optional storage-ref field keeps both its semantic and its nullability",
      () => {
        let schema = S.schema(
          s =>
            {
              "imageUrl": s.matches(
                S.option(Reventless.StorageRef.forStore(~store="productImages")),
              ),
            },
        )->S.castToUnknown
        let json = SuryToJsonSchema.deriveObjectSchema(schema)
        expect(semanticOf(json, "imageUrl"))->toEqual(Some(JSON.Encode.string("storageRef")))
        expect(
          getPropertyOf(json, "imageUrl")
          ->Option.flatMap(s => getProperty(s, "oneOf"))
          ->Option.isSome,
        )->toBe(true)
      },
    )
    ...
    testSync(
      "a type-carried semantic beats an annotation naming the same field",
      () => {
        let withSpec = (schema, spec) =>
          schema->S.Metadata.set(~id=Reventless.StateAnnotations.stateAnnotationsId, spec)
        let schema =
  ...
  describe("a member-reference field:", () => {
    let targetOf = (json, name) =>
      getPropertyOf(json, name)->Option.flatMap(s => getProperty(s, "x-reventless-semantic-target"))

    testSync(
      "carries the collection, the view holding it, and what a member is",
      () => {
        let json = SuryToJsonSchema.deriveObjectSchema(
          S.schema(
            s =>
        ...
        expect(
          getPropertyOf(json, "productImage")->Option.flatMap(
            s => getProperty(s, "x-reventless-semantic"),
          ),
    ...
    testSync(
      "omits the view where the collection is on this very record",
      () =>
        expect(
          targetOf(
            SuryToJsonSchema.deriveObjectSchema(
              S.schema(
                s => {"productImage": s.matches(Reventless.MemberRef.of_(~field="productImages"))},
              )->S.castToUnknown,
            ),
            "productImage",
          ),
          ...
          Some(
            JSON.Encode.object(Dict.fromArray([("field", JSON.Encode.string("productImages"))])),
          ),
        ),
    )
    ...
    testSync(
      "declares no store, so nothing is provisioned for it",
      () =>
        expect(
    ...
    testSync(
      "stays a plain string on the wire",
      () =>
        expect(
          getPropertyOf(
            SuryToJsonSchema.deriveObjectSchema(
  ...
  describe("branded scalars keep their underlying shape:", () => {
    ...
    testSync(
      "a string scalar stays a string and carries its id",
      () =>
        expect((
    ...
    testSync(
      "a numeric scalar stays a number and carries its id",
      () =>
        expect((
    ...
    testSync(
      "both name the type as their source",
      () =>
        expect((
  ...
  describe("Money changes the field's shape, and says so:", () => {
    ...
    testSync(
      "the field is an object, not a number",
      () => expect(keyOf("price", "type"))->toEqual(Some(JSON.Encode.string("object"))),
    )

    testSync(
      "it carries the money id, sourced from the type",
      () =>
        expect((
          keyOf("price", "x-reventless-semantic"),
          keyOf("price", "x-reventless-semantic-source"),
    ...
    testSync(
      "a plain numeric field emits no semantic",
      () =>
    ...
    testSync(
      "the amount and the currency both reach the wire",
      () => {
    ...
    testSync(
      "the currency is an enum of ISO codes",
      () => {
        let codes =
  ...
  describe("DateRange is an object whose parts stay date-times:", () => {
    ...
    testSync(
      "it carries the dateRange id, sourced from the type",
      () =>
        expect((
    ...
    testSync(
      "both instants keep their own date-time marker",
      () => {
    ...
    testSync(
      "a lone date-time field stays a string",
      () =>
  ...
  describe("GeoPoint is an object carrying the geoPoint marker:", () => {
    ...
    testSync(
      "the field is an object, not a pair of numbers",
      () => expect(keyOf("location", "type"))->toEqual(Some(JSON.Encode.string("object"))),
    )

    testSync(
      "it carries the geoPoint id, sourced from the type",
      () =>
        expect((
    ...
    testSync(
      "a bare lat float emits no semantic",
      () =>
    ...
    testSync(
      "both coordinates reach the wire as numbers",
      () => {
  ...
  describe("an optional field inside a nested record:", () => {
    ...
    testSync(
      "while its non-optional sibling still is",
  ...
  describe("a field declaring a default:", () => {
    ...
    testSync(
      "carries a string default the same way",
      () =>
        expect(defaultOf(json, ~field="shippingMethod"))->toEqual(
          Some(JSON.Encode.string("Standard")),
        ),
    )

    testSync(
      "while a field declaring none says nothing",
    ...
    testSync(
      "and reaches a field of a nested record",
      () => {
        let lineItem = S.schema(
          s =>
            {
              "productId": s.matches(S.string),
              "quantity": s.matches(S.int->Reventless.FieldDefault.int(1)),
