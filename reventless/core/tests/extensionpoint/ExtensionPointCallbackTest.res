...
describe("ExtensionPoint_Callback.handleIncomingCommands:", () => {
  describe("AbstractPublishCommand — known aggregate", () => {
    testPromise(
      "dispatches to aggregate publishJsons, returns Ok(reference)",
      async () => {
  ...
  describe("AbstractPublishCommand — unknown aggregate", () => {
    testPromise(
      "throws internally, returns Error(reference)",
      async () => {
  ...
  describe("AbstractHandleDirective handler succeeds", () => {
    testPromise(
      "handler called, returns Ok(reference)",
      async () => {
        let results = await Stream.fromIterable([
  ...
  describe("mixed actions", () => {
    testPromise(
      "AbstractPublishCommand and AbstractHandleDirective both resolved",
      async () => {
        let results = await Stream.fromIterable([
