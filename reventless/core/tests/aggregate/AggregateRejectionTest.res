...
describe("Aggregate decide-rejection propagation:", () => {
  describe("single rejected command", () => {
    testPromise(
      "Create on existing aggregate reports Rejected with errorCode",
      async () => {
  ...
  describe("mixed batch", () => {
    testPromise(
      "rejected command does not cancel surviving commands",
      async () => {
        // Seed.
        let _ = await Stream.fromIterable([
  ...
  describe("all-reject batch", () => {
    testPromise(
      "3 rejecting commands → no append, all reported Rejected, no Accepted",
      async () => {
        ...
        capturedRejections.contents->Array.forEach(
          ((_ref, info)) => expect(info.errorCode)->toEqual("NotFound"),
        )
  ...
  describe("payload-less variant errors", () => {
    testPromise(
      "errorDetail is empty string for unit-payload variants",
      async () => {
        // AggSpec.error variants are all payload-less (NotFound | AlreadyExists).
        installChannels()
        let _ = await Stream.fromIterable([makeTopicItem("ref-1", AggSpec.Rename({newName: "X"}))])
        ->TestHandler.handleCommands
        ->Effect.runPromise
        uninstallChannels()
