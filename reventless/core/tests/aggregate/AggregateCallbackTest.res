...
describe("Aggregate_Callback.handleCommands:", () => {
  describe("single command — new aggregate", () => {
    testPromise(
      "Create returns Ok(reference) and stores 1 event",
      async () => {
  ...
  describe("command on existing aggregate", () => {
    testPromise(
      "Rename after Create replays history and appends new event",
      async () => {
  ...
  describe("behavior returns no events", () => {
    testPromise(
      "Rename on new aggregate returns Ok with no event stored",
      async () => {
  ...
  describe("append failure", () => {
    testPromise(
      "eventLog.append fails → reference returned Error",
      async () => {
  ...
  describe("multiple commands for same aggregate ID", () => {
    testPromise(
      "Create then Rename in one batch accumulate state correctly",
      async () => {
        let results = await Stream.fromIterable([
  ...
  describe("commands for different aggregate IDs", () => {
    testPromise(
      "each ID processed independently",
      async () => {
        let results = await Stream.fromIterable([
