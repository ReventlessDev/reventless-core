...
describe("CommandTopic_Callback.handleJsonCommands:", () => {
  describe("valid command JSON", () => {
    testPromise(
      "decoded command passed to commandsHandler",
      async () => {
  ...
  describe("results returned from commandsHandler", () => {
    testPromise(
      "handler results propagated as return value",
      async () => {
        commandHandlerResults := [Ok("agg-1"), Error("agg-2")]
        let item = makeTopicItem("agg-1", TestSpec.CreateItem({itemId: "item-1"}))
        let results = await TestHandler.handleJsonCommands(
  ...
  describe("malformed command JSON", () => {
    testPromise(
      "skipped gracefully — commandsHandler called with empty array",
      async () => {
        let invalidItem: CommandTopic.topicItem<JSON.t> = {
          reference: "ref-1",
          command: JSON.Encode.string("not-a-command-object"),
        }
        let _ = await TestHandler.handleJsonCommands(
          Stream.fromIterable([invalidItem]),
        )->Effect.runPromise
  ...
  describe("multiple commands in batch", () => {
    testPromise(
      "all decodable commands reach commandsHandler",
      async () => {
        let items = [
  ...
  describe("mixed valid and invalid commands", () => {
    testPromise(
      "only valid commands reach commandsHandler",
      async () => {
