...
describe("the harvested model against the switch", () => {
  ...
  testSync("two observed targets leave the declaration to speak", () => {
    ...
    expect(
      edgeOf(
        [edge(~command="Book", ~allowedStates=["Booked"], ~targets=["Booked", "Abandoned"])],
        "Book",
      ),
...
describe("Plugin_Structure.make — Phase 2 graph fields", () => {
  describe("stateChangeSlices", () => {
    testSync(
      "produces two SCS entries in declaration order",
      () => {
        expect(structure.stateChangeSlices->Array.length)->toBe(2)
      },
    )
    ...
    testSync(
      "PlaceOrder: producedEventTypes contains OrderPlaced (qualified)",
      () => {
    ...
    testSync(
      "PlaceOrder: consumedEventTypes contains CatalogProductSynced qualified (payload-less variants excluded)",
      () => {
        let placeOrder = structure.stateChangeSlices->Array.getUnsafe(0)
    ...
    testSync(
      "PlaceOrder: linkedViews contains Orders (consumes its OrderPlaced)",
      () => {
        let placeOrder = structure.stateChangeSlices->Array.getUnsafe(0)
    ...
    testSync(
      "PlaceOrder: consistencyRead is AvailableProducts (only SVS consuming CatalogProductSynced)",
      () => {
        let placeOrder = structure.stateChangeSlices->Array.getUnsafe(0)
    ...
    testSync(
      "PlaceOrder command: level Collection (creation command), aggregateIdField orderId for UUID injection",
      () => {
        let placeOrder = structure.stateChangeSlices->Array.getUnsafe(0)
    ...
    testSync(
      "ShipOrder: producedEventTypes contains OrderShipped (qualified)",
      () => {
    ...
    testSync(
      "ShipOrder: consumedEventTypes is empty (all consumed events are payload-less literals)",
      () => {
        let shipOrder = structure.stateChangeSlices->Array.getUnsafe(1)
    ...
    testSync(
      "ShipOrder: linkedViews contains Orders (consumes its OrderShipped)",
      () => {
        let shipOrder = structure.stateChangeSlices->Array.getUnsafe(1)
    ...
    testSync(
      "ShipOrder: consistencyRead is None (empty consumedEventTypes cannot overlap any SVS)",
      () => {
        let shipOrder = structure.stateChangeSlices->Array.getUnsafe(1)
    ...
    testSync(
      "ShipOrder command: level Instance, aggregateIdField orderId",
      () => {
        let shipOrder = structure.stateChangeSlices->Array.getUnsafe(1)
    ...
    testSync(
      "ShipOrder: payload-less command CancelShipment is surfaced in commands",
      () => {
        let shipOrder = structure.stateChangeSlices->Array.getUnsafe(1)
    ...
    testSync(
      "ShipOrder: the @noApi CancelShipment is tagged apiExposed=false; ShipOrder stays exposed",
      () => {
        let shipOrder = structure.stateChangeSlices->Array.getUnsafe(1)
    ...
    testSync(
      "ShipOrder: the switch's target flows through to commandDef.targetState",
      () => {
    ...
    testSync(
      "ShipOrder: the switch's from-set flows through to commandDef.allowedStates",
      () => {
    ...
    testSync(
      "ShipOrder: the @noApi variant carries no callable mutation field (no sibling leak)",
      () => {
        ...
        let cancelField =
          byName("CancelShipment")->Option.map(c => c.mutationField)->Option.getOr("x")
    ...
    testSync(
      "ShipOrder: the command schema publishes each argument's GraphQL type",
      () => {
        let shipOrder = structure.stateChangeSlices->Array.getUnsafe(1)
    ...
    testSync(
      "CancelShipment: a @noApi variant publishes no argument types",
      () => {
        let shipOrder = structure.stateChangeSlices->Array.getUnsafe(1)
        let cancel =
    ...
    testSync(
      "ShipOrder: payload-less event ShipmentVoided is surfaced in events",
      () => {
        let shipOrder = structure.stateChangeSlices->Array.getUnsafe(1)
        expect(shipOrder.events->Array.map(e => e.name))->toEqual([
          "OrderShipped",
          "ShipmentVoided",
        ])
      },
    ...
    testSync(
      "ShipOrder: producedEventTypes still excludes the payload-less ShipmentVoided (DCB filter intact)",
      () => {
        let shipOrder = structure.stateChangeSlices->Array.getUnsafe(1)
        expect(shipOrder.producedEventTypes)->toEqual(["TestPlugin.OrderShipped"])
      },
    ...
    testSync(
      "PlaceOrder: the single payload-less error variant is surfaced",
      () => {
        let placeOrder = structure.stateChangeSlices->Array.getUnsafe(0)
    ...
    testSync(
      "ShipOrder: both error variants surface, in declaration order",
      () => {
        let shipOrder = structure.stateChangeSlices->Array.getUnsafe(1)
    ...
    testSync(
      "ShipOrder: a payload-carrying error carries its field schema",
      () => {
        let shipOrder = structure.stateChangeSlices->Array.getUnsafe(1)
        let notShippable =
    ...
    testSync(
      "ShipOrder: a payload-less error carries no fields and no references",
      () => {
        let shipOrder = structure.stateChangeSlices->Array.getUnsafe(1)
        let notFound =
          shipOrder.errors->Array.find(e => e.name == "OrderNotFound")->Option.getOrThrow
        expect((
          notFound.schema->String.includes("\"reason\""),
          notFound.references->Array.length,
        ))->toEqual((false, 0))
      },
    )
  })
  ...
  describe("stateViewSlices", () => {
    testSync(
      "produces four SVS entries in declaration order",
      () => {
    ...
    testSync(
      "OrdersView: consumedEventTypes contains the three order events (qualified)",
      () => {
    ...
    testSync(
      "OrdersView: linkedWriteSide contains both SCS producing order events",
      () => {
        let ordersView = structure.stateViewSlices->Array.getUnsafe(0)
    ...
    testSync(
      "AvailableProductsView: consumedEventTypes contains catalog events (qualified)",
      () => {
    ...
    testSync(
      "AvailableProductsView: linkedWriteSide is empty (no SCS produces these events)",
      () => {
        let apv = structure.stateViewSlices->Array.getUnsafe(1)
  ...
  describe("labelField / searchableFields", () => {
    testSync(
      "OrdersView: no @displayName → first non-`*Id` string field wins (customerName)",
      () => {
        let ordersView = structure.stateViewSlices->Array.getUnsafe(0)
    ...
    testSync(
      "AvailableProductsView: no @displayName → first non-`*Id` string field wins (name)",
      () => {
        let apv = structure.stateViewSlices->Array.getUnsafe(1)
    ...
    testSync(
      "Customers: composite @displayName → labelField=displayName, searchableFields=raw source fields in declaration order",
      () => {
  ...
  describe("labelField — the shape rule", () => {
    ...
    testSync(
      "a DateTime is not a name — a state whose only string is one falls to id",
      () => {
    ...
    testSync(
      "a DateTime is skipped, not fatal — a later plain string still wins",
      () => {
        let schema = S.schema(
          s =>
            {
    ...
    testSync(
      "an optional string is still the entity's name",
      () => {
        let schema = S.schema(
          s =>
            {
    ...
    testSync(
      "a reference is not a name even when its field is not called `*Id`",
      () => {
        let schema = S.schema(
          s =>
            {
    ...
    testSync(
      "a DCB-tagged string is not a name",
      () => {
        let schema = S.schema(
          s =>
            {
    ...
    testSync(
      "the `*Id` test is case-insensitive — `productID` is a reference",
      () => {
        let schema = S.schema(
          s =>
            {
    ...
    testSync(
      "a semantic-carrying string is a value, not a name",
      () => {
    ...
    testSync(
      "a state whose only string is a semantic falls to id",
      () => {
        let schema = S.schema(
          s =>
            {
    ...
    testSync(
      "`id` itself is still excluded by name",
      () => {
  ...
  describe("labelField — the conventional-name rung", () => {
    let labelOf = schema => {
      let r = Plugin_Structure.labelFieldsFromStateSchema(
        ~entityName="Test",
    ...
    testSync(
      "a field named `name` beats an earlier string",
      () => {
        let schema = S.schema(
          s =>
            {
    ...
    testSync(
      "`title` counts too, case-insensitively",
      () => {
        let schema = S.schema(
          s =>
            {
    ...
    testSync(
      "the match is exact — `customerName` is a customer's name, not this record's",
      () => {
    ...
    testSync(
      "an optional `name` still outranks an earlier string",
      () => {
        let schema = S.schema(
          s =>
            {
  ...
  describe("labelFieldSource — which rung answered", () => {
    ...
    testSync(
      "a @displayName spec is a declaration",
      () => {
        let customers = structure.stateViewSlices->Array.getUnsafe(2)
    ...
    testSync(
      "a field named `name` is a guess the consumer can also make",
      () => {
        let schema = S.schema(
          s =>
            {
    ...
    testSync(
      "declaration order is a guess only this side can make",
      () => {
    ...
    testSync(
      "no candidate at all is the state saying it has no human field",
      () => {
        let schema = S.schema(
          s =>
            {
    ...
    testSync(
      "every built def states one",
      () => {
        let stated =
          Array.concat(structure.readModels, structure.stateViewSlices)->Array.every(
            q => q.labelFieldSource->Option.isSome,
          )
  ...
  describe("singleQueryField — the singular the schema actually serves", () => {
    ...
    testSync(
      "an `-ies` plural publishes the `y` singular, not a truncation",
      () => {
    ...
    testSync(
      "every built def states the name Api_Naming returned",
      () => {
        let stated =
          structure.stateViewSlices->Array.every(
            q =>
              q.singleQueryField ==
                Some(
  ...
  describe("idField / idFieldSource — the key and its provenance", () => {
    ...
    testSync(
      "a name-matching `*Id` field is a guess the consumer can also make",
      () =>
    ...
    testSync(
      "a sole `*Id` field is a fact only this side can see",
      () =>
    ...
    testSync(
      "an `-ies` component name singularises to its key",
      () =>
    ...
    testSync(
      "a declared @id is used where inference would decline",
      () =>
  ...
  describe("lifecycleField — the name rule and the shape rule", () => {
    ...
    testSync(
      "a `lifecycle` field holding a closed set of values is the lifecycle field",
      () => {
        let schema = S.schema(
          s =>
            {
    ...
    testSync(
      "an optional one counts too",
      () => {
        let schema = S.schema(
          s =>
            {
              "lifecycle": s.matches(
                S.option(S.union([S.literal("Placed"), S.literal("Shipped")])),
              ),
            },
        )
        expect(lifecycleOf(schema))->toEqual(Some("lifecycle"))
    ...
    testSync(
      "free text named `lifecycle` is not a lifecycle",
      () => {
    ...
    testSync(
      "an unannotated field named `status` resolves to None",
      () => {
        let schema = S.schema(
          s =>
            {
    ...
    testSync(
      "the annotation names a field the convention would never reach",
      () => {
        let schema = S.schema(
          s =>
            {
  ...
  describe("retiredField — annotation and nothing else", () => {
    let retiredOf = schema => Plugin_Structure.retiredFieldFromStateSchema(schema->S.castToUnknown)
    ...
    testSync(
      "publishes the retirement state beside the field",
      () => {
    ...
    testSync(
      "publishes no states on the boolean form",
      () => {
    ...
    testSync(
      "declines a conventionally-named boolean nobody annotated",
      () => {
        let schema = S.schema(
          s =>
            {
    ...
    testSync(
      "declines a schema carrying no annotation spec at all",
      () => {
  ...
  describe("@retired state names are checked against the field's own enum", () => {
    ...
    testSync(
      "each undeclared state is reported on its own",
      () => {
    ...
    testSync(
      "a field with no cases to compare against says so instead of passing",
      () => {
        let schema =
    ...
    testSync(
      "the boolean form is not a skip",
      () => {
        let schema =
    ...
    testSync(
      "a state on a non-lifecycle field is not a failure",
      () => {
        let schema = S.schema(
          s =>
            {
  ...
  describe("queryableDef.schema propagates x-reventless-* annotations", () => {
    ...
    testSync(
      "itemId carries x-reventless-id",
      () => {
        expect(
          annotatedSchema
    ...
    testSync(
      "version carries x-reventless-subId",
      () => {
        expect(
          annotatedSchema
    ...
    testSync(
      "ownerId carries x-reventless-index = byOwner",
      () => {
        expect(
          annotatedSchema
    ...
    testSync(
      "@semantic(\"currency\") flows through the PPX to x-reventless-semantic",
      () => {
        expect(
          annotatedSchema
          ->getPropertyOf("total")
    ...
    testSync(
      "@sensitive flows through the PPX to x-reventless-sensitive",
      () => {
        expect(
          annotatedSchema
    ...
    testSync(
      "@sensitive reaches an event variant's payload, not only state",
      () =>
        expect(
    ...
    testSync(
      "@default flows through the PPX to JSON Schema's `default`",
      () => {
        expect(
          annotatedSchema
    ...
    testSync(
      "@default leaves the field's own shape alone",
      () => {
        expect(
          annotatedSchema
          ->getPropertyOf("quantity")
    ...
    testSync(
      "@metric flows through the PPX to x-reventless-metric {aggregate,label}",
      () => {
        let metricObj =
          annotatedSchema
          ->getPropertyOf("total")
        ...
        expect((
          metricObj
          ->Option.flatMap(m => getProperty(m, "aggregate"))
          ->Option.flatMap(JSON.Decode.string),
          metricObj
          ->Option.flatMap(m => getProperty(m, "label"))
          ->Option.flatMap(JSON.Decode.string),
    ...
    testSync(
      "unannotated field 'name' has no x-reventless-* keys",
      () => {
    ...
    testSync(
      "unannotated state-view slice (Orders) emits no x-reventless-* keys",
      () => {
        ...
        expect(orderIdField->Option.flatMap(s => getProperty(s, "x-reventless-id")))->toBe(None)
      },
    )

    testSync(
      "@live(false) flows through the PPX to top-level x-reventless-live",
      () => {
        expect(
    ...
    testSync(
      "unannotated state-view slice (Orders) has no top-level x-reventless-live",
      () => {
        let ordersSchema = structure.stateViewSlices->Array.getUnsafe(0)->parseSchema
  ...
  describe("componentChapters threading", () => {
    ...
    testSync(
      "a chaptered state-change slice carries Some(chapter)",
      () => {
    ...
    testSync(
      "a chaptered state-view slice carries Some(chapter)",
      () => {
    ...
    testSync(
      "a component absent from the map carries None",
      () => {
    ...
    testSync(
      "no ~componentChapters → every def is chapterless (None)",
      () => {
        let placeOrder = structure.stateChangeSlices->Array.getUnsafe(0)
  ...
  describe("requiredStores", () => {
    ...
    testSync(
      "an unqualified store resolves against the declaring plugin",
      () => expect(stores->Array.includes("TestPlugin.documents"))->toBe(true),
    )

    testSync(
      "a qualified store keeps its own plugin",
      () => expect(stores->Array.includes("branding.logos"))->toBe(true),
    )

    // `documents` is declared on both a command field and an event field.
    testSync(
      "a store declared by several fields is collected once",
      () => expect(stores->Array.filter(s => s == "TestPlugin.documents")->Array.length)->toBe(1),
    )

    testSync("only the declared stores are collected", () => expect(stores->Array.length)->toBe(2))

    testSync(
      "a plugin declaring no stores collects none — not None",
      () => expect(structure.requiredStores)->toEqual(Some([])),
    )

    testSync(
      "each requirement keeps its declaring (component, field) sites",
      () => {
    ...
    describe(
      ...
      () => {
        ...
        testSync(
          "keys are byte-identical to requiredStores",
          () =>
        ...
        testSync(
          ...
          () => {
            let entry =
              ...
              ->Array.find(c => c.key == "TestPlugin.documents")
              ->Option.getOrThrow
        ...
        testSync(
          "a structure declaring nothing yields an empty list, not a missing manifest",
          () =>
            expect(
              Reventless.CapabilityManifest.fromStructure(structure).capabilities,
            )->toEqual([]),
        )
        ...
        describe(
          ...
          () => {
            ...
            testSync(
              "two components needing one capability yield one entry",
              () => expect(entries->Array.map(e => e.key))->toEqual(["Geocoding"]),
            )

            testSync(
              "both declaring components are kept, with no field",
              () =>
            ...
            testSync(
              "a capability this build does not know is dropped",
              () =>
                expect(
        ...
        testSync(
          "rendering is deterministic and newline-terminated",
          () => {
...
`)
  ...
  describe("uploadable types derive their store from the field name", () => {
    ...
    testSync(
      "the derived store is the field name, pluralised",
      () => expect(stores->Array.includes("TestPlugin.productImages"))->toBe(true),
    )

    testSync(
      "an optional field still declares its store",
    ...
    testSync(
      "an array field declares through its element",
      () => expect(stores->Array.includes("TestPlugin.datasheets"))->toBe(true),
    )

    testSync(
      "an explicit @storageRef overrides the derived name",
      () => expect(stores->Array.includes("branding.logos"))->toBe(true),
    )
    ...
    testSync(
      "each requirement keeps its declaring sites",
      () => {
        let expected: array<Reventless.Plugin.requiredStoreDeclaration> = [
          {
    ...
    let commandJson =
      (
        (withUploads.stateChangeSlices->Array.getUnsafe(0)).commands->Array.getUnsafe(0)
      ).schema->JSON.parseOrThrow
    ...
    testSync(
      "the wire carries the content half, not just the store",
    ...
    testSync(
      "an overridden field keeps its uploadable semantic",
      () => expect(semanticOf("logo"))->toBe(Some("uploadableImage")),
    )

    testSync(
      "the non-image arm carries its own id, on the element",
      () =>
        expect(
          commandJson
  ...
  describe("near-duplicate stores are refused", () => {
    ...
    testSync(
      "two stores one transposition apart fail the build",
      () => expect(failureText() != "")->toBe(true),
    )

    testSync(
      "the failure names both stores and both fields",
      () => {
    ...
    testSync(
      "the four stores of the uploadable fixture do not collide",
      () =>
        expect(
    ...
    testSync(
      "one edit, transpositions included; two edits not",
      () =>
        expect(
          [
    ...
    testSync(
      "a singular/plural pair collides even at two edits apart",
  ...
  describe("optional carriers keep their declarations", () => {
    ...
    testSync(
      "an optional storage ref still requires its store",
    ...
    testSync(
      "and still carries one declaring site",
      () => {
        let expected: array<Reventless.Plugin.requiredStoreDeclaration> = [
          {
    ...
    testSync(
      "an optional reference is still collected",
      () => {
    ...
    testSync(
      "the command schema carries the field's storage-ref marker",
      () => {
        let cmd = (withOptional.stateChangeSlices->Array.getUnsafe(0)).commands->Array.getUnsafe(0)
        let target =
          cmd.schema
    ...
    testSync(
      "an optional command argument is not required",
      () => {
        let cmd = (withOptional.stateChangeSlices->Array.getUnsafe(0)).commands->Array.getUnsafe(0)
        let required =
          cmd.schema
  ...
  describe("capability inference", () => {
    ...
    testSync(
      "a heuristic-only match produces a warning",
      () => {
    ...
    testSync(
      "a declared field with a heuristic name does not warn",
      () => {
    ...
    testSync(
      "a heuristic-only match reaches no manifest entry",
      () => {
        ...
        expect(
          manifest.capabilities->Array.every(
            entry => entry.declaredBy->Array.every(site => site.field != Some("photoUrl")),
          ),
        )->toBe(true)
      },
    )
    ...
    testSync(
      "the warning names the field and the settling annotation",
      () => {
        let text = commandWarnings->Array.getUnsafe(0)->Capability_Inference.message
    ...
    testSync(
      "suffix and exact name rules match; ordinary names do not",
      () => {
        expect(
  ...
  describe("plural references reach the manifest", () => {
    ...
    // that holds it.
    testSync(
      "a command collects its array refs alongside its scalar one",
      () => {
    ...
    testSync(
      "a reference on a nested record's field is named by its path",
      () => {
        let cmd = slice.commands->Array.getUnsafe(0)
        expect(
    ...
    testSync(
      "a flat reference in the same variant keeps its bare name",
      () => {
        let cmd = slice.commands->Array.getUnsafe(0)
    ...
    testSync(
      "an optional array ref is collected under the field's own name",
      () => {
        let cmd = slice.commands->Array.getUnsafe(0)
        expect(cmd.references->Array.find(({fieldName}) => fieldName == "warehouseIds"))->toEqual(
          Some({Reventless.Plugin.fieldName: "warehouseIds", entity: "Warehouses", plugin: None}),
        )
      },
    )
    ...
    testSync(
      "an event collects its array ref too",
      () => {
    ...
    testSync(
      "a nested input object publishes the type name the SDL declares",
      () => {
        let cmd = slice.commands->Array.getUnsafe(0)
        let itemName =
          cmd.schema
    ...
    testSync(
      "the declared entity wins over the one the field name suggests",
      () => {
        let cmd = slice.commands->Array.getUnsafe(0)
        expect(
          cmd.references
...
describe("the port's translation table", () => {
  ...
  testSync("accepts a many-to-one declaration", () =>
    expect(
      failures(
        ~declared=[published("ProductWithdrawn", ["Archived", "Discontinued"])],
      )->Array.length,
    )->toBe(0)
  )

  testSync("rejects a published name the extension point does not declare", () =>
    expect(failures(~declared=[published("ProductRetired", ["Archived"])])->Array.length)->toBe(1)
  )

  testSync("rejects a source name the delegate does not declare", () =>
    expect(failures(~declared=[published("ProductWithdrawn", ["Archivd"])])->Array.length)->toBe(1)
  )
})
...
describe("the translation table against the probe", () => {
  ...
  testSync("fails on an edge the probe saw and the declaration omits", () => {
    let (failures, _) = drift(~observed=[("Archived", "ProductRelisted")], ~followed=["Archived"])
...
describe("the subscriber's half of the table", () => {
  ...
  testSync("rejects a command neither side declares", () =>
    expect(failures([handled("ProductBecameAvailable", ["SyncNewProdct"])])->Array.length)->toBe(1)
  )
})
