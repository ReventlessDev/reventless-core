...
describe("SchemaWalker:", () => {
  describe("record schema", () => {
    ...
    testSync(
      "required int field typeDescription is number",
      () => {
  ...
  describe("variant schema", () => {
    ...
    testSync(
      "payload-less constructor Removed is excluded",
      () => {
    ...
    testSync(
      "Added constructor has sorted fields",
      () => {
        let ctors = result.constructors->Option.getOr([])
        let added =
          ctors
          ->Array.find(c => c.name == "Added")
          ->Option.getOr({Plugin_BuiltHook.name: "", fields: []})
        expect(added.fields->Array.map(f => f.name))->toEqual(["id", "name"])
      },
    )

    testSync(
      "Renamed constructor has sorted fields",
      () => {
        let ctors = result.constructors->Option.getOr([])
        let renamed =
          ctors
          ->Array.find(c => c.name == "Renamed")
          ->Option.getOr({Plugin_BuiltHook.name: "", fields: []})
  ...
  describe("hash properties", () => {
    testSync(
      "same schema produces identical hash on repeated calls",
      () => {
    ...
    testSync(
      "typeName does not affect structuralHash",
      () => {
    ...
    testSync(
      "structurally distinct schemas produce different hashes",
      () => {
  ...
  describe("scalar / unsupported schema", () => {
    testSync(
      "plain string schema → kind unknown",
      () => {
    ...
    testSync(
      "plain string schema → empty fields",
      () => {
        let result = SchemaWalker.walk("Scalar", S.string)
    ...
    testSync(
      "plain string schema → empty structuralHash",
      () => {
        let result = SchemaWalker.walk("Scalar", S.string)
