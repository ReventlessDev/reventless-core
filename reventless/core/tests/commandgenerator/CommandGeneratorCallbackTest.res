...
describe("CommandGenerator_Callback.generateCommand:", () => {
  describe("zero-param command", () => {
    testPromise(
      "publishJsons called with plain string commandJson",
      async () => {
    ...
    testPromise(
      "returns a Pending outcome with non-empty msgId",
      async () => {
        let payload = makeZeroParamPayload(~id="agg-1", ~command="Create")
    ...
    testPromise(
      "meta.service equals AggregateSpec.name",
      async () => {
        let payload = makeZeroParamPayload(~id="agg-1", ~command="Create")
    ...
    testPromise(
      "meta.msgId equals meta.correlationId",
      async () => {
        let payload = makeZeroParamPayload(~id="agg-1", ~command="Create")
        let outcome = await TestGenerator.generateCommand(payload)->Effect.runPromise
        let publishedCmd = capturedCmds.contents->Array.getUnsafe(0)
  ...
  describe("multi-param command", () => {
    testPromise(
      "publishJsons called with object commandJson {TAG, param}",
      async () => {
        let payload = makeOneParamPayload(
          ~id="agg-1",
          ~command="CreateWithName",
          ~paramName="name",
          ~paramValue="Widget",
        )
        let _msgId = await TestGenerator.generateCommand(payload)->Effect.runPromise
  ...
  describe("schema validation failure", () => {
    testPromise(
      "invalid command name throws an error",
      async () => {
  ...
  describe("StateChangeSlice envelope id derivation", () => {
    ...
    testPromise(
      "composite @compositePartitionTag: id is joined partition-key value",
      async () => {
        let payload = makeSlicePayload(
    ...
    testPromise(
      "explicit id in args wins over derivation (regression check)",
      async () => {
        let payload = makeSlicePayload(
          ~command="CreateItem",
          ~args=Dict.fromArray([
  ...
  describe("Aggregate envelope id (regression)", () => {
    testPromise(
      "explicit id in args is preserved unchanged",
      async () => {
  ...
  describe("an argument sent as null", () => {
    ...
    testPromise(
      "is dropped, and the command decodes and publishes",
      async () => {
        ...
        let publishedCmd = capturedCmds.contents->Array.getUnsafe(0)
        expect((capturedCmds.contents->Array.length, publishedCmd->fieldOf("note")))->toEqual((
          1,
          None,
        ))
      },
    ...
    testPromise(
      "carrying a value is left exactly as sent",
      async () => {
        let payload = storeItem(
          ~args=Dict.fromArray([
        ...
        let _outcome = await optionalFieldSliceGen(payload)->Effect.runPromise
        expect(capturedCmds.contents->Array.getUnsafe(0)->fieldOf("note"))->toEqual(
          Some(JSON.Encode.string("gift wrap")),
        )
      },
    )
    ...
    testPromise(
      "and one simply omitted produce the same command",
      async () => {
        ...
        expect((published->Array.getUnsafe(0)).commandJson)->toEqual(
          (published->Array.getUnsafe(1)).commandJson,
        )
      },
    )
    ...
    // nulls are data.
    testPromise(
      "nested inside a value is left alone",
      async () => {
  ...
  describe("a payload that cannot be decoded", () => {
    ...
    testPromise(
      "is marked as the caller's fault, naming the field",
      async () => {
