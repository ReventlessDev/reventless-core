...
describe("EventLog_Operations:", () => {
  describe("append", () => {
    testPromise(
      "stores encoded events in storage",
      async () => {
    ...
    testPromise(
      "publishes events to event topic",
      async () => {
        let event = ItemEventLogSpec.ItemCreated({name: "Widget"})
        let event' = makeEvent'("item-1", event)
        let _ = await Ops.append(1, "item-1", [event'])
    ...
    testPromise(
      "storage failure returns Error result",
      async () => {
        failNextAppend := true
        let event = ItemEventLogSpec.ItemCreated({name: "Widget"})
        let event' = makeEvent'("item-1", event)
  ...
  describe("replay", () => {
    testPromise(
      "returns empty array for unknown id",
      async () => {
    ...
    testPromise(
      "decodes stored events back to typed events",
      async () => {
        let event = ItemEventLogSpec.ItemCreated({name: "Widget"})
        let event' = makeEvent'("item-1", event)
        let _ = await Ops.append(1, "item-1", [event'])
    ...
    testPromise(
      "decodes multiple events in order",
      async () => {
        let events = [
    ...
    testPromise(
      "separate aggregates have independent event logs",
      async () => {
        let _ = await Ops.append(
          1,
          "agg-A",
          [makeEvent'("agg-A", ItemEventLogSpec.ItemCreated({name: "A"}))],
        )
        let _ = await Ops.append(
          1,
          "agg-B",
          [makeEvent'("agg-B", ItemEventLogSpec.ItemDeleted({id: "B"}))],
        )
