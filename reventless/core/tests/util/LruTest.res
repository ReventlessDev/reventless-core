...
describe("Lru:", () => {
  ...
  describe("eviction", () => {
    testSync(
      ...
      () => {
        ...
        expect((
          ...
          cache->Lru.get("c"),
          cache->Lru.size,
        ))->toEqual((None, Some(2), Some(3), 2))
      },
    )

    testSync(
      "get refreshes recency so a touched key survives eviction",
      () => {
        let cache = Lru.make(~capacity=2)
        cache->Lru.put("a", 1)
        cache->Lru.put("b", 2)
    ...
    testSync(
      "101 distinct keys against capacity 100 keeps exactly 100",
      () => {
        ...
        expect((cache->Lru.size, cache->Lru.get(0), cache->Lru.get(100)))->toEqual((
          100,
          None,
          Some(100),
        ))
