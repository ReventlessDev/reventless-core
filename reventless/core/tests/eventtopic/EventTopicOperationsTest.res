...
describe("EventTopic_Operations:", () => {
  describe("publish", () => {
    testPromise(
      "calls publishJson for each event",
      async () => {
        let events = [
    ...
    testPromise(
      "calls publishJson with correct service (id string)",
      async () => {
    ...
    testPromise(
      "publishes empty array without error",
      async () => {
        await Ops.publish([])
    ...
    testPromise(
      "encodes event with correct JSON structure",
      async () => {
        let event' = makeEvent'("item-1", ItemEventTopicSpec.ItemPublished({name: "Widget"}))
        await Ops.publish([event'])
