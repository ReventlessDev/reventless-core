...
describe("MakeCounterHandler.handleCounterEvents:", () => {
  describe("event with matching source mapping", () => {
    testPromise(
      "publishes command JSON for OrderPlaced",
      async () => {
        ...
        await TestCounterHandler.handleCounterEvents(
          Stream.fromIterable([eventJson]),
        )->Effect.runPromise
  ...
  describe("invalid event JSON", () => {
    testPromise(
      "skipped gracefully — no throw, no publish",
      async () => {
        let invalidJson = JSON.Encode.string("not-an-object")
        await TestCounterHandler.handleCounterEvents(
          Stream.fromIterable([invalidJson]),
        )->Effect.runPromise
        expect(capturedCmds.contents->Array.length)->toBe(0)
      },
    )
...
describe("MakeEventCollectorHandler.handleJsonEvents:", () => {
  describe("Count action", () => {
    testPromise(
      "calls count with the countItem",
      async () => {
        ...
        mockCommonHandler := (async _ => (Promise.resolve([]), [Counter.Count(countItem)]))
        await TestECHandler.handleJsonEvents(
          Stream.fromIterable([JSON.Encode.null]),
        )->Effect.runPromise
  ...
  describe("AddToCounterTarget action", () => {
    testPromise(
      "calls addToCounterTarget with the counterTargetRef",
      async () => {
  ...
  describe("Publisher action", () => {
    testPromise(
      "calls publishJsons with the command",
      async () => {
  ...
  describe("mixed actions", () => {
    testPromise(
      ...
      async () => {
        ...
        mockCommonHandler := (async _ => (Promise.resolve([cmd]), [Counter.Count(countItem)]))
        await TestECHandler.handleJsonEvents(
          Stream.fromIterable([JSON.Encode.null]),
        )->Effect.runPromise
        expect((
  ...
  describe("doCount retry", () => {
    testPromise(
      "retries when count throws, eventually succeeds",
      async () => {
        let countItem: ReventlessInfra.Counter.countItem = {
          counterId: "c1",
          reference: "ref-1",
