...
describe("Mapping.Make:", () => {
  ...
  describe("project", () => {
    testSync(
      "ItemCreated maps to Create action",
      () => {
        let event' = makeSourceEvent'(
          "item-1",
          SourceSpec.ItemCreated({name: "Widget", price: 9.99}),
        )
    ...
    testSync(
      "ItemPriceUpdated maps to Update action",
      () => {
    ...
    testSync(
      "ItemRemoved maps to Delete action",
      () => {
    ...
    testSync(
      "Update action applies function correctly",
      () => {
        let event' = makeSourceEvent'("item-1", SourceSpec.ItemPriceUpdated({newPrice: 14.99}))
        let action = ItemMapping.project(event')
        switch action {
