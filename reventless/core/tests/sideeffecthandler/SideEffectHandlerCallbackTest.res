...
describe("SideEffectHandler_Callback.handleJsonEvents:", () => {
  describe("event matching registered side effect", () => {
    testPromise(
      "execute called with decoded id and event",
      async () => {
        let event = TestSource.SomethingHappened({value: "hello"})
        let eventJson = makeEventJson("entity-1", event)
        await TestHandler.handleJsonEvents(Stream.fromIterable([eventJson]))->Effect.runPromise
  ...
  describe("event with malformed JSON", () => {
    testPromise(
      "caught gracefully — does not throw, execute not called",
      async () => {
        let invalidJson = JSON.Encode.string("not-an-object")
        await TestHandler.handleJsonEvents(Stream.fromIterable([invalidJson]))->Effect.runPromise
        expect(capturedExecuteCalls.contents->Array.length)->toBe(0)
      },
    )
  })
  ...
  describe("execute throws", () => {
    testPromise(
      "error caught per-event, other events still processed",
      async () => {
        ...
        await TestHandler.handleJsonEvents(Stream.fromIterable([event1, event2]))->Effect.runPromise
  ...
  describe("multiple events in batch", () => {
    testPromise(
      ...
      async () => {
        ...
        await TestHandler.handleJsonEvents(Stream.fromIterable(events))->Effect.runPromise
