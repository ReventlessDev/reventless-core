...
function fromJson(json) {
  try {
    return {
      TAG: "Ok",
      _0: Sury.parseOrThrow(json, Task$ReventlessInterop.resolvedOutputsSchema)
    };
  } catch (raw_exn) {
...
function fromJson$1(json) {
  try {
    return {
      TAG: "Ok",
      _0: Sury.parseOrThrow(json, EventMapper$ReventlessInterop.resolvedOutputsSchema)
    };
  } catch (raw_exn) {
    let exn = Primitive_exceptions.internalToException(raw_exn);
    let msg = Stdlib_Option.getOr(Stdlib_Option.flatMap(Stdlib_JsExn.fromException(exn), Stdlib_JsExn.message), "parse error");
...
function fromJson$2(json) {
  try {
    return {
      TAG: "Ok",
      _0: Sury.parseOrThrow(json, ExtensionPoint$ReventlessInterop.resolvedOutputsSchema)
    };
  } catch (raw_exn) {
    let exn = Primitive_exceptions.internalToException(raw_exn);
    let msg = Stdlib_Option.getOr(Stdlib_Option.flatMap(Stdlib_JsExn.fromException(exn), Stdlib_JsExn.message), "parse error");
