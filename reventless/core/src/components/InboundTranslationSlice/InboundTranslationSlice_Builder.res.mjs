// Generated by ReScript, PLEASE EDIT WITH CARE

import * as Id$Reventless from "@reventlessdev/reventless-spec/src/types/Id.res.mjs";
import * as Pulumi from "@pulumi/pulumi";
import * as ReadModel$Reventless from "@reventlessdev/reventless-spec/src/components/ReadModel.res.mjs";
import * as Component$ReventlessCore from "../Component.res.mjs";
import * as ComponentType$ReventlessCore from "../../ComponentType.res.mjs";
import * as QueryDb_Builder$ReventlessCore from "../QueryDb/QueryDb_Builder.res.mjs";
import * as InboundTranslationSlice$ReventlessCore from "./InboundTranslationSlice.res.mjs";
import * as InboundTranslationSlice_Callback$ReventlessCore from "./InboundTranslationSlice_Callback.res.mjs";

function Make(QueryDbStorage) {
  return QueryDbResolvers => (Api => {
    let Make = Spec => (Translation => {
      let Callback = InboundTranslationSlice_Callback$ReventlessCore.Make(Spec)(Translation);
      let queryDbName = Spec.name + "Audit";
      let moduleUrl = import.meta.url;
      let config = ReadModel$Reventless.config(undefined, undefined, undefined);
      let SpecificQueryDb = QueryDb_Builder$ReventlessCore.Make({
        Id: Id$Reventless.$$String,
        name: queryDbName,
        moduleUrl: moduleUrl,
        stateSchema: InboundTranslationSlice_Callback$ReventlessCore.auditRowSchema,
        config: config,
        subIdConfig: undefined,
        authorization: "AllowAuthenticated",
        visibility: "Public"
      })(QueryDbStorage)(QueryDbResolvers);
      let syncToQueryDb = async (queryDbOps, requestId) => {
        let row = InboundTranslationSlice_Callback$ReventlessCore.takeAuditRow(Callback.auditLog, requestId);
        if (row !== undefined) {
          await queryDbOps.save(Id$Reventless.$$String.makeFromString(requestId), row, "Overwrite", undefined);
          return;
        }
      };
      let make = (publishJsons, param, opts) => Component$ReventlessCore.make(ComponentType$ReventlessCore.toString(InboundTranslationSlice$ReventlessCore.componentType), Spec.name, (extra, extra$1) => {
        let opts_parent = Component$ReventlessCore.toPulumiResource(extra);
        let opts = {
          parent: opts_parent
        };
        let queryDb = SpecificQueryDb.make(Api.api(), Api.apiRole(), undefined, {
          kind: "InboundTranslationSlice",
          name: Spec.name
        }, opts);
        let publishJsonsRef = {
          contents: undefined
        };
        publishJsons.apply(pj => {
          publishJsonsRef.contents = pj;
        });
        Component$ReventlessCore.setOperations(extra, Pulumi.all([
          Component$ReventlessCore.operations(queryDb),
          publishJsons
        ]).apply(param => {
          let publishJsonsFn = param[1];
          let queryDbOps = param[0];
          return {
            receive: async inputJson => {
              let result = await Callback.receive(publishJsonsFn, inputJson);
              await syncToQueryDb(queryDbOps, InboundTranslationSlice_Callback$ReventlessCore.requestIdOf(result));
              return result;
            }
          };
        }));
        let outputs_resources = [];
        let outputs_queryDb = Component$ReventlessCore.outputs(queryDb);
        let outputs = {
          resources: outputs_resources,
          queryDb: outputs_queryDb
        };
        return Component$ReventlessCore.setOutputs(extra, outputs);
      }, opts);
      return {
        Spec: Spec,
        Translation: Translation,
        queryDbName: queryDbName,
        make: make
      };
    });
    return {
      Make: Make
    };
  });
}

export {
  Make,
}
/* Id-Reventless Not a pure module */
