      ...
      return Component$ReventlessCore.make(ComponentType$ReventlessCore.toString(EventMapper$ReventlessCore.componentType), name, (extra, extra$1) => {
        ...
        let eventCollector = match[0].apply(param => {
          ...
          let publishJsonsDetached = jsons => {
            Stdlib_Promise.$$catch(publishJsons(jsons), exn => {
              log.error("EventMapper", undefined, `detached publish error: ` + Stdlib_Option.getOr(Stdlib_Option.flatMap(Stdlib_JsExn.fromException(exn), Stdlib_JsExn.message), "unknown"));
              return Promise.resolve();
            });
            return Promise.resolve();
          };
          let EventCollectorHandler = EventMapper_Callback$ReventlessCore.MakeEventCollectorHandler({
            publishJsons: publishJsonsDetached,
            count: param.count,
