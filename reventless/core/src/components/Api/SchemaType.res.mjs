...
function isIdentity(schema) {
  let match = Sury.$Metadata_get(schema, Semantic$Reventless.semanticId);
  if (match !== undefined) {
    return match.id === Semantic$Reventless.Id.identity;
  } else {
    return false;
  }
}
...
function shapeOf(parentName, fieldName, schema) {
  if (DcbTag$Reventless.isTagged(schema) || Stdlib_Option.isSome(Reference$Reventless.getTarget(schema)) || isIdentity(schema)) {
    return "EntityId";
  }
  switch (schema.type) {
    case "string" :
