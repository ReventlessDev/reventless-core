...
function Make(Spec) {
  return Channel => {
    ...
    let filteringHandler = stream => Effect.map(Stream.runCollect(Stream$1.mapEffect(stream, item => {
      ...
      return Effect.promise(async () => {
        ...
        await Promise.all(handlers.map(async handlerEntry => {
          try {
            let results = await Effect.runPromise(handlerEntry.handler(Stream$1.fromIterable([{
                command: json,
                reference: reference
              }])));
