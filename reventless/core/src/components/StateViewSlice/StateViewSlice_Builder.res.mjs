        ...
        let eventCollector = Component$ReventlessCore.operations(queryDb).apply(queryDbOps => {
          ...
          let jsonEventsHandler = stream => Effect.flatMap(Stream.runCollect(stream), events => {
            ...
            return Stdlib_Array.reduce(events.map((json, i) => {
              let envelopeDict = Stdlib_Option.getOr(Stdlib_JSON.Decode.object(json), {});
              let rawEvent = Stdlib_Option.getOr(envelopeDict["event"], json);
              let m = envelopeDict["meta"];
              let meta;
              if (m !== undefined) {
                try {
                  meta = Util_Sury$Reventless.fromJson(m, Message$ReventlessCore.metaSchema);
                } catch (exn) {
                  meta = Message$ReventlessCore.generateMeta(Spec.name, undefined, undefined, undefined, undefined, undefined, undefined, undefined);
                }
              } else {
                meta = Message$ReventlessCore.generateMeta(Spec.name, undefined, undefined, undefined, undefined, undefined, undefined, undefined);
              }
              let recordedAt = Stdlib_Option.getOr(Stdlib_Option.flatMap(envelopeDict["recordedAt"], Stdlib_JSON.Decode.string), "");
