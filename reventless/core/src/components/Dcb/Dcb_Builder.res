...
module Make = (
  ...
  let construct = (
    ...
    if hasDcb {
      ...
      let syncSlices =
        stateChangeSlices->Array.filter((module(M: StateChangeSlice.T)) => !M.isAsync)
      let asyncSlices =
        stateChangeSlices->Array.filter((module(M: StateChangeSlice.T)) => M.isAsync)
      ...
      let consumed =
        ...
        ->Array.concat(
          stateViewSlices->Array.map((module(V: StateViewSlice.T)) => (
            V.Spec.name,
            V.Spec.consumedEventSchema->S.castToUnknown,
          )),
        )
        ->Array.concat(
          ...
          automationSlices->Array.flatMap((module(A: AutomationSlice.T)) =>
            A.Automation.mappings->Array.map((module(M: A.Automation.Mapping)) => (
              A.Spec.name,
              M.sourceEventSchema->S.castToUnknown,
            ))
          ),
        )
        ->Array.concat(
          outboundTranslationSlices->Array.map((module(O: OutboundTranslationSlice.T)) => (
            O.Spec.name,
            O.Spec.consumedEventSchema->S.castToUnknown,
          )),
        )
      ...
      inferred.ambiguities->Array.forEach(((sliceName, reason)) =>
        log.warn(~comp="Dcb_Builder", `DCB scope-inference ambiguity (${sliceName}): ${reason}`)
      )
      ...
      HooksConfig.hooks.onDcbEventLogCreated->Option.forEach(hook => hook(dcbEventLog->Obj.magic))
      ...
      let asyncStateChangeSlicesOutputs = switch asyncDcbCommandTopicOpt {
      | None => Dict.make()
      ...
      | Some(registerResolver) =>
        syncSlices->Array.forEach((module(S: StateChangeSlice.T)) => {
        ...
        asyncSlices->Array.forEach((module(S: StateChangeSlice.T)) => {
          let commandSchema = S.Spec.commandSchema->Reventless.DcbTag.toUnknownSchema
          if !ApiNoApiHelpers.isNoApi(commandSchema) {
            registerResolver(
              ~kind=Dcb,
            ...
            syncSlices->Array.forEach((module(S: StateChangeSlice.T)) => {
              ...
              if !ApiNoApiHelpers.isNoApi(commandSchema) {
                ...
                Api_Naming.sliceMutationFields(
                  ...
                  ((fieldName, _)) => bindHandler(~field=fieldName, ~generateCommand),
                )
              }
            })
          })
      ...
      stateViewSlices->Array.forEach((module(V: StateViewSlice.T)) => {
      ...
      automationSlices->Array.forEach((module(A: AutomationSlice.T)) => {
      ...
      outboundTranslationSlices->Array.forEach((module(O: OutboundTranslationSlice.T)) => {
      ...
      inboundTranslationSlices->Array.forEach((module(I: InboundTranslationSlice.T)) => {
      ...
      allEventTopics->Dict.set(name ++ "DcbEventLog", (dcbEventLog->Component.outputs).eventTopic)
              ...
              | (Some(JSON.String(_)), Some(_)) =>
                ...
                | None =>
                  log.warn(
                    ...
                    "so it is dispatched without an owner stamp. Any field the command " ++ "marks as its owner keeps the value the caller sent.",
                  )
                  generateFallback
                }
      ...
      let dcbMutationData = stateChangeSlices->Array.flatMap((module(S: StateChangeSlice.T)) => {
        let commandSchema = S.Spec.commandSchema->Reventless.DcbTag.toUnknownSchema
        if ApiNoApiHelpers.isNoApi(commandSchema) {
          []
        } else {
      ...
      HooksConfig.hooks.onDcbSlicesCreated->Option.forEach(hook => hook(dcbEventLog->Obj.magic))
      ...
      let sliceMemoryFloor = slices =>
        slices->Array.reduce(0, (acc, module(M: StateChangeSlice.T)) =>
          Math.Int.max(
            acc,
            ReventlessInfra.RuntimeHints.resolveMemory(
              componentRuntime->Dict.get(M.Spec.name),
              ~default=0,
            ),
          )
        )
      let sliceTimeoutFloor = slices =>
        slices->Array.reduce(0, (acc, module(M: StateChangeSlice.T)) =>
          Math.Int.max(
            acc,
            ReventlessInfra.RuntimeHints.resolveTimeout(
              componentRuntime->Dict.get(M.Spec.name),
              ~default=0,
            ),
          )
      ...
      let automationEntries = automationSlices->Array.map((module(A: AutomationSlice.T)) => {
        let qn = Api_Naming.queryFieldNamesForSliceQueryDb(~plugin=name, ~queryDbName=A.queryDbName)
        {
          ReventlessInfra.Api.singleFieldName: qn.singleFieldName,
          listFieldName: qn.listFieldName,
      ...
      let outboundEntries =
        outboundTranslationSlices->Array.map((module(O: OutboundTranslationSlice.T)) => {
          let qn = Api_Naming.queryFieldNamesForSliceQueryDb(
            ~plugin=name,
            ~queryDbName=O.queryDbName,
          )
          {
            ReventlessInfra.Api.singleFieldName: qn.singleFieldName,
            listFieldName: qn.listFieldName,
      ...
      let inboundEntries =
        inboundTranslationSlices->Array.map((module(I: InboundTranslationSlice.T)) => {
          let qn = Api_Naming.queryFieldNamesForSliceQueryDb(
            ~plugin=name,
            ~queryDbName=I.queryDbName,
          )
          {
            ReventlessInfra.Api.singleFieldName: qn.singleFieldName,
            listFieldName: qn.listFieldName,
