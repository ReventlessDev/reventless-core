...
describe("Seed_Random:", () => {
  ...
  describe("reproducibility — the reason this module exists:", () => {
    ...
    testSync(
      "a fresh generator does not continue the previous one",
      () => {
  ...
  describe("float stays a unit interval:", () => {
    ...
    testSync(
      "and the draws are not all the same value",
      () => {
  ...
  describe("int is inclusive on both ends, as documented:", () => {
    ...
    testSync(
      "a single-value range is that value",
  ...
  describe("pick:", () => {
    ...
    testSync(
      "always lands on a member",
      () => {
        let r = Seed_Random.make(~seed=2)
        let xs = ["a", "b", "c", "d"]
        let hits =
          Array.make(~length=200, "")->Array.map(_ => r->Seed_Random.pickOr(~fallback="MISS", xs))
        expect(hits->Array.every(h => xs->Array.includes(h)))->toBe(true)
      },
    )

    testSync(
      "pickOr falls back only when there is nothing to pick",
      () =>
  ...
  describe("sampleWeighted is without replacement:", () => {
    ...
    testSync(
      "count = length yields a permutation, not a multiset",
      () => {
        let xs = ["a", "b", "c", "d", "e"]
    ...
    testSync(
      "count < length returns exactly count distinct items",
      () => {
        let xs = ["a", "b", "c", "d", "e"]
    ...
    testSync(
      "count > length clamps to the pool",
      () =>
        expect(
    ...
    testSync(
      "a zero-weight entry is never chosen while others remain",
      () => {
        let out =
  ...
  describe("zipfWeights skews toward the head:", () => {
    ...
    testSync(
      "weight decreases monotonically with rank",
      () => {
    ...
    testSync(
      "the leader is clear — first outweighs last several times over",
      () => {
