...
function createSchedule(role) {
  return (queueResources, schedule) => {
    if (queueResources.length !== 0) {
      ...
      return Effect.Effect.runPromise(Effect.Effect.map(Effect.Effect.flatMap(Effect.Effect.promise(() => CloudWatchEvents$AwsSdk.PutRuleCommand.send(new ClientCloudwatchEvents.PutRuleCommand({
        Name: schedule.name,
        ScheduleExpression: toScheduleExpression(schedule.rate),
        RoleArn: role.arn.get(),
        State: "ENABLED"
      }))), param => Effect.Effect.promise(() => CloudWatchEvents$AwsSdk.PutTargetsCommand.send(new ClientCloudwatchEvents.PutTargetsCommand({
        Rule: schedule.name,
        Targets: [{
            Arn: resource.urn,
            Id: resource.name,
    ...
    return Effect.Effect.runPromise(Effect.Effect.flatMap(Effect.Effect.logError(err), () => Effect.Effect.sync(() => Stdlib_JsError.throwWithMessage(err))));
  };
}
...
function deleteSchedule(queueResources, name) {
  if (queueResources.length !== 0) {
    let resource = queueResources[0];
    return Effect.Effect.runPromise(Effect.Effect.map(Effect.Effect.flatMap(Effect.Effect.promise(() => CloudWatchEvents$AwsSdk.RemoveTargetsCommand.send(new ClientCloudwatchEvents.RemoveTargetsCommand({
      Rule: name,
      Ids: [resource.name]
    }))), param => Effect.Effect.promise(() => CloudWatchEvents$AwsSdk.DeleteRuleCommand.send(new ClientCloudwatchEvents.DeleteRuleCommand({
      Name: name
    })))), param => {}));
  }
