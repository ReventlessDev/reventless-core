...
function loadStream(table) {
  return id => Effect$1.Stream.flatMap(Effect$1.Stream.fromEffect(Effect.tryPromise(err => ({
    TAG: "NotLoadedFromStorage",
    _0: Stdlib_Option.getOr(Stdlib_JsExn.message(err), "DynamoDB loadStream error")
  }), () => Util_DynamoDb_Runtime$ReventlessAws.queryById(table, id))), items => Effect$1.Stream.fromIterable(items));
}
