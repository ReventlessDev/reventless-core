...
function loadStream(table) {
  return id => Effect.Stream.flatMap(Effect.Stream.fromEffect(Effect.Effect.tryPromise({
    try: () => Util_DynamoDb_Runtime$ReventlessAws.queryById(table, id),
    catch: err => ({
      TAG: "NotLoadedFromStorage",
      _0: Stdlib_Option.getOr(Stdlib_JsExn.message(err), "DynamoDB loadStream error")
    })
  })), items => Effect.Stream.fromIterable(items));
}
