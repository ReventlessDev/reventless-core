...
function make(name, indexes, opts) {
  ...
  return {
    ...
    operations: Util_DynamoDb$ReventlessAws.toRuntimeTableOutput(table).apply(runtimeTable => {
      ...
      let readStream = (query, after) => Effect.Stream.flatMap(Effect.Stream.fromEffect(Effect.Effect.map(Effect.Effect.tryPromise({
        try: () => readFn(query, after),
        catch: err => Stdlib_Option.getOr(Stdlib_JsExn.message(err), "DcbEventLog read error")
      }), result => result.events)), arr => Effect.Stream.fromIterable(arr));
      return {
        read: readFn,
