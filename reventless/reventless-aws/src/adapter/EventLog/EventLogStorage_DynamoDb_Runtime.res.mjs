// Generated by ReScript, PLEASE EDIT WITH CARE

import * as Effect from "@reventlessdev/rescript-effect/src/Effect.res.mjs";
import * as Effect$1 from "effect";
import * as Stdlib_Math from "@rescript/runtime/lib/es6/Stdlib_Math.js";
import * as Stdlib_JsExn from "@rescript/runtime/lib/es6/Stdlib_JsExn.js";
import * as Stdlib_Option from "@rescript/runtime/lib/es6/Stdlib_Option.js";
import * as Primitive_exceptions from "@rescript/runtime/lib/es6/Primitive_exceptions.js";
import * as Logger$ReventlessCore from "@reventlessdev/reventless-core/src/util/Logger.res.mjs";
import * as Util_Promise$ReventlessCore from "@reventlessdev/reventless-core/src/util/Util_Promise.res.mjs";
import * as DynamoDb_DocumentClient$AwsSdk from "@reventlessdev/rescript-aws-sdk/src/DynamoDb_DocumentClient.res.mjs";
import * as Util_DynamoDb_Runtime$ReventlessAws from "../../util/Util_DynamoDb_Runtime.res.mjs";

function append(table) {
  return async (_sequenceNr, _id, jsons) => {
    let result = Util_DynamoDb_Runtime$ReventlessAws.batchWriteWithRetries(undefined, undefined, Util_DynamoDb_Runtime$ReventlessAws.toTable(jsons.map(Util_DynamoDb_Runtime$ReventlessAws.toPutRequest), table.name));
    let unprocessedItems;
    try {
      unprocessedItems = await result;
    } catch (exn) {
      return {
        TAG: "Error",
        _0: "AwsSdk.DynamoDb.DocumentClient.batchWriteWithRetries failed !"
      };
    }
    if (unprocessedItems.TAG === "Ok") {
      return {
        TAG: "Ok",
        _0: undefined
      };
    }
    Logger$ReventlessCore.error(undefined, undefined, undefined, "Error: unprocessed items:", unprocessedItems._0);
    return {
      TAG: "Error",
      _0: "AwsSdk.DynamoDb.DocumentClient.batchWriteWithRetries resulted in unprocessed items !"
    };
  };
}

async function tryReplay(retryOpt, tableName, id) {
  let retry = retryOpt !== undefined ? retryOpt : 0;
  try {
    return await DynamoDb_DocumentClient$AwsSdk.queryById(tableName, id);
  } catch (raw_e) {
    let e = Primitive_exceptions.internalToException(raw_e);
    if (e.RE_EXN_ID === "JsExn") {
      Logger$ReventlessCore.warn("File \"EventLogStorage_DynamoDb_Runtime.res\", line 23, characters 11-18", undefined, undefined, `Couldn't replay events for id ` + id + `, retry:` + retry.toString(), e._1);
      let timeout = (100 * retry | 0) + Stdlib_Math.Int.random(0, 100) | 0;
      await Util_Promise$ReventlessCore.finishTimeout(timeout);
      return await tryReplay(retry + 1 | 0, tableName, id);
    }
    throw e;
  }
}

function replay(table) {
  return async id => await tryReplay(undefined, table.name, id);
}

function replayStream(table) {
  return id => Effect$1.Stream.flatMap(Effect$1.Stream.fromEffect(Effect.tryPromise(err => Stdlib_Option.getOr(Stdlib_JsExn.message(err), "DynamoDB replay error"), () => tryReplay(undefined, table.name, id))), arr => Effect$1.Stream.fromIterable(arr));
}

function appendStream(table) {
  return (startingSeqNr, id, stream) => {
    let seqNrRef = {
      contents: startingSeqNr
    };
    return Effect$1.Stream.runForEach(stream, json => Effect$1.Effect.flatMap(Effect.tryPromise(err => Stdlib_Option.getOr(Stdlib_JsExn.message(err), "DynamoDB appendStream error"), () => append(table)(seqNrRef.contents, id, [json])), result => {
      if (result.TAG !== "Ok") {
        return Effect$1.Effect.fail(result._0);
      }
      seqNrRef.contents = seqNrRef.contents + 1 | 0;
      return Effect$1.Effect.succeed();
    }));
  };
}

export {
  append,
  tryReplay,
  replay,
  replayStream,
  appendStream,
}
/* Effect Not a pure module */
