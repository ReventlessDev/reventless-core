...
function registerBundledCounter(counterName, targetSpecModulePath, mappingsModulePath, publishQueueUrl) {
  bundledCounterInfos[counterName] = {
    targetSpecModulePath: targetSpecModulePath,
...
function make(name, referencesName, referencesDb, countsName, countsDb, param, opts) {
  ...
  if (info !== undefined) {
    let factoryModulePath = Util_Bundle$ReventlessAws.resolveModule("@reventlessdev/reventless-aws/src/adapter/Runtime/BundledCounterHandlerFactory.mjs");
    ...
    let entryPointCode = Util_EntryPoint$ReventlessAws.generateBundledCounterEntryPoint({
      name: name,
  ...
  console.warn(`CounterHandler_DynamoDbStream_Bundled: no bundled info registered for "` + name + `"`);
  return {
