...
let registerBundledCounter = (~counterName, ~targetSpecModulePath, ~mappingsModulePath, ~publishQueueUrl) =>
  bundledCounterInfos->Dict.set(
    counterName,
    {targetSpecModulePath, mappingsModulePath, publishQueueUrl},
  )
...
let make: ReventlessCore.Counter_Adapter.handlerMaker = (
  ...
  | Some(info) =>
    let factoryModulePath = Util_Bundle.resolveModule(
      "@reventlessdev/reventless-aws/src/adapter/Runtime/BundledCounterHandlerFactory.mjs",
    )
    ...
    let entryPointCode = Util_EntryPoint.generateBundledCounterEntryPoint({
      name,
      targetSpecModulePath: info.targetSpecModulePath,
  ...
  | None =>
    Console.warn(
      `CounterHandler_DynamoDbStream_Bundled: no bundled info registered for "${name}"`,
    )
    {
