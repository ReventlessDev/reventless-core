...
function registerBundledStateViewSlice(name, specModulePath, queryDbTableName) {
  bundledInfos[name] = {
...
function forEventCollector(param, eventTopics, resources, memorySizeOpt, timeoutOpt, eventCollector) {
  ...
  console.log(`StateViewSliceRuntime_Builder_Single_Bundled: registered ` + eventCollectorName + ` for ` + parentName);
}
...
function finish() {
  ...
  if (storedSpecs.length !== 0) {
    ...
    if (parent !== undefined) {
      ...
      let factoryModulePath = Util_Bundle$ReventlessAws.resolveModule("@reventlessdev/reventless-aws/src/adapter/Runtime/BundledStateViewSliceHandlerFactory.mjs");
      ...
      storedSpecs.forEach(spec => {
        ...
        console.warn(`StateViewSliceRuntime_Builder_Single_Bundled: no bundled info registered for ` + spec.sliceName);
      });
      let entryPointCode = Util_EntryPoint$ReventlessAws.generateBundledStateViewSliceEntryPoint({
      ...
      let runtime = RuntimeEnvironment_Lambda$ReventlessAws.makeBundledFromEntryPoint("AllStateViewSlices", entryPointCode, envVars, match[0], match[1], opts);
      ...
      console.warn("StateViewSliceRuntime_Builder_Single_Bundled.finish: grandParent not set");
    }
  }
