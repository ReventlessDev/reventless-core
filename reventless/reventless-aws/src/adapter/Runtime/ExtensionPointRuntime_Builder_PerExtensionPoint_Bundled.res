...
let registerBundledExtensionPoint = (
  ~name,
  ~specModulePath,
  ~mappingsModulePath,
...
> = (
  ...
  | Some(info) =>
    ...
    let factoryModulePath = Util_Bundle.resolveModule(
      "@reventlessdev/reventless-aws/src/adapter/Runtime/BundledExtensionPointHandlerFactory.mjs",
    )
    ...
    let registration: Util_EntryPoint.bundledExtensionPointRegistration = {
    ...
    let entryPointCode = Util_EntryPoint.generateBundledExtensionPointEntryPoint({
      name: epName,
  ...
  | None =>
    Console.warn(
      `ExtensionPointRuntime_Builder_PerExtensionPoint_Bundled: no bundled info for ${epName}`,
    )
  }
}
