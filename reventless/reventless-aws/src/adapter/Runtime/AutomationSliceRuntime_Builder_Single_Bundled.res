    ...
    if storedSpecs->Array.length > 0 {
      ...
      | Some(parent) =>
        ...
        let factoryModulePath = Util_Bundle.resolveModule(
          "@reventlessdev/reventless-aws/src/adapter/Runtime/BundledAutomationSliceHandlerFactory.mjs",
        )
        ...
        storedSpecs->Array.forEach(spec => {
          ...
          | Some(info) =>
            ...
            let registration: Util_EntryPoint.bundledAutomationSliceRegistration = {
          ...
          | None =>
            Console.warn(
              `AutomationSliceRuntime_Builder_Single_Bundled: no bundled info for ${spec.sliceName}`,
            )
          }
        })

        let entryPointCode = Util_EntryPoint.generateBundledAutomationSliceEntryPoint({
      ...
      | None =>
        Console.warn(
          "AutomationSliceRuntime_Builder_Single_Bundled.finish: grandParent not set",
        )
      }
    }
    finished := true
