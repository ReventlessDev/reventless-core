// Async variant of AggregateRuntime_Builder_Micro.
// Uses SQS_Async (FIFO) instead of SQS_Sync — commands return CommandPending immediately.
// Use via Platform.Aggregate.MakeAsync (or directly in _Aws plugin variants) for
// high-contention aggregates where the caller cannot wait for synchronous processing.
...
> = (
  ...
  | None =>
    ...
    JsError.throwWithMessage(
      `forCommandTopic(micro-async): commandTopic ${name} has no Aggregate parent`,
    )
  }
}
...
> = (
  ...
  | None =>
    ...
    JsError.throwWithMessage(
      `forEventCollector(micro-async): eventCollector ${name} has no Aggregate parent`,
    )
  }
}
    ...
    if specs->Array.length > 0 {
      specs->Array.forEach(spec => {
        ...
        | Some(info) =>
          ...
          | (Some(_), None) =>
            Console.warn(
              `AggregateRuntime_Builder_Micro_Async: eventCollector registered for ${spec.aggregateName} but no mappingsModulePath — skipping EventMapper Lambda`,
            )
          | _ => ()
          }
        | None =>
          Console.warn(
            `AggregateRuntime_Builder_Micro_Async: no handler registered for ${spec.aggregateName}`,
          )
        }
      })
    }
