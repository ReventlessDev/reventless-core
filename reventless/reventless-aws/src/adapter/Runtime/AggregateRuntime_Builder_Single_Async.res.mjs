...
function forCommandTopic(param, connect, memorySizeOpt, timeoutOpt, commandTopic) {
  ...
  Stdlib_JsError.throwWithMessage(`forCommandTopic(single-async): commandTopic ` + name + ` has no Aggregate parent`);
}
...
function forEventCollector(param, eventTopics, resources, memorySizeOpt, timeoutOpt, eventCollector) {
  ...
  Stdlib_JsError.throwWithMessage(`forEventCollector(single-async): eventCollector ` + name + ` has no Aggregate parent`);
}
  ...
  if (specs.length !== 0) {
    ...
    if (parent !== undefined) {
      ...
      specs.forEach(spec => {
        ...
        if (info === undefined) {
          return log.warn("AggregateRuntime_Builder_Single_Async", undefined, `no handler registered for ` + spec.aggregateName);
        }
      ...
      let runtime = RuntimeEnvironment_Lambda$ReventlessAws.makeFromCodeAsset("AllAggregatesAsyncCmdHandler", match.code, match.sourceCodeHash, envVars, cfg.memorySize, cfg.timeout, cfg.reservedConcurrency, cfg.ephemeralStorageMb, cfg.logRetentionDays, opts);
      specs.forEach(param => {
        param.connects.forEach(connect => connect(runtime));
      });
      CommandTopicChannel_SQS_Async$ReventlessAws.clearBatchSize();
      let channelSpecs = Stdlib_Array.keepSome(specs.map(param => param.eventCollectorChannelSpec));
      EventCollectorChannel_DynamoDbStream$ReventlessAws.connect("AllAggregatesAsync", channelSpecs, runtime, opts);
    }
  }
