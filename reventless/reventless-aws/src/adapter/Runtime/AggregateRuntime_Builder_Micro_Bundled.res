  ...
  if !finished.contents {
    ...
    if specs->Array.length > 0 {
      ...
      let commandTopicFactoryModulePath = Util_Bundle.resolveModule(
        "@reventlessdev/reventless-aws/src/adapter/Runtime/BundledAggregateHandlerFactory.mjs",
      )

      let commandGeneratorFactoryModulePath = Util_Bundle.resolveModule(
        "@reventlessdev/reventless-aws/src/adapter/Runtime/BundledCommandGeneratorHandlerFactory.mjs",
      )

      let eventMapperFactoryModulePath = Util_Bundle.resolveModule(
        "@reventlessdev/reventless-aws/src/adapter/Runtime/BundledEventMapperHandlerFactory.mjs",
      )
      ...
      specs->Array.forEach(spec => {
        ...
        | Some(info) =>
          ...
          let cmdTopicRegistration: Util_EntryPoint.bundledHandlerRegistration = {
          ...
          let cmdTopicEntryPointCode = Util_EntryPoint.generateBundledAggregateEntryPoint({
          ...
          if spec.commandGeneratorConnects->Array.length > 0 {
            ...
            let cmdGenEntryPointCode = Util_EntryPoint.generateBundledCommandGeneratorEntryPoint({
          ...
          | (Some(channelSpec), Some(mappingsModulePath)) =>
            ...
            let evtMapperEntryPointCode = Util_EntryPoint.generateBundledEventMapperEntryPoint({
          ...
          | (Some(_), None) =>
            Console.warn(
              `AggregateRuntime_Builder_Micro_Bundled: eventCollector registered for ${spec.aggregateName} but no mappingsModulePath — skipping EventMapper Lambda`,
            )
          | _ => ()
          }
        | None =>
          Console.warn(
            `AggregateRuntime_Builder_Micro_Bundled: no bundled info registered for ${spec.aggregateName}`,
          )
        }
      })
    }
