...
let registerBundledTaskBucket = (~bucketName, ~callbackModulePath, ~publishToAggregatesQueueUrls) =>
...
let forBucketCallback = (
  ...
  | Some(info) =>
    let factoryModulePath = Util_Bundle.resolveModule(
      "@reventlessdev/reventless-aws/src/adapter/Runtime/BundledTaskHandlerFactory.mjs",
    )
    ...
    let entryPointCode = Util_EntryPoint.generateBundledTaskBucketEntryPoint({
      name: fullName,
  ...
  | None =>
    Console.warn(
      `TaskRuntime_Builder_PerBucket_Bundled: no bundled info registered for bucket "${name}"`,
    )
  }
}
