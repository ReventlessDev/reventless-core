...
function registerBundledSideEffectHandler(sideEffectHandlerName, sideEffectModulePaths) {
...
function forEventCollector(param, eventTopics, resources, memorySizeOpt, timeoutOpt, eventCollector) {
  ...
  console.log(`SideEffectHandlerRuntime_Builder_Single_Bundled: registered ` + eventCollectorName + ` for ` + parentName);
}
...
function finish() {
  ...
  if (storedSpecs.length !== 0) {
    ...
    if (parent !== undefined) {
      ...
      let factoryModulePath = Util_Bundle$ReventlessAws.resolveModule("@reventlessdev/reventless-aws/src/adapter/Runtime/BundledSideEffectHandlerFactory.mjs");
      ...
      storedSpecs.forEach(spec => {
        let info = bundledSideEffectInfos[spec.sideEffectHandlerName];
        if (info !== undefined) {
          let i = idx.contents;
          let iStr = i.toString();
          ...
          let registration_sideEffectModulePaths = info.sideEffectModulePaths;
          let registration = {
            sideEffectModulePaths: registration_sideEffectModulePaths,
        ...
        console.warn(`SideEffectHandlerRuntime_Builder_Single_Bundled: no bundled info registered for ` + spec.sideEffectHandlerName);
      });
      let entryPointCode = Util_EntryPoint$ReventlessAws.generateBundledSideEffectEntryPoint({
      ...
      let runtime = RuntimeEnvironment_Lambda$ReventlessAws.makeBundledFromEntryPoint("AllSideEffectHandlers", entryPointCode, envVars, match[0], match[1], opts);
      ...
      console.warn("SideEffectHandlerRuntime_Builder_Single_Bundled.finish: grandParent not set");
    }
  }
...
export {
  ...
  registerBundledSideEffectHandler,
  storedSpecs,
  grandParent,
  forEventCollector,
  finished,
