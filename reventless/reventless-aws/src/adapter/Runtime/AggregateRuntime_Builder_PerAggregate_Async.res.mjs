...
function forCommandTopic(param, connect, memorySizeOpt, timeoutOpt, commandTopic) {
  ...
  Stdlib_JsError.throwWithMessage(`forCommandTopic(per-aggregate-async): commandTopic ` + name + ` has no Aggregate parent`);
}
...
function forEventCollector(param, eventTopics, resources, memorySizeOpt, timeoutOpt, eventCollector) {
  ...
  Stdlib_JsError.throwWithMessage(`forEventCollector(per-aggregate-async): eventCollector ` + name + ` has no Aggregate parent`);
}
  ...
  if (specs.length !== 0) {
    specs.forEach(spec => {
      ...
      if (info !== undefined) {
        ...
        let runtime = RuntimeEnvironment_Lambda$ReventlessAws.makeFromCodeAsset(name, code, sourceCodeHash, envVars, spec.memorySize, spec.timeout, aggregateOpts);
      ...
      console.warn(`AggregateRuntime_Builder_PerAggregate_Async: no handler registered for ` + spec.aggregateName);
    });
  }
