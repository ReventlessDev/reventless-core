    ...
    if specs->Array.length > 0 {
      ...
      specs->Array.forEach(spec => {
        ...
        | Some(info) =>
          ...
          let registration: Util_EntryPoint.bundledHandlerRegistration = {
          ...
          let entryPointCode = Util_EntryPoint.generateBundledAggregateEntryPoint({
        ...
        | None =>
          Console.warn(
            `AggregateRuntime_Builder_PerAggregate_Bundled: no bundled info registered for ${spec.aggregateName}`,
          )
        }
      })
    }
