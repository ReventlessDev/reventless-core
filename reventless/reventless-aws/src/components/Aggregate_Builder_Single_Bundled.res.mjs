...
import * as AggregateRuntime_Builder_Single_Bundled$ReventlessAws from "../adapter/Runtime/AggregateRuntime_Builder_Single_Bundled.res.mjs";
...
function Make(Spec) {
  return Behavior => (EventMappings => (Config => {
    let Inner = Aggregate_Builder$ReventlessCore.Make(Spec)(Behavior)(EventMappings)({
      ...
      forCommandGenerator: AggregateRuntime_Builder_Single_Bundled$ReventlessAws.forCommandGenerator,
      forCommandTopic: AggregateRuntime_Builder_Single_Bundled$ReventlessAws.forCommandTopic,
      forEventCollector: AggregateRuntime_Builder_Single_Bundled$ReventlessAws.forEventCollector,
      finish: AggregateRuntime_Builder_Single_Bundled$ReventlessAws.finish
    });
    let make = (api, opts) => {
      ...
      AggregateRuntime_Builder_Single_Bundled$ReventlessAws.registerBundledAggregate(Spec.name, Config.specModulePath, Config.behaviorModulePath, eventLogTableName);
      return aggregate;
    };
    let finish = () => AggregateRuntime_Builder_Single_Bundled$ReventlessAws.finish();
    return {
      Spec: Spec,
      make: make,
