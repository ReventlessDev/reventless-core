...
import * as AggregateRuntime_Builder_Micro_Bundled$ReventlessAws from "../adapter/Runtime/AggregateRuntime_Builder_Micro_Bundled.res.mjs";
...
function Make(Spec) {
  return Behavior => (EventMappings => (Config => {
    let Inner = Aggregate_Builder$ReventlessCore.Make(Spec)(Behavior)(EventMappings)({
      ...
      forCommandGenerator: AggregateRuntime_Builder_Micro_Bundled$ReventlessAws.forCommandGenerator,
      forCommandTopic: AggregateRuntime_Builder_Micro_Bundled$ReventlessAws.forCommandTopic,
      forEventCollector: AggregateRuntime_Builder_Micro_Bundled$ReventlessAws.forEventCollector,
      finish: AggregateRuntime_Builder_Micro_Bundled$ReventlessAws.finish
    });
    let make = (api, opts) => {
      ...
      AggregateRuntime_Builder_Micro_Bundled$ReventlessAws.registerBundledAggregate(Spec.name, Config.specModulePath, Config.behaviorModulePath, eventLogTableName, Config.mappingsModulePath);
      return aggregate;
    };
    let finish = () => AggregateRuntime_Builder_Micro_Bundled$ReventlessAws.finish();
    return {
      Spec: Spec,
      make: make,
