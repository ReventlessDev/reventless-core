...
import * as EventCollectorRuntime_Builder_Single_Bundled$ReventlessAws from "../adapter/Runtime/EventCollectorRuntime_Builder_Single_Bundled.res.mjs";
...
function Make(Spec) {
  return Mappings => (Config => {
    let Inner = ReadModel_Builder$ReventlessCore.Make(Spec)(Mappings)({
      ...
      forEventCollector: EventCollectorRuntime_Builder_Single_Bundled$ReventlessAws.forEventCollector,
      finish: EventCollectorRuntime_Builder_Single_Bundled$ReventlessAws.finish
    });
    let make = (api, apiRole, allEventTopics, opts) => {
      ...
      EventCollectorRuntime_Builder_Single_Bundled$ReventlessAws.registerBundledReadModel(Inner.Spec.name, Config.specModulePath, Config.mappingsModulePath, queryDbTableName);
      return readModel;
    };
    let finish = () => EventCollectorRuntime_Builder_Single_Bundled$ReventlessAws.finish();
    return {
      Spec: Inner.Spec,
      make: make,
