...
function Make(Spec) {
  return Config => {
    ...
    let make = (queryBucketName, scheduler, publishToAggregates, queryEngine, resourceNaming, allAggregates, opts) => {
      Stdlib_Dict.forEachWithKey(Config.callbackModulePaths, (modulePath, bucketName) => TaskRuntime_Builder_PerBucket$ReventlessAws.registerTaskBucket(bucketName, modulePath, Config.publishToAggregatesQueueUrls));
