...
function MakeWithConfig(Config) {
  ...
  let publishRetireForOlderPluginVersions = async (rmTableName, cmdTopicQueueUrl, name, version) => {
    try {
      ...
      for (let i = 0, i_finish = staleIds.length; i < i_finish; ++i) {
        ...
        let metaDict = Object.fromEntries([
          ...
          [
            "time",
            Message$ReventlessCore.nowAsISOString()
          ],
          [
            "msgId",
            msgId
        ...
        let envelopeDict = Object.fromEntries([
          [
            "id",
            id
          ],
  ...
  let hooks_preResolversSchemaHook = (name, version, pluginFragment) => {
    ...
    let pluginAggrCmdTopicUrlOutput = platformStackRef !== undefined ? readStackRefString(Primitive_option.valFromOption(platformStackRef), "pluginAggrCmdTopicUrl") : Pulumi.output(undefined);
    return Output$Pulumi.flatMap(Pulumi.all([
      schemaPersistenceTableNameOutput,
      rmTableNameOutput,
      pluginAggrCmdTopicUrlOutput
    ]), param => {
      let pluginAggrCmdTopicUrlOpt = param[2];
      ...
      return Output$Pulumi.flatMap(targetApi, api => Output$Pulumi.flatMap(api.id, apiId => writeAndScanFragments().then(async allPluginFragments => {
        ...
        if (rmTableNameOpt !== undefined && pluginAggrCmdTopicUrlOpt !== undefined && version !== "") {
          return await publishRetireForOlderPluginVersions(rmTableNameOpt, pluginAggrCmdTopicUrlOpt, name, version);
        }
      })));
    });
  };
  ...
  let deployPlatform = (version, hostUiBundle) => {
    ...
    if (pluginAgg !== undefined) {
      let urlOutput = Output$Pulumi.flatMap(pluginAgg.commandTopic, ct => {
        let res = ct.resources[0];
        if (res !== undefined) {
          return res.id;
        } else {
