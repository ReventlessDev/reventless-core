...
function logCountItems(countItems) {
  groupCountItemsByCounterId(countItems).forEach(param => {
    ...
    Effect.Effect.runSync(Effect.Effect.logInfo(`  ` + size.toString() + ` reference(s) for counterId ` + param[0] + `: ` + referencesStr));
  });
}
...
function Make(Ops) {
  let count = async countItems => {
    ...
    if (result.TAG === "Ok") {
      ...
      Effect.Effect.runSync(Effect.Effect.logInfo("Counter_Operations-ReventlessCore" + (`: saved batch of ` + batchSize.toString() + ` reference(s):`)));
      return logCountItems(countItems);
    }
    let err = result._0;
    if (typeof err === "object" && err.TAG === "NotSavedToStorage") {
      let batchSize$1 = countItems.length;
      Effect.Effect.runSync(Effect.Effect.logError(`Counter error: couldn't save batch of ` + batchSize$1.toString() + ` reference(s):`));
    ...
    Effect.Effect.runSync(Effect.Effect.logError(`Unknown Counter error: couldn't save batch of ` + batchSize$2.toString() + ` reference(s):`));
    logCountItems(countItems);
    throw {
      RE_EXN_ID: NotCounted,
      _1: "Unknown error",
