...
function MakeCounterHandler(Target) {
  return Mappings => (Ops => {
    ...
    let handleCounterEvents = stream => Effect.Effect.flatMap(Stream.runCollect(stream), chunk => Effect.Effect.promise(async () => {
      let match = await commonEventsHandler(chunk);
      if (match[1].length !== 0) {
...
function MakeEventCollectorHandler(Ops) {
  ...
  let handleJsonEvents = stream => Effect.Effect.flatMap(Stream.runCollect(stream), chunk => Effect.Effect.promise(async () => {
    let match = await Ops.commonEventsHandler(chunk);
