...
function Make(ReadModelSpec) {
  return Ops => {
    ...
    let loadStream = id => Effect.Stream.mapEffect(Ops.jsonOps.loadStream(ReadModelSpec.Id.toString(id)), json => {
      let s = decode(id, json);
      if (s.TAG === "Ok") {
        return Effect.Effect.succeed(s._0);
      } else {
        return Effect.Effect.fail(s._0);
      }
    });
    let load = async id => {
