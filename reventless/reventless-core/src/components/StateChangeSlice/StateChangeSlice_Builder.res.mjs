...
function Make(Spec) {
  ...
  let makeJsonHandler = dcbEventLogOps => (stream => Effect.Effect.flatMap(Stream.runCollect(Effect.Stream.flatMap(Effect.Stream.mapEffect(stream, param => {
    ...
    return Effect.Effect.sync(() => {
      ...
      try {
        ...
        Logger$ReventlessCore.error("File \"StateChangeSlice_Builder.res\", line 19, characters 30-37", undefined, undefined, `Couldn't decode command ` + commandStr + `:`, err);
        return;
      }
      return {
        command: command$p,
  ...
  })), decodedItems => Effect.Effect.promise(() => Callback.handleCommands(dcbEventLogOps, decodedItems))));
