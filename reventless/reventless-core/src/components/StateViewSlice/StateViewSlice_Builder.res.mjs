    ...
    let Make = Spec => {
      ...
      let make = (dcbEventLog, opts) => Component$ReventlessCore.make(ComponentType$ReventlessCore.toString(StateViewSlice$ReventlessCore.componentType), Spec.name, (extra, extra$1) => {
        ...
        let eventCollector = Component$ReventlessCore.operations(queryDb).apply(queryDbOps => {
          ...
          let jsonEventsHandler = stream => Effect.Stream.runForEach(Effect.Stream.flatMap(Effect.Stream.mapEffect(stream, json => Effect.Effect.sync(() => {
            try {
          ...
          })), actions => Effect.Stream.fromIterable(actions)), action => Effect.Effect.map(Effect.Effect.promise(() => Projection$ReventlessCore.handleAction(action, projectionOps, undefined)), param => {}));
