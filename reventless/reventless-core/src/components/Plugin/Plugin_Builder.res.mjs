  ...
  return ApiSpec => (FragmentProvider => (RuntimeEnvironment => (EventCollectorChannel => (QueryEngineAdapter => (PluginExtensionPointRemoteChannel => (HeartbeatRunner => (PluginRuntimeBuilder => (DcbEventLogStorage => (DcbEventTopicPublisher => (DcbCommandTopicChannel => {
    let make = (name, heartbeatInterval, extensionPointsOpt, extensionsOpt, aggregatesOpt, readModelsOpt, tasksOpt, stateChangeSlicesOpt, stateViewSlicesOpt, automationSlicesOpt, outboundTranslationSlicesOpt, inboundTranslationSlicesOpt, opts) => {
      ...
      return Component$ReventlessCore.make(ComponentType$ReventlessCore.toString(Plugin$ReventlessCore.componentType), name, (extra, extra$1) => {
        ...
        let scheduler = s !== undefined ? s : Stdlib_Exn.raiseError("Plugin_Builder: scheduler not set — call makePlatform/deployPlugin first");
        let a = Spec.hooks.api.contents;
        let api = a !== undefined ? Primitive_option.valFromOption(a) : Stdlib_Exn.raiseError("Plugin_Builder: api not set — call makePlatform/deployPlugin first");
        let r = Spec.hooks.apiRole.contents;
        let apiRole = r !== undefined ? Primitive_option.valFromOption(r) : Stdlib_Exn.raiseError("Plugin_Builder: apiRole not set — call makePlatform/deployPlugin first");
