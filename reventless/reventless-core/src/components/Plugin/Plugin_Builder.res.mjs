...
function Make(Spec) {
  return ApiSpec => (FragmentProvider => (RuntimeEnvironment => (EventCollectorChannel => (QueryEngineAdapter => (PluginExtensionPointRemoteChannel => (HeartbeatRunner => (PluginRuntimeBuilder => (DcbEventLogStorage => (DcbEventTopicPublisher => (DcbCommandTopicChannel => (DcbCommandTopicChannelAsync => {
    ...
    let makeAutoUIDefinition = (name, aggregatesOpt, readModelsOpt, stateViewSlicesOpt, stateChangeSlicesOpt) => {
      ...
      let toCommandDef = v => {
        if (v.type === "object") {
          return Stdlib_Option.flatMap(v.properties["TAG"], tagSchema => {
      ...
      let extractCommandDefs = commandSchema => {
        if (commandSchema.type === "union") {
          return Stdlib_Array.filterMap(commandSchema.anyOf, toCommandDef);
        } else {
          return [];
        }
      };
      ...
      let stateViewDefs = stateViewSlices.map(SVS => {
      ...
      let stateChangeDefs = stateChangeSlices.map(SCS => ({
        name: SCS.Spec.name,
        commands: extractCommandDefs(SCS.Spec.commandSchema)
      }));
      let aggregateDefs = aggregates.map(A => ({
        name: A.Spec.name,
        commands: extractCommandDefs(A.Spec.commandSchema)
      }));
      return {
    ...
    let make = (name, heartbeatInterval, extensionPointsOpt, extensionsOpt, aggregatesOpt, readModelsOpt, tasksOpt, stateChangeSlicesOpt, stateViewSlicesOpt, automationSlicesOpt, outboundTranslationSlicesOpt, inboundTranslationSlicesOpt, uiFragments, uiDefinition, opts) => {
      ...
      return Component$ReventlessCore.make(ComponentType$ReventlessCore.toString(Plugin$ReventlessCore.componentType), name, (extra, extra$1) => {
        ...
        let pluginOutputs_uiDefinition = Pulumi.output(uiDefinition);
        ...
        let pluginOutputs = {
          ...
          uiDefinition: pluginOutputs_uiDefinition,
    ...
    return {
      ...
      makeAutoUIDefinition: makeAutoUIDefinition
    };
  })))))))))));
}
