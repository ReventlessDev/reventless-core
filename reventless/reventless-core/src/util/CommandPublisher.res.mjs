...
function Make(Spec) {
  return Config => {
    ...
    let finishRunning = async () => {
      ...
      try {
        ...
        if (e.RE_EXN_ID === "JsExn") {
          ...
          return Effect.Effect.runSync(Effect.Effect.logError(`Couldn't publish commands: ` + errMsg));
        }
        throw e;
      }
    };
    let toJsons = commandsToSend => {
      Effect.Effect.runSync(Effect.Effect.logInfo(`toJsons: commandsToSend: ` + commandsToSend.length.toString() + ` rest: ` + buffer.length.toString()));
    ...
    let send = async () => {
      ...
      if (typeof chunkSize !== "object") {
        ...
        try {
          ...
          if (e.RE_EXN_ID === "JsExn") {
            let errMsg = Stdlib_Option.getOr(Stdlib_JsExn.message(e._1), "unknown");
            Effect.Effect.runSync(Effect.Effect.logError(`CommandPublisher: Error: Couldn't publish commands: ` + errMsg));
          } else {
            throw e;
          }
        }
        if (exit === 1) {
          Effect.Effect.runSync(Effect.Effect.logInfo(`CommandPublisher.send: finished SendAllInOneChunk: ` + size.toString()));
        }
      ...
      Effect.Effect.runSync(Effect.Effect.logInfo(`send: bufferSize: ` + bufferSizeStr + `, chunk: ` + chunkCountStr + `, size: ` + sizeStr));
      ...
      try {
        ...
        if (e$1.RE_EXN_ID === "JsExn") {
          ...
          Effect.Effect.runSync(Effect.Effect.logError(`CommandPublisher.send: Error: Couldn't publish chunk ` + chunkCountStr + `: ` + errorMessage));
          let timeout = Stdlib_Math.Int.random(3000, 7000);
          await Util_Promise$ReventlessCore.finishTimeout(timeout);
          Effect.Effect.runSync(Effect.Effect.logInfo(`Retry sending after ` + timeout.toString() + ` ms ...`));
          chunkCount.contents = chunkCount.contents - 1 | 0;
      ...
      if (exit$1 === 1) {
        Effect.Effect.runSync(Effect.Effect.logInfo(`send: finished chunk ` + chunkCountStr + `: ` + sizeStr));
      }
      return await send();
    };
    ...
    let clear = () => {
      Effect.Effect.runSync(Effect.Effect.logInfo("CommandPublisher.clear"));
      buffer.splice(0, 1);
      flush.contents = false;
    };
    let flush$1 = async () => {
      Effect.Effect.runSync(Effect.Effect.logInfo("CommandPublisher.flush"));
      flush.contents = true;
      let match = running.contents;
      if (match === undefined) {
        return await send();
