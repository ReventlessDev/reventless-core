...
function Make(RuntimeEnvironment) {
  return CommandTopicChannel => (EventCollectorChannel => {
    ...
    let runEffect = (correlationId, effect) => Effect.Effect.runPromise(Effect.Effect.provideService(effect, RequestContext$ReventlessCore.tag, {
    ...
    let aggregateHandler = aggregateName => (async (event, context) => {
      ...
      if (info !== undefined) {
        ...
        if (handler !== undefined) {
          Effect.Effect.runSync(Effect.Effect.logInfo(`----- ` + desc + ` found handler for CommandGenerator ` + info));
          return await runEffect(correlationId, handler(event, context));
        } else {
          Effect.Effect.runSync(Effect.Effect.logWarning(desc + ` no handler found: ` + info));
          return "";
        }
      }
      await Promise.all(Object.entries(RuntimeEnvironment.groupBySource(event)).map(async param => {
        ...
        if (handler !== undefined) {
          Effect.Effect.runSync(Effect.Effect.logInfo(`----- ` + desc + ` found handler for CommandTopic ` + urn));
          return await runEffect(correlationId, handler(event, context));
        }
        let handlers = eventCollectorHandlers[urn];
        if (handlers === undefined) {
          return Effect.Effect.runSync(Effect.Effect.logWarning(desc + ` no handler found: ` + urn));
        }
        let count = handlers.length.toString();
        Effect.Effect.runSync(Effect.Effect.logInfo(`----- ` + desc + ` found ` + count + ` handler(s) for EventCollector ` + urn));
