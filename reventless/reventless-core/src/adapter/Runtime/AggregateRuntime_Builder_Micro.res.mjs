...
function Make(RuntimeEnvironment) {
  return CommandTopicChannel => (EventCollectorChannel => {
    let forCommandGenerator = (handler, connect, memorySizeOpt, timeoutOpt, commandGenerator) => {
      ...
      return connect(RuntimeEnvironment.make(ComponentType$ReventlessCore.nameOpt(resource.__name, CommandGenerator$ReventlessCore.componentType), handler.apply(handler => RuntimeEnvironment.asEventHandler(handler)), memorySize, timeout, {
        parent: resource
      }));
    };
