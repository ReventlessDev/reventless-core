...
function Make(Spec) {
  return MappingImpl => {
    ...
    let mapIncomingEvent = (extra, extra$1, extra$2) => {
      ...
      let encodeExtensionPointCommand = (command, id, extensionPointName, action) => encodeExtensionPointCommandJson(Message$ReventlessCore.encode(command, Spec.commandSchema), id, extensionPointName, action);
      return mapIncomingEventImpl(extra.id, extra.event, meta, extra$1, extra$2).map(x => {
        switch (x.TAG) {
          ...
          case "PublishExtensionPointCommand" :
            let command = x._1;
            let id = x._0;
            if (Spec.name === PluginExtensionPointSpec$ReventlessCore.name) {
              return {
          ...
          case "Call" :
            let directive = x._1;
            let handler = x._0;
            console.log(`ExtensionMapping incoming from ExtensionPoint ` + extensionPointName + `: Handling directive`, JSON.stringify(Message$ReventlessCore.encode(directive, Spec.directiveSchema)));
            return {
              TAG: "AbstractCall",
      ...
      return mapOutgoingEventImpl(Aggregate.Id.toString(val.id), val.event, meta, extra$1).map(x => {
        switch (x.TAG) {
          ...
          case "Call" :
            let directive = x._1;
            let handler = x._0;
            console.log(`ExtensionMapping outgoing from Aggregate ` + aggregateName + `: Handling directive`, JSON.stringify(Message$ReventlessCore.encode(directive, Spec.directiveSchema)));
            return {
              TAG: "AbstractCall",
              _0: () => handler(directive)
            };
