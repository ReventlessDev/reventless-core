...
function Make(Spec) {
  let forwardCommand = async (_id, command, extensionPointName, queryEngine) => {
    ...
    if (jsons.length === 0) {
      return Effect.Effect.runSync(Effect.Effect.logWarning(`ForwardCommand: Couldn't find Plugin with ExtensionPoint ` + extensionPointName));
    }
    let plugin = jsons[0];
    let exit = 0;
    let plugin$1;
    try {
      ...
      return Effect.Effect.runSync(Effect.Effect.logError(`ForwardCommand: Couldn't decode Plugin: ` + errMsg));
    }
    if (exit === 1) {
      ...
      if (extensionPoint === undefined) {
        return Effect.Effect.runSync(Effect.Effect.logWarning(`ForwardCommand: Couldn't find ExtensionPoint ` + extensionPointName + ` in ` + plugin$1.name));
      }
      let exit$1 = 0;
      let val;
      try {
        ...
        return Effect.Effect.runSync(Effect.Effect.logError(`PluginExtensionPoint_PluginMapping: Error on publish command: ` + errMsg$1));
      }
      if (exit$1 === 2) {
        return Effect.Effect.runSync(Effect.Effect.logInfo(`ForwardCommand: published command to ` + plugin$1.name + ` ` + extensionPoint.commandTopic));
      }
    }
  };
  ...
  let mapIncomingCommand = (id, cmd, _meta) => {
    ...
    switch (cmd.TAG) {
      ...
      case "ConnectPlugin" :
        ...
        let reportAction = protocolErrors.length !== 0 ? (Effect.Effect.runSync(Effect.Effect.logWarning(`[Core.Plugin] Protocol version mismatch for plugin ` + pluginDefinition.id + `: ` + Stdlib_Option.getOr(JSON.stringify(protocolErrors), "[]"))), [{
              TAG: "PublishCommand",
              _0: id,
              _1: {
