...
describe("Aggregate_Callback.handleCommands:", () => {
  describe("single command — new aggregate", () => {
    test("Create returns Ok(reference) and stores 1 event", async () => {
      let results = await AggregateFixtures$ReventlessCore.TestHandler.handleCommands([AggregateFixtures$ReventlessCore.makeTopicItem(undefined, "ref-1", {
          TAG: "Create",
          name: "Widget"
        })]);
      expect([
  ...
  describe("command on existing aggregate", () => {
    test("Rename after Create replays history and appends new event", async () => {
      await AggregateFixtures$ReventlessCore.TestHandler.handleCommands([AggregateFixtures$ReventlessCore.makeTopicItem(undefined, "ref-1", {
          TAG: "Create",
          name: "Widget"
        })]);
      let results = await AggregateFixtures$ReventlessCore.TestHandler.handleCommands([AggregateFixtures$ReventlessCore.makeTopicItem(undefined, "ref-2", {
          TAG: "Rename",
          newName: "Gadget"
        })]);
      expect([
  ...
  describe("multiple commands for same aggregate ID", () => {
    test("Create then Rename in one batch accumulate state correctly", async () => {
      let results = await AggregateFixtures$ReventlessCore.TestHandler.handleCommands([
