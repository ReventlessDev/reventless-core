...
Jest.describe("StateChangeSlice_Callback:", () => {
  Jest.describe("handleCommands - happy path", () => {
    Jest.testPromise("CreateItem on empty log succeeds", undefined, async () => {
      let results = await Effect.Effect.runPromise(TestHandler.handleCommands(testDcbEventLog, Effect.Stream.fromIterable([makeTopicItem("ref-1", {
          TAG: "CreateItem",
          itemId: "item-1",
          name: "Test"
        })])));
    ...
    Jest.testPromise("stored event has correct tags", undefined, async () => {
      await Effect.Effect.runPromise(TestHandler.handleCommands(testDcbEventLog, Effect.Stream.fromIterable([makeTopicItem("ref-1", {
          TAG: "CreateItem",
          itemId: "item-1",
          name: "Test"
        })])));
  ...
  Jest.describe("handleCommands - decide returns Ok([])", () => Jest.testPromise("NoOp returns Ok without storing events", undefined, async () => {
    let results = await Effect.Effect.runPromise(TestHandler.handleCommands(testDcbEventLog, Effect.Stream.fromIterable([makeTopicItem("ref-noop", "NoOp")])));
    return Jest.Expect.toEqual(Jest.Expect.expect([
      results,
      mock.getEvents().length
    ]), [
  ...
  Jest.describe("handleCommands - decide returns Error", () => Jest.testPromise("CreateItem when item exists returns Error", undefined, async () => {
    ...
    let results = await Effect.Effect.runPromise(TestHandler.handleCommands(testDcbEventLog, Effect.Stream.fromIterable([makeTopicItem("ref-2", {
        TAG: "CreateItem",
        itemId: "item-1",
        name: "Duplicate"
      })])));
  ...
  Jest.describe("handleCommands - batch", () => {
    Jest.testPromise("multiple successful commands", undefined, async () => {
      let results = await Effect.Effect.runPromise(TestHandler.handleCommands(testDcbEventLog, Effect.Stream.fromIterable([
        makeTopicItem("ref-1", {
          TAG: "CreateItem",
          itemId: "item-1",
          name: "First"
    ...
    Jest.testPromise("mixed success and failure", undefined, async () => {
      await Effect.Effect.runPromise(TestHandler.handleCommands(testDcbEventLog, Effect.Stream.fromIterable([makeTopicItem("ref-0", {
          TAG: "CreateItem",
          itemId: "item-1",
          name: "Existing"
        })])));
