// Generated by ReScript, PLEASE EDIT WITH CARE

import * as S from "sury/src/S.res.mjs";
import * as StateViewSlice_GWT$ReventlessGwt from "../src/StateViewSlice_GWT.res.mjs";

let stateSchema = S.schema(s => ({
  categoryId: s.m(S.string),
  name: s.m(S.string),
  archived: s.m(S.bool)
}));

let consumedEventSchema = S.union([
  S.schema(s => ({
    TAG: "CategoryAdded",
    categoryId: s.m(S.string),
    name: s.m(S.string)
  })),
  S.schema(s => ({
    TAG: "CategoryRenamed",
    categoryId: s.m(S.string),
    name: s.m(S.string)
  })),
  S.schema(s => ({
    TAG: "CategoryArchived",
    categoryId: s.m(S.string)
  }))
]);

function project(event) {
  switch (event.TAG) {
    case "CategoryAdded" :
      let categoryId = event.categoryId;
      return [{
          TAG: "Set",
          _0: categoryId,
          _1: {
            categoryId: categoryId,
            name: event.name,
            archived: false
          }
        }];
    case "CategoryRenamed" :
      let name = event.name;
      return [{
          TAG: "Update",
          _0: event.categoryId,
          _1: state => ({
            categoryId: state.categoryId,
            name: name,
            archived: state.archived
          })
        }];
    case "CategoryArchived" :
      return [{
          TAG: "Update",
          _0: event.categoryId,
          _1: state => ({
            categoryId: state.categoryId,
            name: state.name,
            archived: true
          })
        }];
  }
}

let CategoriesViewSpec = {
  name: "CategoriesView",
  stateSchema: stateSchema,
  consumedEventSchema: consumedEventSchema,
  project: project,
  subIdConfig: undefined
};

let include = StateViewSlice_GWT$ReventlessGwt.Make(CategoriesViewSpec);

let describe = include.describe;

let test = include.test;

let givenEvents = include.givenEvents;

let whenEvent = include.whenEvent;

let thenStateWithId = include.thenStateWithId;

describe("CategoriesView StateViewSlice", () => {
  test("projects CategoryAdded into a new row", undefined, () => thenStateWithId(whenEvent(givenEvents([]), {
    TAG: "CategoryAdded",
    categoryId: "c1",
    name: "Electronics"
  }), "c1", {
    categoryId: "c1",
    name: "Electronics",
    archived: false
  }));
  test("CategoryRenamed updates the name", undefined, () => thenStateWithId(whenEvent(givenEvents([{
      TAG: "CategoryAdded",
      categoryId: "c1",
      name: "Electronics"
    }]), {
    TAG: "CategoryRenamed",
    categoryId: "c1",
    name: "Consumer Electronics"
  }), "c1", {
    categoryId: "c1",
    name: "Consumer Electronics",
    archived: false
  }));
});

let Spec = include.Spec;

let describeWithId = include.describeWithId;

let whenEvents = include.whenEvents;

let thenStates = include.thenStates;

let thenStatesWithId = include.thenStatesWithId;

let thenAllStates = include.thenAllStates;

let thenState = include.thenState;

let thenNoState = include.thenNoState;

let thenThrow = include.thenThrow;

let thenFail = include.thenFail;

export {
  CategoriesViewSpec,
  Spec,
  describe,
  describeWithId,
  test,
  givenEvents,
  whenEvent,
  whenEvents,
  thenStates,
  thenStatesWithId,
  thenAllStates,
  thenState,
  thenStateWithId,
  thenNoState,
  thenThrow,
  thenFail,
}
/* stateSchema Not a pure module */
