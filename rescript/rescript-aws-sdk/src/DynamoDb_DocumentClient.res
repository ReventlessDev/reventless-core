...
let rec queryRecursive: (
  ...
  let (allResults, lastEvaluatedKey) = switch await QueryCommand.send(params->QueryCommand.make) {
  | res => (
      {
        ...res,
        metadata: {
          attempts: allResults.metadata.attempts + res.metadata.attempts,
          ...
          totalRetryDelay: res.metadata.totalRetryDelay + res.metadata.totalRetryDelay,
        },
        items: allResults.items
        ->Option.getOr([])
        ->Array.concat(res.items->Option.getOr([])),
        count: allResults.count->Option.getOr(0) + res.count->Option.getOr(0),
        scannedCount: allResults.scannedCount->Option.getOr(0) + res.scannedCount->Option.getOr(0),
      },
  ...
  | Some(lastEvaluatedKey) =>
    ...
    await queryRecursive(~params, ~allResults)
  }
}

let queryById = async (tableName, id) => {
  switch await queryRecursive(
    ~params={
      tableName,
      consistentRead: true,
...
let rec scanRecursive: (
  ...
  let (allResults, lastEvaluatedKey) = switch await ScanCommand.send(ScanCommand.make(params)) {
  | res => (
      {
        ...res,
        metadata: {
  ...
  | Some(lastEvaluatedKey) =>
    ...
    await scanRecursive(~allResults, ~params)
  }
}
