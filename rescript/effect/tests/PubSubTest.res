...
describe("PubSub", () => {
  testPromise("unbounded pubsub: publish + subscribe + Queue.take delivers message", async () => {
    ...
    ->Effect.flatMap(queue => PubSub.publish(ps, "hello")->Effect.zipRight(Queue.take(queue)))
    ->Effect.scoped
    ->Effect.runPromise
    expect(v)->toBe("hello")
  })
  ...
  testPromise("size reflects subscriber count", async () => {
    ...
    ->Effect.flatMap(_q1 => PubSub.subscribe(ps)->Effect.flatMap(_q2 => PubSub.size(ps)))
    ->Effect.scoped
    ->Effect.runPromise
  ...
  testPromise("publishAll delivers all items to subscriber", async () => {
    ...
    ->Effect.flatMap(
      queue => PubSub.publishAll(ps, [1, 2, 3])->Effect.zipRight(Queue.takeAll(queue)),
    )
    ->Effect.scoped
    ->Effect.runPromise
