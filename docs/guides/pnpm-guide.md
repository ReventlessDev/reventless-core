...
- Root `pnpm-workspace.yaml` lists workspace globs (`packages/*`,
  `rescript/*`, `reventless/*`, `examples/**`) and `onlyBuiltDependencies`.
...
Summary: a gitignored `pnpm-workspace.local.yaml` adds sibling repo paths
to the workspace; `pnpm link:on` / `link:off` toggle between link mode
(sibling is live source) and release mode (registry version).
...
- **Duplicated-package warnings** from ReScript are hoisting artifacts,
  ...
  `workspace:*`.
- **Do not commit `pnpm-workspace.yaml` with the `# GENERATED by …`
  header.** That means link-mode is active; run `pnpm link:off` first.
  See the cross-repo guide for details.
