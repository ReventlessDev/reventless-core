...
| `comp` | when provided | Component (e.g. `Aggregate(Product)`) |
...
### 4. Command-handler activity by request id
...
### 5. Busiest components (where is the log volume?)
...
### 6. Truncated detail payloads (find logs dropping data)
...
- **Cross-Lambda tracing** is the payoff of `correlationId`: it is set at the
  request boundary (`runEffect`) and rides every `EffectLogger` line inside that
  scope automatically, plus every Lambda entry-point shim annotates it.
