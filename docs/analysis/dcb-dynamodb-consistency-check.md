...
**Status:** The duplicate-read amplifier is gone, and the slice's retry loop is now real (it fires on actual conflicts). The remaining `(1 + retries)` decision-model reads are now **delta reads** on warm instances: the projection cache (shipped 2026-06-20, plan [`dcb-decision-model-projection-cache`](../plans/done/dcb-decision-model-projection-cache.md)) re-seeds from the just-read snapshot on conflict, so each retry reads only the events the conflicting writer added rather than full history. A cold instance still pays one full read; the worst-case multiplier is gone for hot entities.
