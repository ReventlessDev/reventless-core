...
| **Always-in-process adapter** (no `_Sqlite` sibling) | `InMemory_Bus`, `CommandTopicChannel_InMemory`, `ScheduledPublisher_InMemory` | **Drop the suffix entirely** → `Bus`, `CommandTopicChannel`, `ScheduledPublisher`. The suffix is noise here: there is no alternative backend to discriminate against. (Per **decided OQ1** — normalize these.)|

A literal "rename everything in-memory to local" is the wrong instruction. The decided policy makes the suffix **mean something**: `_InMemory` appears **only** where a real `_InMemory` ⇄ `_Sqlite` choice exists; everywhere else the suffix is dropped. Identity moves to `local`; the three persisted storage surfaces keep their backend label; the always-in-process plumbing sheds the suffix.
