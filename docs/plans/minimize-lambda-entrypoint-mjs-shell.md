...
- [x] **Tier 2.5 — Aggregate functor wiring. Verifiability gap closed; typing gap proven un-closable under dynamic import.** The follow-up first built the missing guard — `AggregateEntryPoint_IntegrationTest.res` + `AggIntegrationHarness.res` (plain `id`/`position` EventLog table) + `AggTestAggregate`/`…Behavior` fixtures — driving the real `cmdGenHandler` through CommandGenerator → CommandTopic → Aggregate callback → EventLog append on DynamoDB Local to `CommandAccepted`. To enable it, `AggregateEntryPoint.mjs` gained the injectable-loader `buildHandlersForConfig` export (mirrors DCB). Green.

  With the guard in hand, the wiring itself was re-examined and **deliberately left in the shell** — this time on proof, not caution. Aggregate's wiring is three chained functors (`EventLog_Operations.Make`, `Aggregate_Callback.Make`, `CommandTopic_Callback.Make`) applied to **dynamically-imported opaque modules**. A ReScript functor can only be applied to a module at compile time; the loaded modules are runtime values, so any port to ReScript must bind the functors as `external`s. But an `external` is unchecked FFI — ReScript trusts the hand-written type, so the framework's real `Ops` signatures are **not** checked against the core's usage; a framework field-rename would still compile. Moving the wiring to `.res` therefore adds `Obj.magic` ceremony with **zero** new type safety — the "ReScript in name only" anti-pattern this whole effort exists to avoid. The only genuinely-typed part (the EventLog storage ops) was already captured in Tier 1. True end-to-end typing here would require statically importing the plugin modules at deploy time (Alternative D) — a separate architecture decision, not an incremental cleanup. Net closure: the **verifiability** gap (the real reason for the earlier deferral) is closed by the new harness; the typing gap is proven inherent to dynamic import.
