...
- The handler signatures differ across domains (ExtensionPointMapping's `directiveHandler` carries Schedule + QueryEngine; ExtensionMapping uses `Reventless.Handler.handler<'directive>` with no infra args). That asymmetry is intentional — extension points run "inside" the framework with access to scheduling/query infra; extensions are user code. Leave it. If we ever unify them, do it in a follow-up plan.
