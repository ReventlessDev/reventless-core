...
**Status**: DONE. Core cache shipped 2026-06-20 (roadmap Phase 4); Step 5 hit/miss + delta metrics shipped 2026-07-25. Step 4 (per-slice capacity knob) is parked — a build-time tuning knob gated on a real workload shown to be mis-served by the flat-100 default, and it needs a reventless-ppx republish (a required Spec field), so it lands opportunistically with the next PPX pass rather than holding this plan open. Its full design is retained in §Status → Step 4 below.
...
**Sibling plan**: [dcb-dynamodb-atomic-append.md](dcb-dynamodb-atomic-append.md)
...
**Core shipped — 2026-06-20** (roadmap Phase 4). Steps 1–3, the Step 6 analysis-doc update, and **Step 5 (hit/miss + delta metrics, 2026-07-25)** are done; Step 4 (per-slice capacity knob) remains a deferred follow-up (it needs a PPX republish — see below).
...
- **Step 5 — cache hit/miss metrics. — DONE (2026-07-25).** Per-slice signals so operators can see whether the cache helps a given slice in production: `DcbDecisionModelCacheHit` / `DcbDecisionModelCacheMiss` counters and a `DcbDecisionModelDeltaEventCount` histogram. If `Miss` dominates `Hit`, the cache is just memory pressure for that workload — the operator's cue to set its capacity to `0` (Step 4). Reused the Phase 5a primitives with **no Spec change, no PPX/republish** (exactly as scoped):
  - **Core** ([`StateChangeSlice_Callback.res`](../../../reventless/core/src/components/StateChangeSlice/StateChangeSlice_Callback.res)): one `CacheHit`/`CacheMiss` sample per command, keyed on whether the initial `cacheGet()` seeded the decision read (emitted once, before the retry loop — retries don't inflate it). One `DcbDecisionModelDeltaEventCount` per hit, emitted on the **first attempt only** (`cacheHit && retries == maxRetries`) with `value = delta events read` — so samples align 1:1 with `CacheHit`, are near-zero on a healthy warm hit, and grow only when the entity churns between commands. Conflict-driven retry reads stay covered by `AppendRetry`. Emitted via the same `Metrics.emitCount` as `AppendRetry`/`AppendConflict` (JSON-sink-gated, provider-neutral).
  - **AWS** ([`RuntimeEnvironment_Lambda.res`](../../../reventless/aws/src/adapter/Runtime/RuntimeEnvironment_Lambda.res)): the three metric names appended to the existing `dcbMetrics` `LogMetricFilter` array (namespace `Reventless/DCB`, `slice` dimension, `$.value`) — same managed-log-group constraint as `AppendRetry`/`AppendConflict` (needs `logRetentionDays` set; otherwise the raw lines are still queryable via Logs Insights).
  - **Tests**: new `handleCommands - projection cache metrics` case in [`DcbStateChangeSliceTest.res`](../../../reventless/core/tests/dcb/DcbStateChangeSliceTest.res) captures the emitted metric lines (typed `console` binding, mirroring `LogFormatTest`'s `captureLogs` — no `%raw`/`Obj.magic`) and asserts the hit/miss sequence `[Miss, Hit, Hit]` over three same-entity commands plus one bounded delta sample per hit. Full core suite green (523).
  - **Remaining**: emit signals shipped; wiring a CloudWatch dashboard/alarm off `Reventless/DCB` is an operator task, not framework code.
