...
| Part 3 — zero-downtime synthetic heartbeat + deploy contract | ✅ done (deploy-path unverified) | 3.1 deploy-time synthetic heartbeat at end of `deployPlugin` (reuses runtime heartbeat path — `Heartbeat(timeout)` onto EP FIFO queue via `CommandTopicChannel_SQS_Runtime.publishJsons`; `Pulumi.isDryRun()`-guarded; graceful-degrades to next natural tick). 3.2 fast rollback = existing write-side `VersionPromoted` re-promote rule (GWT-covered) — documented, no new code. 3.3 expand/contract contract + handover mechanics documented in the live-updates guide. Builds green; **3.1 SQS send only exercises on a real `pulumi up`** (can't verify locally) |
