...
- Platform wiring is split-state across local and AWS — see Phase 6c. (Original deferral note said "AWS missing the ReadModel"; the real picture is the opposite for the ReadModel and the AWS platform is missing the cross-plugin resolver instead.)
...
## ~~Phase 6c — Local PlatformEventGraph resolvers~~ ✅ DONE
...
**Validation.** Build clean (zero warnings), 375 local tests pass.

**Commit message.** `feat(local): expose Platform_PlatformEventGraph[s] admin queries from pluginStructuresStore`
