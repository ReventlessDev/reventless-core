...
- 2026-07-02 — **A6 DcbEventLog_Builder wiring done.** The standalone-DCB-log builder now uses `DcbEventLogStorage_InMemory.Make(Bus)` (the backend-aware functor, as Platform does) instead of the plain module: it honours `REVENTLESS_LOCAL_BACKEND=sqlite` and registers the read with the Bus so `Bus.getDcbEventLogRead` sees it. Local suite green (421). **Still deferred (each needs a design decision, not a surgical fix):** (a) *typed conflict sentinel end-to-end* — a 4-package interface change (infra/core/local/aws) vs. the current string sentinel, which A6 already made reliable by ensuring only genuine conflicts emit it; robustness nicety, not a live bug. (b) *QueryDb `count` running-total* — DynamoDB couples the counter to the item attribute read back via `loadStream`; a faithful local version needs item-key/subId semantics and create-on-missing, and no test pins it, so a wrong guess could diverge from `loadStream` or break local counting flows. (c) *InMemory TTL* — session-scoped in-memory has no expiry mechanism; cosmetic in dev. (d) *`ensureUIFragmentRegistryQueryDbStore` SQLite persistence*.
