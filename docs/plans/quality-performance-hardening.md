...
**Still deferred (cosmetic / low-value, "align or return not-implemented"):** (c) *InMemory TTL* — session-scoped in-memory has no expiry mechanism; cosmetic in dev. (d) *`ensureUIFragmentRegistryQueryDbStore` SQLite persistence*.
