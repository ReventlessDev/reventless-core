...
- **AWS (next, deploy-time): CloudWatch metric filter.** `reventless-aws` translates the generic line into a CloudWatch metric via a Pulumi `LogMetricFilter` on the command-handler Lambda log groups. Requires: (1) a new `Cloudwatch_LogMetricFilter` binding in `rescript-pulumi-aws` (none today); (2) scoping to command-handler Lambdas (the log group in [`RuntimeEnvironment_Lambda.res:167`](../../reventless/reventless-aws/src/adapter/Runtime/RuntimeEnvironment_Lambda.res#L167) is generic and only managed when `logRetentionDays` is set — needs a flag + that managed log group); (3) a metric transformation extracting `slice` as a dimension + `value`, namespace `Reventless/DCB`. Until this lands, the signal is already queryable/alarmable via Logs Insights or a hand-defined filter.
