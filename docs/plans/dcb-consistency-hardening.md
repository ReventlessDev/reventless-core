...
**Per-slice strong-read override — DONE / released.** Shipped and merged: the reventless-ppx republish landed (now pinned at **alpha.51**, past the alpha.42 that carried this pass), and the injected `readConsistency` field is present in the compiled example slices (`AddCategory.res.mjs` etc.), so CI compiles against the published binary. A build-time `readConsistency` mode per StateChangeSlice, mirroring `authorization`/`visibility`:
...
5. **Phase 5** items opportunistically — Issue 14 (vacuous-clause cleanup), Issue 12 (fence-scan hardening), Issue 5 (composite exact-match guard) **done (2026-06-20)**, and Phase 5a (opt-in strong reads + contention metric) **done (2026-06-21)**, including the per-slice strong-read override PPX follow-up **done / released (ppx alpha.51, live on alpha)**; remaining items (Issues 4/6/9) land opportunistically. **Phase 6** hot-fence contention **done 2026-07-07** (composite-fence fix, plan in `done/`); passive live re-check on next deploy.
