...
**Contention metric (NOT deferred — ships with this item).** Emit a per-slice CloudWatch metric for append retries and exhausted-retry conflicts, sourced from the existing retry loop in [`StateChangeSlice_Callback.res:309-330`](../../reventless/reventless-core/src/components/StateChangeSlice/StateChangeSlice_Callback.res#L309) (currently log-only: `WARN "append failed (retrying N/3)"`, `ERROR "append failed, retries exhausted"`, `CommandRejected{errorCode:"Conflict"}`). Use EMF (embedded-metric-format log lines) so no extra API calls. Dimensions: slice name (+ plugin). This turns "grep the logs" into a dashboard/alarm so the eventual-default flip can be watched, and gives an operator the signal to flip a hot slice back to strong.
