...
In progress — moved out of Backlog 2026-07-03. **Steps 1–4 are code-complete** (see
progress log): the Phase 1 replay cache ships, all three backends implement
`latestSnapshot`/`writeSnapshot`/`replayStream(~fromSeq)`, and the Step 2 config surface
(`Reventless.Snapshot`, `Behavior.T.snapshot`, PPX default injection +
`@@reventless.snapshots(N)`) is written and locally verified. **⚠ Gate: CI stays red
until reventless-ppx is republished** (the published binary doesn't inject `snapshot`);
after the republish, regenerate the example `.res.mjs` (this + the pending
`externalSystem` injection) in one commit. Remaining: **Step 5** (callback wiring —
unblocked in code, lands green only after the republish), **Step 6** (end-to-end
example), **Step 7** (docs). The SQLite snapshot table shipped ahead of B5's
persisted-data-versioning decision; fold its migration story in when that lands.
