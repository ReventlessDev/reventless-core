...
gate is cleared**, and the example `.res.mjs` are regenerated. Remaining: **Step 5**
(callback wiring: cold path through `latestSnapshot`, `SchemaWalker` hash gate,
boundary-crossing fire-and-forget writes — now lands green), **Step 6** (enable
