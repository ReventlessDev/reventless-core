...
In progress — moved out of Backlog 2026-07-03. **Steps 1–4 are done** (see progress log):
the Phase 1 replay cache ships, all three backends implement
...
`@@reventless.snapshots(N)`) is landed — **reventless-ppx republished to alpha.49, the CI
gate is cleared**, the example `.res.mjs` are regenerated, and **Step 5** (callback
wiring: cold path through `latestSnapshot`, `SchemaWalker` hash gate, boundary-crossing
fire-and-forget writes) is done and green. Remaining: **Step 6** (enable
`@@reventless.snapshots(N)` on an example + SQLite restart smoke), **Step 7** (docs). The
SQLite snapshot table shipped ahead of B5's persisted-data-versioning decision; fold its
migration story in when that lands.
