...
**What CI and the deploy added, same day.** The push ran all of the above again on a clean checkout —
goldens byte-identical there too — plus the two Docker-backed integration suites this machine could
not run. The alpha deploy of `online-shop-hybrid` then succeeded on the tip carrying the mechanism,
with **no `unclassifiedUnions` warning in the log**: the new deploy-time report stayed quiet against
the real platform, which is the right answer while the `Plugins` view's opaque `JSON.t` fields are
deliberately outside what it reports on.
...
**What this plan deliberately cannot verify — and the deploy did not change it.** Nothing here proves
a union field survives a *deployed* read. The deploy above proves non-regression, which is a different
